...
## 14. Rekonsiliasi lanjutan GPT BC (run mandiri GPT 36182512996 rev1 alat gagal, 36182902112 run2; `out/gpt_bc_native_followup_run2.md`) — 26 Sep ~03:05 WIB
| Klaim GPT | Verifikasi/pandangan Fable | Status bersama |
|---|---|---|
| GBC-1: key custody baru yang "mengklaim barang sama" lewat `notes` diinspeksi + dinilai → stok 3→6, jurnal +6 dua kali; kontrol fisik baru +3 → 9. Status INCOMPLETE/NEEDS_SOURCE_IDENTITY_POLICY | Sama dengan batas yang writer catat (`limit.new_custody_key=ACCEPTED`) dan §12 saya: sistem tidak punya identitas fisik untuk item pending; `notes` bukan bukti kesamaan. Bukan cacat kode yang pasti, bukan PASS. | **SETUJU: ACC-C12 PARTIAL/UNVERIFIED sampai owner menetapkan aturan identitas sumber** (pilihan a/b di §12). Diteruskan ke owner. |
| GBC-2: di browser, enam mandor seed UUID non-RFC aktif → halaman Nota Ambil kosong ("Identitas aksesori tidak valid atau ganda"), RPC OWNER 200; setelah seed dinonaktifkan hanya di klon + kontrol v4 → halaman jalan. GPT: **P2 bersyarat** | Sama dengan verifikasi sumber saya (§3a) dan browser D09 writer. Perbedaan hanya label: saya P3 (ID buatan aplikasi = v4, lolos), GPT P2 bersyarat (bila data cutover memuat ID aktif non-RFC). | **Rumusan bersama: F3 = P3 pada produk dengan ID buatan aplikasi; naik P2 bila inventaris data cutover/hosted (baca-saja, oleh writer/operator, bukan auditor) menemukan kontraktor aktif ber-ID non-RFC.** Perbaikan sempit tetap disarankan (UUID kanonik). |
| GBC-3 lintas tab: belum dijalankan GPT | Sudah dijalankan Fable dengan skenario sendiri: run 36182433079 `FAB_BROWSER:STOCK_SEARCH_VISIBLE_AND_CLEARABLE_IN_DOCUMENTS` PASS (§13). | GPT boleh mengutip dengan label REUSED_FABLE; atau jalankan sendiri. |
| product_ref GPT = 40d6906 | `run_identity` menghitung `supabase/dev` sebagai jalur produk, sehingga berkas dev BD (40d6906) menggeser product_ref; produk BC (src + paket rilis) tetap 27e1a05. | Catatan identitas saja, bukan perbedaan produk. |
| Job GPT merah "by design" (kasus INCOMPLETE) | Konsisten dengan aturan: INCOMPLETE membuat job merah; tidak dilabel ulang. | OK |
