...
## 7. Cek silang GPT BB round 10 (`out/gpt_bb_round10_result.md`, run 36155477550 rev2 pada 4c61aca)
| Kasus GPT | Hasil GPT | Bacaan Fable |
|---|---|---|
| `G10:BB_TWO_DRAFT_SHARED_RESERVE` (dua draf S02 berbagi stok 10: reservasi 3 dan 2, edit 4, cancel satu, POST satu) | PASS: tersedia 5→4→6→6, cancel tanpa jurnal, POST satu kali AR 40 / pendapatan 40 / COGS 24 / FG 24 | **CONFIRMED konsisten** dengan 5 kasus S02 writer + 2 race S02 yang Fable jalankan di runtime auditor (57/57 dan 14/14). Oracle GPT (reservasi dibawa) memang yang dipakai writer |
| `G10:BA_TEN_DOCUMENT_CENT_POOL` (10 dokumen @10,005 bertumpuk, 10 potong) | rev1 COUNTEREXAMPLE = kesalahan baseline auditor GPT (diakui, tidak dilabel ulang); rev2 PASS: total 100,10, stok 0, **WIP per PO 10,05 / 10,01 / 10,00 / 10,01 / …** | **CONFIRMED** dan **memperjelas T3**: pada stok bertumpuk, PO pertama menyerap sen dari **semua** dokumen yang bertumpuk sebelumnya (di sini +0,04 di atas dokumennya sendiri: 10,05 vs 10,01). Jadi batasnya bukan "1 sen per PO" melainkan "≤1 sen per dokumen yang bertumpuk, terkumpul di pemakaian berikutnya". Total dan per tanggal tetap benar (M:6632). Pertanyaan owner T3 sebaiknya dibaca dengan angka ini: opsi A = HPP satu PO boleh berbeda beberapa sen (sebanyak dokumen yang bertumpuk) dari notanya; opsi B = ubah aturan (CR penilaian per roll) |

Tidak ada REFUTED. Catatan untuk writer: perbarui kalimat pertanyaan owner di `docs/cp6-t3-cent-per-po-and-t5-advisor-note.md` agar menyebut "sampai n sen bila n dokumen bertumpuk" (contoh nyata GPT: 10,05), bukan "paling banyak 1 sen".

## 8. Keputusan owner (2026-09-25T15:55:00Z) — OWNER_CONFIRMED_TO_AUDITOR
- **D06 disahkan** atas lampiran C6 rev4 pada 4c61acad (sha256 `42e0481579497c0a…`), termasuk scope ACC-04b, LAU-05b, LAU-06b, ALL 22. `PENDING_POLICY_VALUE` tetap pending. Pengesahan ≠ penerimaan uji; CP6 HOLD, production_go=false. → O2/D06 **TERTUTUP**.
- **T3 = opsi A**: rata-rata bergerak untuk bahan sejenis yang bercampur; selisih pembulatan beberapa nota boleh terkumpul pada satu PO (contoh 10,05 vs 10,01). Tidak ada batas tetap "≤1 sen per PO". Oracle yang wajib tetap: total nilai, nilai per tanggal, stok habis = nilai 0, jejak sumber/penyesuaian. Penilaian per roll = CR terpisah. → dua COUNTEREXAMPLE beku Fable (3 penerimaan bertumpuk) tetap tercatat apa adanya; **klasifikasinya kini: bukan cacat** (sesuai keputusan owner), oracle Fable rev berikutnya tidak lagi menuntut per-PO = dokumen pada stok campuran.
- Tugas writer: perbaiki kalimat pertanyaan T3 di `docs/cp6-t3-cent-per-po-and-t5-advisor-note.md` (hapus "paling banyak 1 sen"; tulis keputusan A dan kewajiban ujinya) dan catat D06 di addendum induk bagian 9.
