...
## 13. Putaran 10 — uji sendiri atas hal yang berbeda dari GPT (2026-09-25T12:00Z, sementara)

Head writer bergerak: d1bc8ad → 72bf53f → fbafa51 → **db9e276** (BB P04/Y02). File alat auditor (`cp6_auditor_modes.py`, `cp6_auditor_runner.py`, `cp6_auditor_scenario.py`, `cp6_auditor_browser_host.mjs`, workflow) **identik** d1bc8ad..db9e276; `supabase/dev/cp6_ba_t1_family.sql` dan `supabase/migrations` **tidak berubah**. Dispatcher dipin ke db9e276.

| Hal yang tadinya hanya klaim GPT | Bukti Fable sendiri | Status |
|---|---|---|
| T3 paket rinci (advisor, restore drill, cek data) | Log job install 108029182401 (run Fable 36121924229) dibaca: 25/25 PASS, gate {installed, primary_unchanged, backup_restore_drill, security_advisors} semua true; advisor before 73 / after 129, **+56 INFO `rls_enabled_no_policy`** (semua tabel erp: 25 capsule rollback + tabel initial_import/pocket/po_hpp/laundry_rate_owner_estimates dll.), removed []; restore drill `RESTORED_SAME_MEANING` dengan **19 error pg_cron** (extension hanya di database postgres) semua dijelaskan, data 320 tabel/1.659 baris identik, engine READY=READY; cek cutover: UUID CLEAN 765 kolom, alias CASH_BANK NONE (baseline T3, bukan hosted). Job pins 108029182558: `T3_PINS_REPRODUCED equal=true differ=[]` | **CONFIRMED natively** (JSON di `audit/runs_fable/r9/t3_pins_108029182558.json`) |
| Rollback 127 kasus | Log job 108033340129 (run Fable 36123219619) dibaca: `t3_rollback` mode cycle PASS, **127/127 PASS** (50 restoration checks, 25 reinstall-equal, 25 post-use refusals + 24 refusal rows + 2 + 1) | **CONFIRMED natively** (`audit/runs_fable/r9/t3_rollback_108033340129.json`) |
| W7 status di luar kosakata → INCOMPLETE | `xaudit_10_tool.py` (Fable, sha b64e9443): cases/races/HTTP masing-masing kontrol + status aneh (`MAYBE`, `SUCCESS`, `pass`, None, tanpa kunci) + non-dict + exception — run 36132001602 pada db9e276 | menunggu (§14) |
| BB T1 (GPT 25/25 pada 72bf53f) | `round10_fable/fable_bb_independent.py` (sha 14a4b584): PLAN writer 34 kasus (F: 24 + P: P03/P04/Y02 10) **tanpa ubahan** + 5 kasus Fable (arah jurnal S01; pelunasan tepat 65 lalu 0,01 ditolak; dua pelunasan parsial lalu reverse yang **pertama**; balance batch lain ditolak; P03 tanpa kewajiban ganda) — workflow `fable-cp6-bb-t1.yml` pinned db9e276, run 36132268786 (before/after) | menunggu (§14) |
| Sen per-PO 3 penerimaan (GPT: M:485 hanya pool kain kantong, bukan izin lintas dokumen; GPT menguji varian **berurutan** stok nol di antara penerimaan) | `xaudit_8.py` rev3 (sha 97f95e6a): varian staggered (penerimaan hari d+i, potong hari yang sama) INVOICE UP dan DIRECT DOWN — run 36132361821 | menunggu (§14). Bila per-PO tetap meleset saat stok nol di antara penerimaan, klasifikasi NOTED P3 akan dinaikkan (mesin biaya mem-pool lintas dokumen meski stok kosong) |

## 14. Putaran 10 — hasil uji sendiri (2026-09-25T12:10Z)

| Item | Run / job (head db9e276, alat identik d1bc8ad) | Hasil Fable | Klasifikasi |
|---|---|---|---|
| W7 keluaran tidak sah (Fable `xaudit_10_tool.py` b64e9443) | 36132001602 / 108061174322 | cases: kontrol PASS; status `MAYBE` → INCOMPLETE (`status_outside_vocabulary=MAYBE`), list → INCOMPLETE `AUDITOR_CASE_RESULT_NOT_A_DICT`, exception → INCOMPLETE dengan pesan. races: kontrol PASS; `SUCCESS`, `None`, string → INCOMPLETE. HTTP: kontrol PASS; `pass` (huruf kecil), tanpa kunci status, `None` → INCOMPLETE. Semua grup mencetak planned/final/missing (missing []); auth restored; database_remaining 0; job merah by design | **W7 CONFIRMED penuh** (melampaui GPT: 9 bentuk keluaran tidak sah, ketiga mode) |
| BB T1 rerun (writer PLAN 34 + Fable 5) | 36132268786 / before 108062037519, after 108062037685 | **before**: 34 NO_ROUTE + 2 COUNTEREXAMPLE (`OSS_DIRECT_BEFORE_CUTOVER`, `OSS_DATED_CAPACITY` — cacat yang BB perbaiki) + 3 PASS (kontrol + `FAB:S01_OPENING_AR_DIRECTION` sudah benar di BA), REVIEW_COMPLETE, mismatch {}. **after**: writer 34/34 PASS, Fable 4/4 PASS (`S01_OPENING_AR_DIRECTION` Dr AR 70 / Dr bank 100 / Cr OPENING_EQUITY 170, hanya jurnal OPENING_BALANCE; `P02_SETTLE_EXACT_THEN_CENT` sisa 0 lalu 0,01 ditolak `BB_OSS_EXCEEDS_AVAILABLE` tanpa efek, reverse pulih 65; `S01_TWO_PARTIALS_REVERSE_FIRST` 25+45 habis, reverse yang pertama → sisa 25 & bank +45, reverse ganda ditolak, semua reverse → 70; `OSS_BALANCE_OF_OTHER_BATCH_REFUSED` ditolak tanpa efek); `FAB:P03_NO_DUPLICATE_OBLIGATION` INCOMPLETE karena **SQL auditor salah** (kolom `purchase_id` tidak ada di `material_supplier_invoices`) → rev2 run 36132808005 (§15). `bb_install` stage `AV_PLUS_AW_AX_AY_AZ_BA_BB_T1`; primary_unchanged, clone 0 | **BB T1 (writer 34 kasus) CONFIRMED natively pada db9e276** — konvergen dengan GPT (25/25 pada 72bf53f). Cakupan tetap T1: belum race dua sesi, HTTP/UI, T2/T3/rollback BB |
| Sen per-PO, varian berurutan (`xaudit_8.py` rev3 97f95e6a) | 36132361821 / 108062342986 | `THREE_STAGGERED_INVOICE_UP` **PASS** (10,01 / 10,01 / 10,01; total 30,03), `THREE_STAGGERED_DIRECT_DOWN` **PASS** (10,00 ×3; total 30,00); varian bertumpuk tetap COUNTEREXAMPLE beku (10,02/10,00/10,01 dan 9,99/10,01/10,00); LAU-T14 4/4 dan 2-penerimaan 4/4 tetap PASS | Konvergen dengan GPT (`out/gpt_r9_cent_origin_result.md`, 3/3 PASS): selisih 1 sen per PO **hanya** saat beberapa penerimaan bertumpuk di stok sebelum dipakai (rata-rata bergerak), tidak saat stok nol di antara penerimaan. Status: **OPEN P3, bukan blocker** — kontrak menjamin total & per tanggal (M:6632); per-PO lintas dokumen pada stok campuran tidak diatur eksplisit (M:485 = pool kain kantong). Diteruskan ke writer/owner sebagai pertanyaan kecil, hasil beku tidak dilabel ulang |

## 15. BB rev2 (run 36132808005, before 108063760177 success, after 108063760411) — 2026-09-25T12:25Z

- after: writer **34/34 PASS**, Fable **4/4 PASS**, `FAB:P03_NO_DUPLICATE_OBLIGATION` **FAIL** hanya pada check `only_opening_journal`: jenis jurnal yang tercipta = {`OPENING_BALANCE`, `OPENING_UNINVOICED_RECEIPT`}. Enam check substantif **true**: inventory +22,00; GRNI −12,00 (hanya 6 unit belum ditagih); AP −7,00 (hanya bagian nota yang masih terbuka); OPENING_EQUITY −103,00 (= 22 + 100 kas − 12 − 7); AP native 0; tidak ada invoice/pembayaran historis yang dikarang; state item qty 10 / posted 4 / PARTIAL / biaya 2,200000.
- Penilaian Fable: **cek auditor terlalu ketat**. Kontrak M:831 mewajibkan jurnal kewajiban awal Dr OPENING_EQUITY / Cr GRNI_MATERIAL pada cutover dan melarang jurnal MATERIAL_PURCHASE/reclass AP/invoice/pembayaran historis — `OPENING_UNINVOICED_RECEIPT` adalah jurnal kewajiban awal itu sendiri (jalur AO/AP), bukan jurnal pembelian. Bukan cacat produk. Hasil beku FAIL tidak dilabel ulang; oracle rev3 akan menerima kedua jenis jurnal pembukaan tersebut. JSON: `audit/runs_fable/r10/auditor_r10_bb_after_rev2.json`.
- Kesimpulan BB T1 pada db9e276: **writer 34/34 CONFIRMED natively; Fable 4/4 PASS + P03 substantif benar**. Belum: race dua sesi pada satu balance (butuh mode races dengan BB terpasang), HTTP/UI, T2/T3/rollback BB, family BC–BE.
