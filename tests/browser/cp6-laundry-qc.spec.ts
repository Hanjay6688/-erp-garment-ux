...
test('CP6 blocks the legacy browser Nota writer in connected UAT', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText(/Formula Nota FG lama belum menjadi kontrak backend/)).toBeVisible()
  await expect(page.getByText('Tidak ada data yang dihapus atau diposting.', { exact: true })).toBeVisible()
