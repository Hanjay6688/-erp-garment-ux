...
test('CP5 local mocked-UAT contract keeps server-side Pattern truth and one mutation', async ({ page }, testInfo) => {
  ...
  const patternFilter = page.getByLabel('FILTER POLA CP5')
  await expect(patternFilter).toBeVisible()
  await expect(patternFilter.locator('option[value="pattern-1"]')).toHaveText('REG · R2 · Regular')

  await patternFilter.selectOption('pattern-2')
  ...
  await patternFilter.selectOption('pattern-1')
