-- CP6 Z: accounting-close dates use the canonical Jakarta business day.
-- Native counterexample #176, reproduced #177; both incomplete harness runs retained.
-- One function changes; admitted migrations and rollbacks through Y remain immutable.
begin;
...
do $lock_all_erp_v2620z$
declare v_table text;
begin
  for v_table in select c.relname from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='erp' and c.relkind in('r','p') and c.relname not in('schema_migrations','cp6_v2620z_rollback_capsule') order by c.relname loop
...
do $predecessor_v2620z$
declare r record;c record;
begin
  if not exists(select 1 from erp.schema_migrations where version='v2.6.20y')
     or exists(select 1 from erp.schema_migrations where version='v2.6.20z')
     or to_regclass('erp.cp6_v2620z_rollback_capsule') is not null then
    raise exception 'Z_REQUIRES_EXACT_Y_WITHOUT_Z_RESIDUE';
  end if;
  if (select count(*) from supabase_migrations.schema_migrations where name='erp_v2_6_20y_cp6_cash_business_dates')<>1
     or not exists(select 1 from supabase_migrations.schema_migrations
       where version='20260914043146' and name='erp_v2_6_20y_cp6_cash_business_dates'
     ...
     or exists(select 1 from supabase_migrations.schema_migrations where version>'20260914043146')
     or (select count(*) from erp.cp6_v2620y_rollback_capsule)<>6 then
    raise exception 'Z_PREDECESSOR_PLATFORM_OR_CAPSULE_MISMATCH';
  end if;
  ...
  loop
    ...
    if c.object_regidentity is null
      ...
      raise exception 'Z_TRUSTED_Y_PREDECESSOR_PIN_MISMATCH: %',r.identity;
    end if;
  end loop;
  ...
  for r in select * from(values
    ...
    if encode(extensions.digest(convert_to(pg_get_functiondef(
      ...
      raise exception 'Z_TRUSTED_U_INPUT_OWNER_ACL_MISMATCH: %',r.identity;
    end if;
  end loop;
  if not exists(select 1 from pg_class
    ...
    raise exception 'Z_INHERITED_T_SECURITY_MISMATCH';
  end if;

  if encode(extensions.digest(convert_to(pg_get_functiondef('erp.close_accounting_through(date,text)'::regprocedure),'UTF8'),'sha256'),'hex') is distinct from 'c9517690638ac40bb8914e0613b7d5bdf92256274e9c5551747060d09a79536b'
     or (select pg_get_userbyid(proowner) from pg_proc where oid='erp.close_accounting_through(date,text)'::regprocedure) is distinct from 'postgres'
     or (select array(select a::text from unnest(proacl) a order by a::text)
       from pg_proc where oid='erp.close_accounting_through(date,text)'::regprocedure) is distinct from array['authenticated=X/postgres','postgres=X/postgres']::text[] then
    raise exception 'Z_PREDECESSOR_FUNCTION_OWNER_ACL_MISMATCH';
  end if;
  if exists(select 1 from erp.accounting_period_control where closed_through>=erp._cp3_business_date(current_timestamp)) then
    raise exception 'Z_PREEXISTING_PERIOD_CLOSE_REVIEW_REQUIRED';
  end if;
end
$predecessor_v2620z$;

create table erp.cp6_v2620z_rollback_capsule(
  like erp.cp6_v2620y_rollback_capsule including all
);
alter table erp.cp6_v2620z_rollback_capsule enable row level security;
revoke all on erp.cp6_v2620z_rollback_capsule from public,anon,authenticated,service_role;
insert into erp.cp6_v2620z_rollback_capsule(
...
where p.oid='erp.close_accounting_through(date,text)'::regprocedure;


do $canonical_close_v2620z$
declare d text;anchor text:='p_closed_through>=current_date';
begin
  select pg_get_functiondef('erp.close_accounting_through(date,text)'::regprocedure) into d;
  if (length(d)-length(replace(d,anchor,'')))/length(anchor)<>1 then
    raise exception 'Z_CANONICAL_CLOSE_ANCHOR';
  end if;
  execute replace(d,anchor,'p_closed_through>=erp._cp3_business_date(current_timestamp)');
end
$canonical_close_v2620z$;

do $installed_v2620z$
declare r record;c record;
begin
  if (select count(*) from erp.cp6_v2620z_rollback_capsule)<>1 then
    raise exception 'Z_CAPSULE_CARDINALITY_MISMATCH';
  end if;
  update erp.cp6_v2620z_rollback_capsule cap set installed_definition_sha256=
  ...
  loop
    select * into c from erp.cp6_v2620z_rollback_capsule
    ...
    if c.object_regidentity is null
       ...
       or (select array(select a::text from unnest(proacl) a order by a::text)
            from pg_proc where oid=to_regprocedure(r.identity)) is distinct from r.acl then
      raise exception 'Z_INSTALLED_FUNCTION_OWNER_ACL_MISMATCH: %',r.identity;
    end if;
  end loop;
...
begin
  ...
  if (select count(*) from jsonb_object_keys(v_snapshot))<>211 then
    raise exception 'Z_FULL_ERP_BOUNDARY_CARDINALITY expected211 actual%',
      (select count(*) from jsonb_object_keys(v_snapshot));
  end if;
  update erp.cp6_v2620z_rollback_capsule set boundary_snapshot=v_snapshot;
end
$boundary_v2620z$;

insert into erp.schema_migrations(version,description)
values('v2.6.20z','Canonical Jakarta accounting-close day; authorization, reason, reopen, checkpoints and audit history preserved');
commit;
