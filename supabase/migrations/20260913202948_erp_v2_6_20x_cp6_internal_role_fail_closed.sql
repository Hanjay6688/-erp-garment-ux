-- CP6 X: fail closed when the mapped internal ERP role is NULL.
-- Forward-only successor; all published migrations and rollbacks through W stay immutable.
begin;
...
do $predecessor_v2620x$
declare r record;c record;
begin
  if not exists(select 1 from erp.schema_migrations where version='v2.6.20w')
     or exists(select 1 from erp.schema_migrations where version='v2.6.20x')
     or to_regclass('erp.cp6_v2620x_rollback_capsule') is not null then
    raise exception 'X_REQUIRES_EXACT_W_WITHOUT_X_RESIDUE';
  end if;
  if (select count(*) from supabase_migrations.schema_migrations
    ...
    raise exception 'X_PREDECESSOR_PLATFORM_OR_CAPSULE_MISMATCH';
  end if;
  ...
  loop
    ...
    if c.object_regidentity is null
      ...
      raise exception 'X_TRUSTED_U_FUNCTION_OWNER_ACL_MISMATCH: %',r.identity;
    end if;
  end loop;
  ...
  loop
    ...
    if c.object_regidentity is null
       ...
       or (select array(select a::text from unnest(proacl) a order by a::text)
            from pg_proc where oid=to_regprocedure(r.identity)) is distinct from r.acl then
      raise exception 'X_TRUSTED_V_FUNCTION_OWNER_ACL_MISMATCH: %',r.identity;
    end if;
  end loop;
  ...
  loop
    ...
    if c.object_regidentity is null
       ...
       or (select array(select a::text from unnest(proacl) a order by a::text)
            from pg_proc where oid=to_regprocedure(r.identity)) is distinct from r.acl then
      raise exception 'X_INSTALLED_FUNCTION_OWNER_ACL_MISMATCH: %',r.identity;
    end if;
  end loop;
  ...
  for r in select * from(values
    ...
    if encode(extensions.digest(convert_to(pg_get_functiondef(
       ...
       or (select array(select a::text from unnest(proacl) a order by a::text)
         from pg_proc where oid=to_regprocedure(r.identity)) is distinct from r.acl then
      raise exception 'X_TRUSTED_U_INPUT_OWNER_ACL_MISMATCH: %',r.identity;
    end if;
  end loop;
  if not exists(select 1 from pg_class
    ...
    raise exception 'X_INHERITED_T_SECURITY_MISMATCH';
  end if;
  if exists(
    ...
    raise exception 'X_PREEXISTING_CANONICAL_DATE_REVIEW_REQUIRED';
  end if;
end
$predecessor_v2620x$;

create table erp.cp6_v2620x_rollback_capsule(
  like erp.cp6_v2620w_rollback_capsule including all
);
alter table erp.cp6_v2620x_rollback_capsule enable row level security;
revoke all on erp.cp6_v2620x_rollback_capsule from public,anon,authenticated,service_role;
insert into erp.cp6_v2620x_rollback_capsule(
...
do $role_fail_closed_v2620x$
declare d text;anchor text;replacement text;
begin
  select pg_get_functiondef('erp.require_internal()'::regprocedure) into d;
  anchor:=$a$if v_app_role not in('OWNER','ADMIN','STAFF') then$a$;
  replacement:=$r$if coalesce(v_app_role,'') not in('OWNER','ADMIN','STAFF') then$r$;
  if (length(d)-length(replace(d,anchor,'')))/length(anchor)<>1 then
    raise exception 'X_INTERNAL_ROLE_GUARD_ANCHOR';
  end if;
  execute replace(d,anchor,replacement);
end
$role_fail_closed_v2620x$;

do $installed_v2620x$
declare r record;c record;
begin
  if (select count(*) from erp.cp6_v2620x_rollback_capsule)<>1 then
    raise exception 'X_CAPSULE_CARDINALITY_MISMATCH';
  end if;
  update erp.cp6_v2620x_rollback_capsule cap set installed_definition_sha256=
  ...
  if (select count(*) from erp.run_v268_financial_report_checks()
    ...
    raise exception 'X_FINANCIAL_REPORT_SCOPE_NOT_CONNECTED';
  end if;
end
$installed_v2620x$;
...
values('v2.6.20x','Fail-closed mapped internal role while preserving authorized internal and permission-scoped paths');
commit;
