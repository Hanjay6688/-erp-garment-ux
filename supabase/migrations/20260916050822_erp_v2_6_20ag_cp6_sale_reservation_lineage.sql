-- CP6 AG: preserve sale draft/reservation lineage across every normal entry point.
...
begin
  for v_table in select c.relname from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='erp' and c.relkind in('r','p') and c.relname not in('schema_migrations','cp6_v2620ag_rollback_capsule') order by c.relname loop
...
begin
  ...
  if (select count(*) from supabase_migrations.schema_migrations where name='erp_v2_6_20af_cp6_posted_child_integrity')<>1
     or not exists(select 1 from supabase_migrations.schema_migrations where version='20260916014332' and name='erp_v2_6_20af_cp6_posted_child_integrity'
       and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')='54c5f73b99a2e63c858a777f669432f28260177daf4c45d806bb0e8a7c7c8e8e')
     or exists(select 1 from supabase_migrations.schema_migrations where version>'20260916014332')
...
create table erp.cp6_v2620ag_rollback_capsule(
  like erp.cp6_v2620af_rollback_capsule including all
);
alter table erp.cp6_v2620ag_rollback_capsule enable row level security;
revoke all on erp.cp6_v2620ag_rollback_capsule from public,anon,authenticated,service_role;
insert into erp.cp6_v2620ag_rollback_capsule(
...
where p.oid in('erp.guard_child_by_parent_status()'::regprocedure,'erp.guard_posted_document()'::regprocedure,'erp.post_sale(uuid)'::regprocedure,'erp.run_v268_financial_report_checks()'::regprocedure);


do $canonical_opening_v2620ag$
begin
...
$function$
$definition$;
  execute $definition$CREATE OR REPLACE FUNCTION erp.guard_posted_document()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path TO 'erp', 'public', 'pg_temp'
AS $function$
BEGIN
  ...
  IF TG_OP='INSERT' THEN
    IF NEW.status IS DISTINCT FROM 'DRAFT' THEN
      RAISE EXCEPTION 'Document must be created as DRAFT; use the ERP posting RPC to post it';
    END IF;
    RETURN NEW;
  END IF;
  ...
  IF TG_OP='DELETE' THEN
    IF OLD.status<>'DRAFT' THEN
      RAISE EXCEPTION 'Posted/non-draft document cannot be deleted directly; use reversal/correction';
    END IF;
    RETURN OLD;
  END IF;

  IF OLD.status<>'DRAFT' THEN
    RAISE EXCEPTION 'Posted/non-draft document cannot be edited directly; use reversal/correction';
  END IF;
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    RAISE EXCEPTION 'Document status cannot leave DRAFT by direct table update; use the ERP posting/cancellation RPC';
  END IF;
  RETURN NEW;
END;
$function$
$definition$;
  execute $definition$CREATE OR REPLACE FUNCTION erp.post_sale(p_sale_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'erp', 'public', 'pg_temp'
...
do $installed_v2620ag$
declare r record;c record;
begin
  if (select count(*) from erp.cp6_v2620ag_rollback_capsule)<>4 then
    raise exception 'AG_CAPSULE_CARDINALITY_MISMATCH';
  end if;
  update erp.cp6_v2620ag_rollback_capsule cap set installed_definition_sha256=
  ...
  loop
    ...
    if c.object_regidentity is null
      ...
      raise exception 'AG_INSTALLED_FUNCTION_OWNER_ACL_MISMATCH: %',r.identity;
    end if;
  end loop;
...
begin
  ...
  if (select count(*) from jsonb_object_keys(v_snapshot))<>219 then
    raise exception 'AG_FULL_ERP_BOUNDARY_CARDINALITY expected219 actual%',
      (select count(*) from jsonb_object_keys(v_snapshot));
  end if;
  update erp.cp6_v2620ag_rollback_capsule set boundary_snapshot=v_snapshot;
end
$boundary_v2620ag$;

insert into erp.schema_migrations(version,description)
values('v2.6.20ag','Reserved sale edits use the atomic draft RPC; posting and reports require exact source/stock lineage');
commit;
