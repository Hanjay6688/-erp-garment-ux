-- CP6 Q: exact supplier-invoice capacity and derived completion states.
-- Six-decimal numeric quantities conserve exactly; one micro-unit can carry
-- a material money amount. Published predecessor SQL remains immutable.
begin;
...
do $predecessor_v2620q$
declare r record;c record;v_actual text;
begin
  if not exists(select 1 from erp.schema_migrations where version='v2.6.20p')
     or exists(select 1 from erp.schema_migrations where version='v2.6.20q')
     or to_regclass('erp.cp6_v2620q_rollback_capsule') is not null then
    raise exception 'Q_REQUIRES_EXACT_P_WITHOUT_Q_RESIDUE';
  end if;
  if (select count(*) from supabase_migrations.schema_migrations
     ...
     or not exists(select 1 from supabase_migrations.schema_migrations
       where version='20260912051922'
         ...
         and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')
           in('109a991d12643ff1c056fd984cea3b3a26bc9fe9d9d716f62c84c4bcb053c138',
    ...
    raise exception 'Q_PREDECESSOR_PLATFORM_OR_CAPSULE_MISMATCH';
  end if;
  ...
  loop
    ...
    if c.object_regidentity is null
      ...
      raise exception 'Q_TRUSTED_P_FUNCTION_OWNER_ACL_MISMATCH: %',r.identity;
    end if;
  end loop;
  -- These Q inputs are outside P's replaced identities. Pin their exact
  -- installed definitions and effective privileges before taking snapshots.
  for r in select * from(values
    ...
    if encode(extensions.digest(convert_to(pg_get_functiondef(
       ...
       or (select array(select a::text from unnest(proacl) a order by a::text)
         from pg_proc where oid=to_regprocedure(r.identity)) is distinct from r.acl then
      raise exception 'Q_TRUSTED_P_INPUT_OWNER_ACL_MISMATCH: %',r.identity;
    end if;
  end loop;
  if not exists(select 1 from pg_class
    ...
    raise exception 'Q_INHERITED_P_SECURITY_MISMATCH';
  end if;
  if exists(
    ...
    where x.matched>x.capacity or (i.invoice_match_state<>'DIRECT_FINAL' and(
    ...
    raise exception 'Q_PREEXISTING_INVOICE_QUANTITY_REVIEW_REQUIRED';
  end if;
end
$predecessor_v2620q$;

create table erp.cp6_v2620q_rollback_capsule(
  like erp.cp6_v2620p_rollback_capsule including all
);
alter table erp.cp6_v2620q_rollback_capsule enable row level security;
revoke all on erp.cp6_v2620q_rollback_capsule from public,anon,authenticated,service_role;
insert into erp.cp6_v2620q_rollback_capsule(
...
where p.oid in(
  ...
  'erp.refresh_material_purchase_item_match_state(uuid)'::regprocedure,
...
begin
  ...
  anchor:=$a$'V267_PAYMENT_EXCEEDS_FINAL_AP')$a$;
  replacement:=$r$'V267_PAYMENT_EXCEEDS_FINAL_AP','V267_INVOICE_MATCH_OVER_RECEIPT')$r$;
  if (length(d)-length(replace(d,anchor,'')))/length(anchor)<>1 then
    raise exception 'Q_EXACT_QUANTITY_ANCHOR_1_MISMATCH';
  end if;
...
declare d text;anchor text;replacement text;
begin
  select pg_get_functiondef('erp.post_material_supplier_invoice(uuid)'::regprocedure) into d;
  anchor:=$a$v_matched+r.qty_invoiced>v_capacity+0.000001$a$;
  replacement:=$r$v_matched+r.qty_invoiced>v_capacity$r$;
  if (length(d)-length(replace(d,anchor,'')))/length(anchor)<>1 then
    raise exception 'Q_EXACT_QUANTITY_ANCHOR_2_MISMATCH';
  end if;
  execute replace(d,anchor,replacement);
end
$patch_v2620q_2$;
...
declare d text;anchor text;replacement text;
begin
  select pg_get_functiondef('erp.refresh_material_purchase_item_match_state(uuid)'::regprocedure) into d;
  anchor:=$a$v_matched<v_capacity-0.000001$a$;
  replacement:=$r$v_matched<v_capacity$r$;
  if (length(d)-length(replace(d,anchor,'')))/length(anchor)<>1 then
    raise exception 'Q_EXACT_QUANTITY_ANCHOR_3_MISMATCH';
  end if;
  execute replace(d,anchor,replacement);
end
$patch_v2620q_3$;
...
begin
  ...
  anchor:=$a$erp.material_purchase_invoice_capacity(i.id)+0.000001$a$;
  replacement:=$r$erp.material_purchase_invoice_capacity(i.id)$r$;
  if (length(d)-length(replace(d,anchor,'')))/length(anchor)<>1 then
    raise exception 'Q_EXACT_QUANTITY_ANCHOR_4_MISMATCH';
  end if;
  execute replace(d,anchor,replacement);
end
$patch_v2620q_4$;
...
begin
  select pg_get_functiondef('erp.run_v267_financial_truth_checks()'::regprocedure) into d;
  anchor:=$a$x.matched<x.capacity-0.000001$a$;
  replacement:=$r$x.matched<x.capacity$r$;
  if (length(d)-length(replace(d,anchor,'')))/length(anchor)<>2 then
    raise exception 'Q_EXACT_QUANTITY_ANCHOR_5_MISMATCH';
  end if;
  execute replace(d,anchor,replacement);
end
$patch_v2620q_5$;
...
begin
  if (select count(*) from erp.cp6_v2620q_rollback_capsule)<>4 then
    raise exception 'Q_CAPSULE_CARDINALITY_MISMATCH';
  end if;
  update erp.cp6_v2620q_rollback_capsule cap set installed_definition_sha256=
  ...
  loop
    ...
    if c.object_regidentity is null or c.definition_sha256 is distinct from r.predecessor_sha256
       ...
       or (select array(select a::text from unnest(proacl) a order by a::text)
            from pg_proc where oid=to_regprocedure(r.identity)) is distinct from r.acl then
      raise exception 'Q_INSTALLED_FUNCTION_OWNER_ACL_MISMATCH: %',r.identity;
    end if;
  end loop;
  ...
  if v_count is distinct from 0 then
    raise exception 'Q_INSTALLED_RETURN_MATCH_STATE_RECONCILIATION_FAILED: %',v_count;
  end if;
  if not exists(select 1 from erp.run_v268_financial_report_checks()
    ...
    raise exception 'Q_FINANCIAL_REPORT_SCOPE_NOT_CONNECTED';
  end if;
end
$installed_v2620q$;
...
begin
  ...
  update erp.cp6_v2620q_rollback_capsule set boundary_snapshot=v_snapshot;
end
$boundary_v2620q$;

insert into erp.schema_migrations(version,description)
values('v2.6.20q','Enforce exact supplier invoice capacity and completion state with connected financial checks');
commit;
