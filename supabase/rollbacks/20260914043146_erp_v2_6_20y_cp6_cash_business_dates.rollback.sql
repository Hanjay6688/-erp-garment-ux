-- REVIEWED PRE-USE ROLLBACK: CP6 v2.6.20y -> exact X.
...
do $platform_guard_v2620y$
begin
  if (select count(*) from supabase_migrations.schema_migrations
      where name='erp_v2_6_20y_cp6_cash_business_dates')<>1
...
lock table erp.cp6_v2620y_rollback_capsule in access exclusive mode;

do $lock_all_erp_v2620y$
declare v_table text;
begin
  for v_table in select relation.relname from pg_class relation join pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='erp' and relation.relkind in('r','p') and relation.relname not in('schema_migrations','cp6_v2620y_rollback_capsule') order by relation.relname loop
    execute format('lock table erp.%I in share row exclusive mode',v_table);
  end loop;
end
$lock_all_erp_v2620y$;

do $restore_guard_v2620y$
...
begin
  if not exists(select 1 from erp.schema_migrations where version='v2.6.20x')
     or not exists(select 1 from erp.schema_migrations where version='v2.6.20y')
     or exists(select 1 from erp.schema_migrations
       where version not in('v2.6.20x','v2.6.20y')
    ...
    raise exception 'Y_ROLLBACK_MARKER_CAPSULE_OR_SUCCESSOR';
  end if;
  ...
  loop
    ...
    if c.object_regidentity is null
      ...
      raise exception 'Y_TRUSTED_PREDECESSOR_PIN_MISMATCH: %',r.identity;
    end if;
  end loop;

  select boundary_snapshot into v_expected
  from erp.cp6_v2620y_rollback_capsule limit 1;
  if (select count(*) from (select relation.relname from pg_class relation join pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='erp' and relation.relkind in('r','p') and relation.relname not in('schema_migrations','cp6_v2620y_rollback_capsule') order by relation.relname) all_erp_tables)<>210
     or v_expected is null
     or (select count(*) from jsonb_object_keys(v_expected))<>210
     or exists(select 1 from erp.cp6_v2620y_rollback_capsule
       where boundary_snapshot is distinct from v_expected) then
    raise exception 'Y_BOUNDARY_SNAPSHOT_MISMATCH';
  end if;
  for v_table in select relation.relname from pg_class relation join pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='erp' and relation.relkind in('r','p') and relation.relname not in('schema_migrations','cp6_v2620y_rollback_capsule') order by relation.relname loop
    ...
    if v_hash is distinct from v_expected->>v_table then
      raise exception 'Y_POST_USE_ROLLBACK_REFUSED: %',v_table;
    end if;
  end loop;

  for c in select * from erp.cp6_v2620y_rollback_capsule
    order by object_regidentity
  loop
    execute c.object_definition;
  end loop;
  for r in select * from(values
    ('erp.post_opening_subledger_settlement(uuid)','c56b387a8593b19cd3b92e6dc20ac95459d2aedeb2b616b6fb1e4d812e729393'),
    ('erp.post_vendor_payment(uuid)','05b067af75c8691c4e012ff719f8793dbe597ee5f0e451db665d4baab1427316'),
    ('erp.post_sales_payment(uuid)','362e4266718275af5af6efcded3c85cd7a7a7f1faba7241979114fe6e57ffd6c'),
    ('erp.run_v267_financial_truth_checks()','845b9868bb0b0e25443c4e75c77cfcf815e34c4fac78b8bd38038b0476e2944a'),
    ('erp._v268_financial_report_checks_pre_scope()','c77df75873558f96425b8e6b9591903259e5d985a9bd534ab96466799610e507'),
    ('erp.run_v268_financial_report_checks()','48d60613970b015d1afb30a2f8a7cec0e5d56d1f692c6a1acfa0ffd48684fda1')
  ...
  loop
    ...
    if v_actual is distinct from r.sha256 then
      raise exception 'Y_ROLLBACK_RESTORE_HASH_MISMATCH: %',r.identity;
    end if;
  end loop;
end
$restore_guard_v2620y$;

drop table erp.cp6_v2620y_rollback_capsule;
delete from erp.schema_migrations where version='v2.6.20y';
...
begin
  ...
  loop
    ...
    if v_actual is distinct from r.sha256 then
      raise exception 'Y_ROLLBACK_POSTCONDITION_FUNCTION_MISMATCH: %',r.identity;
    end if;
  end loop;
  if exists(select 1 from erp.schema_migrations where version='v2.6.20y')
     or exists(select 1 from supabase_migrations.schema_migrations
       where name='erp_v2_6_20y_cp6_cash_business_dates')
     or to_regclass('erp.cp6_v2620y_rollback_capsule') is not null
     ...
     or to_regclass('erp.cp6_v2620x_rollback_capsule') is null
    ...
    raise exception 'Y_ROLLBACK_POSTCONDITION_FAILED';
  end if;
end
$postcheck_v2620y$;
commit;
