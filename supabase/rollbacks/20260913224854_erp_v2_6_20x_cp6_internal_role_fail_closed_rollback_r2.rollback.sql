-- VERSIONED R2 SUCCESSOR FOR THE FROZEN X PRE-USE ROLLBACK.
-- Original 20260913202948 rollback stays immutable for audit.
-- Original SHA256: 8107dc0d884ee624328b2a0a4c7fd03bed4e8d88f428282a7c00660c43b4200a
-- Native169: record c shadows pg_class alias c (SQLSTATE42703).
-- Only the three catalog-selector aliases are qualified below; all predicates,
-- source pins, locks, restoration statements and transaction boundaries remain.
...
begin
  for v_table in select relation.relname from pg_class relation join pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='erp' and relation.relkind in('r','p') and relation.relname not in('schema_migrations','cp6_v2620x_rollback_capsule') order by relation.relname loop
...
begin
  ...
  if (select count(*) from (select relation.relname from pg_class relation join pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='erp' and relation.relkind in('r','p') and relation.relname not in('schema_migrations','cp6_v2620x_rollback_capsule') order by relation.relname) all_erp_tables)<>209
