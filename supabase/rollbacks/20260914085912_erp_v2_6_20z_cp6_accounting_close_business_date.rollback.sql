-- REVIEWED PRE-USE ROLLBACK: CP6 v2.6.20z -> exact Y.
...
do $platform_guard_v2620z$
begin
  if (select count(*) from supabase_migrations.schema_migrations
      where name='erp_v2_6_20z_cp6_accounting_close_business_date')<>1
     or not exists(select 1 from supabase_migrations.schema_migrations
       where version='20260914085912'
         and name='erp_v2_6_20z_cp6_accounting_close_business_date'
     ...
     or exists(select 1 from supabase_migrations.schema_migrations
       where version>'20260914085912') then
    raise exception 'Z_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR';
  end if;
end
$platform_guard_v2620z$;
...
lock table erp.cp6_v2620z_rollback_capsule in access exclusive mode;

do $lock_all_erp_v2620z$
declare v_table text;
begin
  for v_table in select relation.relname from pg_class relation join pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='erp' and relation.relkind in('r','p') and relation.relname not in('schema_migrations','cp6_v2620z_rollback_capsule') order by relation.relname loop
    execute format('lock table erp.%I in share row exclusive mode',v_table);
  end loop;
end
$lock_all_erp_v2620z$;

do $restore_guard_v2620z$
...
begin
  if not exists(select 1 from erp.schema_migrations where version='v2.6.20y')
     ...
     or exists(select 1 from erp.schema_migrations
       where version not in('v2.6.20y','v2.6.20z')
         and installed_at>(select installed_at from erp.schema_migrations
           where version='v2.6.20z'))
     or (select count(*) from erp.cp6_v2620z_rollback_capsule)<>1 then
    raise exception 'Z_ROLLBACK_MARKER_CAPSULE_OR_SUCCESSOR';
  end if;
  ...
  loop
    ...
    if c.object_regidentity is null
      ...
      raise exception 'Z_TRUSTED_PREDECESSOR_PIN_MISMATCH: %',r.identity;
    end if;
  end loop;

  select boundary_snapshot into v_expected
  from erp.cp6_v2620z_rollback_capsule limit 1;
  if (select count(*) from (select relation.relname from pg_class relation join pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='erp' and relation.relkind in('r','p') and relation.relname not in('schema_migrations','cp6_v2620z_rollback_capsule') order by relation.relname) all_erp_tables)<>211
     or v_expected is null
     or (select count(*) from jsonb_object_keys(v_expected))<>211
     or exists(select 1 from erp.cp6_v2620z_rollback_capsule
       where boundary_snapshot is distinct from v_expected) then
    raise exception 'Z_BOUNDARY_SNAPSHOT_MISMATCH';
  end if;
  for v_table in select relation.relname from pg_class relation join pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='erp' and relation.relkind in('r','p') and relation.relname not in('schema_migrations','cp6_v2620z_rollback_capsule') order by relation.relname loop
    ...
    if v_hash is distinct from v_expected->>v_table then
      raise exception 'Z_POST_USE_ROLLBACK_REFUSED: %',v_table;
    end if;
  end loop;

  for c in select * from erp.cp6_v2620z_rollback_capsule
    order by object_regidentity
  loop
    execute c.object_definition;
  end loop;
  for r in select * from(values
    ('erp.close_accounting_through(date,text)','c9517690638ac40bb8914e0613b7d5bdf92256274e9c5551747060d09a79536b')
  ...
  loop
    ...
    if v_actual is distinct from r.sha256 then
      raise exception 'Z_ROLLBACK_RESTORE_HASH_MISMATCH: %',r.identity;
    end if;
  end loop;
end
$restore_guard_v2620z$;

drop table erp.cp6_v2620z_rollback_capsule;
delete from erp.schema_migrations where version='v2.6.20z';
...
begin
  for r in select * from(values
    ('erp.close_accounting_through(date,text)','c9517690638ac40bb8914e0613b7d5bdf92256274e9c5551747060d09a79536b',array['authenticated=X/postgres','postgres=X/postgres']::text[])
  ...
  loop
    ...
    if v_actual is distinct from r.sha256 then
      raise exception 'Z_ROLLBACK_POSTCONDITION_FUNCTION_MISMATCH: %',r.identity;
    end if;
  end loop;
  if exists(select 1 from erp.schema_migrations where version='v2.6.20z')
     or exists(select 1 from supabase_migrations.schema_migrations
    ...
    raise exception 'Z_ROLLBACK_POSTCONDITION_FAILED';
  end if;
end
$postcheck_v2620z$;
commit;
