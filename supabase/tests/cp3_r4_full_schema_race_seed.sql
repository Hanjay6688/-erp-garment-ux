-- Committed CP3 R4 fixture for real two-connection races. Disposable local DB only.
...
begin
  if not exists(select 1 from erp.schema_migrations where version='v2.6.14d') then
    raise exception 'CP3 R4 race seed requires applied v2.6.14d';
  end if;
end
$cp3$;

-- Two additional authoritative January work completions are posted without terminal sewing.
...
values
 ('a5000000-0000-0000-0000-000000000011','CP3R4-WC-LATE-1','a3000000-0000-0000-0000-000000000001','a1000000-0000-0000-0000-000000000001','a3200000-0000-0000-0000-000000000001','2026-01-01 14:00:00+07','DRAFT','CP3 R4 race late 1'),
 ('a5000000-0000-0000-0000-000000000012','CP3R4-WC-LATE-2','a3000000-0000-0000-0000-000000000001','a1000000-0000-0000-0000-000000000001','a3200000-0000-0000-0000-000000000001','2026-01-01 15:00:00+07','DRAFT','CP3 R4 race late 2');
