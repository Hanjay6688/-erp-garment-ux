-- CP3 R4 bounded validator benchmark on the restored ERP Enteng schema after v2.6.14d.
...
values('b1000000-0000-0000-0000-000000000001','CP3R4-BENCH','CP3 R4 benchmark Mandor','MANDOR',true);
select erp.set_contractor_hpp_policy_v1(jsonb_build_object(
 ...
 'is_special',false,'attendance_required',true,'reason','CP3 R4 benchmark'),gen_random_uuid(),null);

insert into erp.product_models(id,model_code,model_name)
values('b2000000-0000-0000-0000-000000000001','CP3R4-BENCH-M','CP3 R4 benchmark model');
insert into erp.production_orders(id,po_number,model_id,contractor_id,target_qty_pcs,status,current_stage)
values('b3000000-0000-0000-0000-000000000001','CP3R4-BENCH-PO','b2000000-0000-0000-0000-000000000001','b1000000-0000-0000-0000-000000000001',2000,'SEWING','SEWING');
insert into erp.cutting_groups(id,po_id,group_number,cut_at,picked_up_at,status)
values('b4000000-0000-0000-0000-000000000001','b3000000-0000-0000-0000-000000000001','CP3R4-BENCH-G','2026-03-01 08:00+07','2026-03-01 09:00+07','SEWING');

insert into erp.work_completion_events(id,completion_number,po_id,contractor_id,cutting_group_id,physical_at,status)
select ('b5'||lpad(g::text,30,'0'))::uuid,'CP3R4-BENCH-WC-'||g,
...
select ('b6'||lpad(g::text,30,'0'))::uuid,'CP3R4-BENCH-SEW-'||g,'SELESAI_DIJAHIT',
 ...
 '2026-03-10 00:00:00+07'::timestamptz+g*interval '1 microsecond',1,'CP3 R4 benchmark'
from generate_series(1,2000) g;
...
create temp table cp3_r4_benchmark_worker as
select erp.save_worker_roster_v1(jsonb_build_object(
 'contractor_id','b1000000-0000-0000-0000-000000000001',
 'worker_code','CP3R4-BENCH-W','worker_name','Benchmark worker',
 'job_description','Jahit','pay_scheme','DAILY',
 'initial_daily_rate',100,'rate_effective_from','2026-03-01',
 'joined_at','2026-03-01','is_active',true,'reason','CP3 R4 benchmark'
),gen_random_uuid(),null) result;

create temp table cp3_r4_benchmark_attendance as
select erp.save_attendance_period_v1(jsonb_build_object(
 'contractor_id','b1000000-0000-0000-0000-000000000001',
 'period_number','CP3R4-BENCH-ATT','period_start','2026-03-10','period_end','2026-03-10',
 'pay_date','2026-03-15','reason','CP3 R4 benchmark',
 'attendance',jsonb_build_array(jsonb_build_object(
   'worker_id',(select result->>'worker_id' from cp3_r4_benchmark_worker),
   'attendance_date','2026-03-10','status','PRESENT'
 ))
),gen_random_uuid(),null,false) result;

select erp.post_attendance_period_v1(
 (select (result->>'period_id')::uuid from cp3_r4_benchmark_attendance),
 'CP3 R4 benchmark post',gen_random_uuid(),
 (select (result->>'row_version')::bigint from cp3_r4_benchmark_attendance)
);

insert into erp.payroll_settlements(id,payroll_number,contractor_id,period_start,period_end,status,attendance_total)
values('b7200000-0000-0000-0000-000000000001','CP3R4-BENCH-PAY','b1000000-0000-0000-0000-000000000001','2026-03-01','2026-03-15','APPROVED',100);
...
select
 ...
 (select (result->>'worker_id')::uuid from cp3_r4_benchmark_worker),
 ar.id,1,100
from erp.attendance_records ar
where ar.worker_id=(select (result->>'worker_id')::uuid from cp3_r4_benchmark_worker)
  and ar.attendance_date='2026-03-10';
select erp.post_journal('PAYROLL_ATTENDANCE_ACCRUAL','b7200000-0000-0000-0000-000000000001','2026-03-15','CP3 R4 benchmark approval',jsonb_build_array(
...
create temp table cp3_r4_benchmark_pool as
select erp.create_attendance_hpp_pool_v1(jsonb_build_object(
 'period_start','2026-03-01','period_end','2026-03-15','reason','CP3 R4 2000 destination benchmark'),gen_random_uuid()) result;

explain(analyze,buffers,format json)
select erp.validate_attendance_hpp_pool_v1((select (result->>'pool_id')::uuid from cp3_r4_benchmark_pool));

do $assert$
declare p uuid:=(select (result->>'pool_id')::uuid from cp3_r4_benchmark_pool);
begin
 if (select count(*) from erp.attendance_hpp_pool_allocations where pool_id=p)<>2000 then
  raise exception 'CP3 R4 benchmark allocation cardinality mismatch';
 end if;
 if (erp.validate_attendance_hpp_pool_v1(p)->>'validation') is distinct from 'PASS' then
  raise exception 'CP3 R4 bounded validator did not return PASS';
 end if;
 if exists(
  select 1 from pg_trigger t join pg_class c on c.oid=t.tgrelid join pg_namespace n on n.oid=c.relnamespace
  where n.nspname='erp' and c.relname like 'attendance_hpp_%' and not t.tgisinternal
 ) then raise exception 'CP3 R4 benchmark found deferred/row triggers on pool objects'; end if;
end
$assert$;
rollback;

select jsonb_build_object('status','PASS','destinations',2000,'validator_calls',2,'pool_row_triggers',0,'residue',0)
as cp3_r4_validator_benchmark_result;
