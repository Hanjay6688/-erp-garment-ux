...
begin
  ...
  retry_result:=erp.create_attendance_hpp_pool_v1(payload,request_id);
  if first_result is distinct from retry_result then
    raise exception 'Retry after changed-payload rejection did not return the original cached result';
  end if;
