...
begin;
-- Built by scripts/cp6_t3_awx_release.py from supabase/dev/cp6_ay_t1_family.sql (sha256 3d628cb09d11d6a71184ad82d98c33392ecce7adbc83ff0b3ef26ccc315c891c): the T1 body below is unchanged apart from the
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 select count(*),encode(extensions.digest(convert_to(coalesce(string_agg(length(key)::text||':'||key||':'||value,E'\n' order by key collate "C"),''),'UTF8'),'sha256'),'hex') into object_count,fingerprint from jsonb_each_text(actual);
 if object_count<>7288 or fingerprint is distinct from 'e53a57c2ee5b44d2b68e5b6fcdbe0067ca5358888b92ebe89ff609b75f750484' then
