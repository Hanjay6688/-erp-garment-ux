...
do $admission$ begin
 if exists(select 1 from erp.schema_migrations where version='v2.6.20ay') or to_regclass('erp.cp6_v2620ay_rollback_capsule') is not null
  or to_regclass('erp.po_hpp_gl_lot_state_v1') is not null or to_regclass('erp.po_hpp_gl_group_state_v1') is not null
...
do $after_data$ declare v_table text;v_hash jsonb;v_after jsonb;v_before jsonb; begin
 ...
 if (v_after-array['po_hpp_gl_lot_state_v1','po_hpp_gl_group_state_v1']::text[]) is distinct from v_before or exists(select 1 from unnest(array['po_hpp_gl_lot_state_v1','po_hpp_gl_group_state_v1']::text[]) t where (v_after->t->>'count') is distinct from '0')
