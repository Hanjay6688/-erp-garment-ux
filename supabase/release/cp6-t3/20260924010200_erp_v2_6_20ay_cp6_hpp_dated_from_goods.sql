...
begin;
-- Built by scripts/cp6_t3_awx_release.py from supabase/dev/cp6_ay_t1_family.sql (sha256 10d6658392f97474e9632049995f706678fe71564f5c8b13474911bbb1d8a5d5): the T1 body below is unchanged apart from the
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 select count(*),encode(extensions.digest(convert_to(coalesce(string_agg(length(key)::text||':'||key||':'||value,E'\n' order by key collate "C"),''),'UTF8'),'sha256'),'hex') into object_count,fingerprint from jsonb_each_text(actual);
 if object_count<>7281 or fingerprint is distinct from 'cb3dd9247d069fe7bb7b6fe5433a848af6cfaa5d18ad15680baa7381a71675fc' then
