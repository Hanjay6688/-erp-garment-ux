...
begin;
-- Built by scripts/cp6_t3_awx_release.py from supabase/dev/cp6_ay_t1_family.sql (sha256 6c3502cff2fb40d707f1b8d678f00c010e9e6f5ea47d1dabf840e05c86a1d9d1): the T1 body below is unchanged apart from the
...
do $admission$ begin
 if exists(select 1 from erp.schema_migrations where version='v2.6.20ay') or to_regclass('erp.cp6_v2620ay_rollback_capsule') is not null
  or to_regclass('erp.po_hpp_gl_lot_state_v1') is not null
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 select count(*),encode(extensions.digest(convert_to(coalesce(string_agg(length(key)::text||':'||key||':'||value,E'\n' order by key collate "C"),''),'UTF8'),'sha256'),'hex') into object_count,fingerprint from jsonb_each_text(actual);
 if object_count<>7288 or fingerprint is distinct from 'f61e59dbfceb105b3049e884423bdc65cf27d47ec94a3139bbf8254e21544e13' then
...
do $after_data$ declare v_table text;v_hash jsonb;v_after jsonb;v_before jsonb; begin
 ...
 if (v_after-array['po_hpp_gl_lot_state_v1']::text[]) is distinct from v_before or exists(select 1 from unnest(array['po_hpp_gl_lot_state_v1']::text[]) t where (v_after->t->>'count') is distinct from '0')
