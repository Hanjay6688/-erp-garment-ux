...
begin;
-- Built by scripts/cp6_t3_awx_release.py from supabase/dev/cp6_ay_t1_family.sql (sha256 e631bef72bf985524408549a966fc190811c6962b99ea3ce048108ad2058ce41): the T1 body below is unchanged apart from the
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 select count(*),encode(extensions.digest(convert_to(coalesce(string_agg(length(key)::text||':'||key||':'||value,E'\n' order by key collate "C"),''),'UTF8'),'sha256'),'hex') into object_count,fingerprint from jsonb_each_text(actual);
 if object_count<>7281 or fingerprint is distinct from 'da55287ee9ad22cfa9c4d5ed0241e197c904f81be17c9dfdba415e1ff9ba15b2' then
