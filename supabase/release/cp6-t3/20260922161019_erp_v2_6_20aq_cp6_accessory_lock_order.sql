-- CP6 AQ: ordered material locks before accessory child insertion.
begin;
...
do $admission$ begin
 if exists(select 1 from erp.schema_migrations where version='v2.6.20aq') or to_regclass('erp.cp6_v2620aq_rollback_capsule') is not null
  or exists(select 1 from supabase_migrations.schema_migrations where version>'20260922135615')
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7138 or fingerprint is distinct from '9db7095e0be59881f23c0422109160ea8bfdf9737662064186bd587e238de096' then
  raise exception 'AQ_PREDECESSOR_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
...
create table erp.cp6_v2620aq_rollback_capsule(like erp.cp6_v2620an_rollback_capsule including all);
alter table erp.cp6_v2620aq_rollback_capsule enable row level security;
revoke all on erp.cp6_v2620aq_rollback_capsule from public,anon,authenticated,service_role;
insert into erp.cp6_v2620aq_rollback_capsule(object_identity,object_regidentity,object_definition,definition_sha256,acl_snapshot,owner_snapshot)
select format('%I.%I(%s)',n.nspname,p.proname,pg_get_function_identity_arguments(p.oid)),'erp.save_contractor_material_issue_draft_v2(jsonb,uuid,bigint)',pg_get_functiondef(p.oid),
 encode(extensions.digest(convert_to(pg_get_functiondef(p.oid),'UTF8'),'sha256'),'hex'),
 array(select a::text from unnest(p.proacl)a order by a::text),pg_get_userbyid(p.proowner)
from pg_proc p join pg_namespace n on n.oid=p.pronamespace where p.oid='erp.save_contractor_material_issue_draft_v2(jsonb,uuid,bigint)'::regprocedure;
...
begin
  ...
  -- AQ: take the strongest material locks in one stable order before FK
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7138 or fingerprint is distinct from 'f98542d5e20d22f6322319f16d8ed050fa0f230a6bb901db2b050c6c7d2d779e' then
  raise exception 'AQ_INSTALLED_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
update erp.cp6_v2620aq_rollback_capsule set installed_definition_sha256='4d2e9b98d26dcda0c054de76d82906e0819af0525ed2327d476d78fbb59133f7';
do $after_data$ declare v_table text;v_hash jsonb;v_after jsonb; begin
 ...
 if v_after is distinct from (select boundary_snapshot->'before' from erp.cp6_v2620aq_rollback_capsule) then raise exception 'AQ_INSTALL_CHANGED_DATA';end if;
 update erp.cp6_v2620aq_rollback_capsule set boundary_snapshot=boundary_snapshot||jsonb_build_object('after',v_after);
end $after_data$;
insert into erp.schema_migrations(version,description) values('v2.6.20aq','Order accessory material locks before saving child lines');
do $capsule_guard$
declare r record;c record;e jsonb;boundary jsonb;actual jsonb;expected jsonb;
begin
