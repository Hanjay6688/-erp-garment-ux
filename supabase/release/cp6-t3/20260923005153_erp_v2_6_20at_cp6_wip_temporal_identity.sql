-- CP6 AT: WIP output identity must cover its physical production day.
begin;
...
do $admission$ begin
 if exists(select 1 from erp.schema_migrations where version='v2.6.20at') or to_regclass('erp.cp6_v2620at_rollback_capsule') is not null
  or exists(select 1 from supabase_migrations.schema_migrations where version>'20260922210815')
 then raise exception 'AT_EXACT_AS_WITHOUT_SUCCESSOR_REQUIRED';end if;
end $admission$;
do $catalog_guard$
...
with relations as (
 ...
 and c.relname not in('cp6_v2620ao_rollback_capsule','cp6_v2620ap_rollback_capsule','cp6_v2620aq_rollback_capsule','cp6_v2620ar_rollback_capsule','cp6_v2620as_rollback_capsule','cp6_v2620at_rollback_capsule')
), objects as (
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7138 or fingerprint is distinct from '5ef6b37f36838068b1953ce47384ea108d61dd5dee7ecf8cecf3659b3a3b2f18' then
  raise exception 'AT_PREDECESSOR_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
...
do $prior_platform$ begin
 if not exists(select 1 from erp.schema_migrations where version='v2.6.20ao')
  ...
  or not exists(select 1 from supabase_migrations.schema_migrations where version='20260922135612' and name='erp_v2_6_20ao_cp6_invoice_retail'
   and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')='f48f4a9611031631dca9bc787247b5beab624dcbe1f8e5635215953d16ee3448')
...
do $prior_platform$ begin
 if not exists(select 1 from erp.schema_migrations where version='v2.6.20ap')
  ...
  or not exists(select 1 from supabase_migrations.schema_migrations where version='20260922135615' and name='erp_v2_6_20ap_cp6_connected_import_materials'
   and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')='513834106e2e70c8e1f1f095cdd4934a85f0bf9cdba566e2d8f699751d15e099')
 then raise exception 'AQ_PRIOR_PLATFORM_DRIFT';end if;
end $prior_platform$;
do $capsule_guard$
declare r record;c record;e jsonb;boundary jsonb;actual jsonb;expected jsonb;
...
do $aq_platform$ begin
 if not exists(select 1 from erp.schema_migrations where version='v2.6.20aq')
  ...
  or not exists(select 1 from supabase_migrations.schema_migrations where version='20260922161019' and name='erp_v2_6_20aq_cp6_accessory_lock_order'
   and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')='10131f9605527ab1febe31bfe7774ce3f8ebed5faa0cf8227a602ecdfadb3ad4')
...
do $ar_platform$ begin
 if not exists(select 1 from erp.schema_migrations where version='v2.6.20ar')
  ...
  or not exists(select 1 from supabase_migrations.schema_migrations where version='20260922185015' and name='erp_v2_6_20ar_cp6_opening_overlap'
   and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')='792b176cc572e4fc7374ff41c2e10954b52d2ff9bd6058b47c3a293b6e168dfd')
...
do $as_platform$ begin
 if not exists(select 1 from erp.schema_migrations where version='v2.6.20as')
  ...
  or not exists(select 1 from supabase_migrations.schema_migrations where version='20260922210815' and name='erp_v2_6_20as_cp6_event_dates_product_identity'
   and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')='5cd3e5dcd84104c6f4d348b568cf8dc39b4d19c3003c6dd30cfb8bc9c8e99167')
 then raise exception 'AT_PRIOR_AS_PLATFORM_DRIFT';end if;
end $as_platform$;
create table erp.cp6_v2620at_rollback_capsule(like erp.cp6_v2620an_rollback_capsule including all);
alter table erp.cp6_v2620at_rollback_capsule enable row level security;
revoke all on erp.cp6_v2620at_rollback_capsule from public,anon,authenticated,service_role;
insert into erp.cp6_v2620at_rollback_capsule(object_identity,object_regidentity,object_definition,definition_sha256,acl_snapshot,owner_snapshot)
...
from unnest(array['erp.complete_initial_import_wip_v1(jsonb)']) i(identity)
...
do $before_data$ declare v_table text;v_hash jsonb;v_before jsonb; begin
 ...
 for v_table in select c.relname from pg_class c join pg_namespace n on n.oid=c.relnamespace
  where n.nspname='erp' and c.relkind in('r','p') and c.relname<>all(array['schema_migrations','cp6_v2620at_rollback_capsule']::text[]) order by 1 loop
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 select count(*),encode(extensions.digest(convert_to(coalesce(string_agg(length(key)::text||':'||key||':'||value,E'\n' order by key collate "C"),''),'UTF8'),'sha256'),'hex') into object_count,fingerprint from jsonb_each_text(actual);
 if object_count<>7138 or fingerprint is distinct from 'c77ead98a959e03e6e7fea96becafc10d9993054753de5fdfd78fb2f979c2284' then
  raise exception 'AT_INSTALLED_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
update erp.cp6_v2620at_rollback_capsule set installed_definition_sha256=encode(extensions.digest(convert_to(pg_get_functiondef(to_regprocedure(object_regidentity)),'UTF8'),'sha256'),'hex');
do $after_data$ declare v_table text;v_hash jsonb;v_after jsonb; begin
 ...
 if v_after is distinct from (select boundary_snapshot->'before' from erp.cp6_v2620at_rollback_capsule limit 1) then raise exception 'AT_INSTALL_CHANGED_DATA';end if;
 update erp.cp6_v2620at_rollback_capsule set boundary_snapshot=boundary_snapshot||jsonb_build_object('after',v_after);
end $after_data$;
insert into erp.schema_migrations(version,description) values('v2.6.20at','Respect WIP output product validity without changing historical opening identity rules');
do $capsule_guard$
declare r record;c record;e jsonb;boundary jsonb;actual jsonb;expected jsonb;
begin
