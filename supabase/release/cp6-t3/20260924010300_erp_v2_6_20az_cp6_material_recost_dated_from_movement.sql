...
begin;
-- Built by scripts/cp6_t3_awx_release.py from supabase/dev/cp6_az_t1_family.sql (sha256 10373d8686d74ebb534d305d7612a3fe9e2f7138d6e8079fcdf9e3b77a3eea9b): the T1 body below is unchanged apart from the
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7281 or fingerprint is distinct from '36f780d496a84f99079a79c314ed5b88d4b583456bf65fb6a70ded24f039595e' then
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 select count(*),encode(extensions.digest(convert_to(coalesce(string_agg(length(key)::text||':'||key||':'||value,E'\n' order by key collate "C"),''),'UTF8'),'sha256'),'hex') into object_count,fingerprint from jsonb_each_text(actual);
 if object_count<>7281 or fingerprint is distinct from '452768a6c8c11f19b37aa8e8b38ff7b093d02fb8a119f20bbcb97197af446d1f' then
