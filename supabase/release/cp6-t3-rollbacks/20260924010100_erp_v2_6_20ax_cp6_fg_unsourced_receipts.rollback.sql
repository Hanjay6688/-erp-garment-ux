...
do $platform$ begin
 if not exists(select 1 from erp.schema_migrations where version='v2.6.20ax')
  or (select count(*) from supabase_migrations.schema_migrations where name='erp_v2_6_20ax_cp6_fg_unsourced_receipts')<>1
  or not exists(select 1 from supabase_migrations.schema_migrations where version='20260924010100' and name='erp_v2_6_20ax_cp6_fg_unsourced_receipts'
   and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')='c736049386c65f75c82fa790b502cd32c999512738207b7be17d711185c3d9f2')
  or exists(select 1 from supabase_migrations.schema_migrations where version>'20260924010100')
 then raise exception 'AX_ROLLBACK_PLATFORM_OR_SUCCESSOR';end if;
end $platform$;
do $prior_platform$
declare r record;
begin
...
create temporary table cp6_t3_rollback_boundary on commit drop as select boundary_snapshot from erp.cp6_v2620ax_rollback_capsule limit 1;
do $pre_use$ declare v_table text;v_hash jsonb;v_after jsonb;b jsonb; begin
 ...
 if b is null or v_after is distinct from b->'after' then raise exception 'AX_POST_USE_ROLLBACK_REFUSED';end if;
 if (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from supabase_migrations.schema_migrations t where version<>'20260924010100') is distinct from b->>'platform_before'
  or (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from erp.schema_migrations t where version<>'v2.6.20ax') is distinct from b->>'markers_before'
 then raise exception 'AX_PRIOR_HISTORY_DRIFT';end if;
end $pre_use$;
do $restore_function$ declare r record; begin
 for r in select object_definition from erp.cp6_v2620ax_rollback_capsule order by object_regidentity loop execute r.object_definition;end loop;
 if exists(select 1 from erp.cp6_v2620ax_rollback_capsule x join pg_proc p on p.oid=to_regprocedure(x.object_regidentity)
  ...
  or (select count(*) from erp.cp6_v2620ax_rollback_capsule x where to_regprocedure(x.object_regidentity) is not null)<>2
...
drop function erp.guard_fg_unsourced_repair_wage_immutable_v1();
drop function erp.post_fg_unsourced_receipt_v1(jsonb,uuid);
drop function erp.reverse_fg_unsourced_receipt_v1(uuid,text,uuid);
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7198 or fingerprint is distinct from '1d844c41d8ef5b8c9aefbc1afe6dbd5a106d43e9a4457bcd873fcf035dcdbbf1' then
  raise exception 'AX_RESTORED_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
do $restored_data$ declare v_table text;v_hash jsonb;v_before jsonb;b jsonb; begin
 ...
 then raise exception 'AX_EXACT_RESTORE_FAILED';end if;
end $restored_data$;
commit;
