...
do $platform$ begin
 if not exists(select 1 from erp.schema_migrations where version='v2.6.20aw')
  ...
  or not exists(select 1 from supabase_migrations.schema_migrations where version='20260924010000' and name='erp_v2_6_20aw_cp6_close_readiness_engine'
   and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')='c38edc9da084f4a90ee615bde16e6fea72e1072f79d6820d9d3bef27cc2a8ecb')
  or exists(select 1 from supabase_migrations.schema_migrations where version>'20260924010000')
 then raise exception 'AW_ROLLBACK_PLATFORM_OR_SUCCESSOR';end if;
end $platform$;
do $prior_platform$
declare r record;
begin
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7198 or fingerprint is distinct from '1d844c41d8ef5b8c9aefbc1afe6dbd5a106d43e9a4457bcd873fcf035dcdbbf1' then
  raise exception 'AW_ROLLBACK_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
...
create temporary table cp6_t3_rollback_boundary on commit drop as select boundary_snapshot from erp.cp6_v2620aw_rollback_capsule limit 1;
do $pre_use$ declare v_table text;v_hash jsonb;v_after jsonb;b jsonb; begin
 ...
 if b is null or v_after is distinct from b->'after' then raise exception 'AW_POST_USE_ROLLBACK_REFUSED';end if;
 if (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from supabase_migrations.schema_migrations t where version<>'20260924010000') is distinct from b->>'platform_before'
  or (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from erp.schema_migrations t where version<>'v2.6.20aw') is distinct from b->>'markers_before'
 then raise exception 'AW_PRIOR_HISTORY_DRIFT';end if;
end $pre_use$;
do $restore_function$ declare r record; begin
 for r in select object_definition from erp.cp6_v2620aw_rollback_capsule order by object_regidentity loop execute r.object_definition;end loop;
 if exists(select 1 from erp.cp6_v2620aw_rollback_capsule x join pg_proc p on p.oid=to_regprocedure(x.object_regidentity)
...
drop function erp.period_blockers_v1(date,date);
drop function erp.period_completeness_from_v1();
drop function erp.period_integrity_check_registry_v1();
drop function erp.period_readiness_v1(date,date);
drop function erp.set_laundry_rate_owner_estimate_v1(uuid,numeric,text);
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7159 or fingerprint is distinct from 'f345b9f9cdefd97250814cb29c459cde447c1a26846354c516f058abb23a9fdf' then
  raise exception 'AW_RESTORED_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
do $restored_data$ declare v_table text;v_hash jsonb;v_before jsonb;b jsonb; begin
 ...
 then raise exception 'AW_EXACT_RESTORE_FAILED';end if;
end $restored_data$;
commit;
