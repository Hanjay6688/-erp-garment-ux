-- T3 release variant of supabase/rollbacks/20260922210815_erp_v2_6_20as_cp6_event_dates_product_identity.rollback.sql (sha256 0d6482d286b26370a8d0db11393ecaed538f0c7c6a495c8d269f05792d74b1ed), built by scripts/cp6_t3_rollback_acav.py from docs/evidence/cp6-t3/release_pins.json.
-- CP6 AS: exact pre-use restore to AR under closed admission.
begin;
...
create temporary table cp6_as_restore_boundary on commit drop as select boundary_snapshot from erp.cp6_v2620as_rollback_capsule limit 1;
do $pre_use$ declare v_table text;v_hash jsonb;v_after jsonb;b jsonb; begin
 select boundary_snapshot into b from erp.cp6_v2620as_rollback_capsule limit 1;
 v_after:='{}'::jsonb;
 ...
 if (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from supabase_migrations.schema_migrations t where not(version='20260922210815')) is distinct from b->>'platform_before'
  or (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from erp.schema_migrations t where not(version='v2.6.20as')) is distinct from b->>'markers_before'
 then raise exception 'AS_PRIOR_HISTORY_DRIFT';end if;
end $pre_use$;
do $restore_function$ declare r record; begin for r in select object_definition from erp.cp6_v2620as_rollback_capsule order by object_regidentity loop execute r.object_definition;end loop;end $restore_function$;
drop table erp.cp6_v2620as_rollback_capsule;
delete from erp.schema_migrations where version='v2.6.20as';
delete from supabase_migrations.schema_migrations where version='20260922210815' and name='erp_v2_6_20as_cp6_event_dates_product_identity';
do $catalog_guard$
declare actual jsonb;fingerprint text;object_count bigint;
begin
 select * into actual from (
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7138 or fingerprint is distinct from '6c2d2d70ecb492992980874f65642b2742dbbf6db8d81bc3bd8777f2341c9604' then
  raise exception 'AS_RESTORED_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
