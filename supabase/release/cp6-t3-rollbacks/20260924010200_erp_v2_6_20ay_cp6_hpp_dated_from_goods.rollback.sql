-- CP6 AY rollback: exact pre-use restore of the T3 release package to its predecessor; closed, drained maintenance required.
begin;
-- Built by scripts/cp6_t3_rollback.py from supabase/release/cp6-t3/20260924010200_erp_v2_6_20ay_cp6_hpp_dated_from_goods.sql (sha256 fd5ec47a16af264cc9b6a3aeed3d6e19023dc9f49beb51ceb8bfb4ecb14dc411) and docs/evidence/cp6-t3/rollback_capture.json (sha256 62bb0e2a9a0bb69106538d32569239bf67bb8c79e05325808f837ad6f8450cf6).
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7297 or fingerprint is distinct from '1c2e99507a2e6fd0b8a2700d7eca65fa7fc6199cb365054d3135c2d8a7782140' then
  raise exception 'AY_ROLLBACK_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
...
begin
 if not exists(select 1 from pg_class where oid='erp.cp6_v2620ay_rollback_capsule'::regclass and relrowsecurity and not relforcerowsecurity and pg_get_userbyid(relowner)='postgres')
   ...
   or (select count(*) from erp.cp6_v2620ay_rollback_capsule)<>2 then raise exception 'AY_CAPSULE_SECURITY_OR_COUNT';end if;
 select jsonb_build_object(
   'relation',(select jsonb_build_array(relkind,relpersistence,relreplident,relispartition,reloptions) from pg_class where oid='erp.cp6_v2620an_rollback_capsule'::regclass),
   'columns',(select jsonb_agg(jsonb_build_array(a.attname,format_type(a.atttypid,a.atttypmod),a.attnotnull,a.attidentity,a.attgenerated,pg_get_expr(d.adbin,d.adrelid)) order by a.attnum) from pg_attribute a left join pg_attrdef d on d.adrelid=a.attrelid and d.adnum=a.attnum where a.attrelid='erp.cp6_v2620an_rollback_capsule'::regclass and a.attnum>0 and not a.attisdropped),
   'constraints',(select jsonb_agg(jsonb_build_array(contype,pg_get_constraintdef(oid),condeferrable,condeferred,convalidated) order by contype,pg_get_constraintdef(oid)) from pg_constraint where conrelid='erp.cp6_v2620an_rollback_capsule'::regclass),
 ...
 if 2>0 and (boundary is null or exists(select 1 from erp.cp6_v2620ay_rollback_capsule where boundary_snapshot is distinct from boundary)
  or not(boundary ?& array['before','after','platform_before','markers_before'])) then raise exception 'AY_CAPSULE_BOUNDARY';end if;
 if exists(select 1 from erp.cp6_v2620ay_rollback_capsule where object_regidentity<>all(array['erp.sync_po_hpp_to_gl(uuid,date)','erp.rebuild_po_hpp(uuid,text)']::text[])
...
create temporary table cp6_t3_rollback_boundary on commit drop as select boundary_snapshot from erp.cp6_v2620ay_rollback_capsule limit 1;
do $pre_use$ declare v_table text;v_hash jsonb;v_after jsonb;b jsonb; begin
 ...
 if b is null or v_after is distinct from b->'after' then raise exception 'AY_POST_USE_ROLLBACK_REFUSED';end if;
 if (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from supabase_migrations.schema_migrations t where version<>'20260924010200') is distinct from b->>'platform_before'
  or (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from erp.schema_migrations t where version<>'v2.6.20ay') is distinct from b->>'markers_before'
 then raise exception 'AY_PRIOR_HISTORY_DRIFT';end if;
end $pre_use$;
do $restore_function$ declare r record; begin
 for r in select object_definition from erp.cp6_v2620ay_rollback_capsule order by object_regidentity loop execute r.object_definition;end loop;
 if exists(select 1 from erp.cp6_v2620ay_rollback_capsule x join pg_proc p on p.oid=to_regprocedure(x.object_regidentity)
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7280 or fingerprint is distinct from 'd27ef6c2d2d1b55e96d9ac2fc2b110dd631486154bfba353f3257a67c55b69ef' then
  raise exception 'AY_RESTORED_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
do $restored_data$ declare v_table text;v_hash jsonb;v_before jsonb;b jsonb; begin
 ...
 then raise exception 'AY_EXACT_RESTORE_FAILED';end if;
end $restored_data$;
commit;
