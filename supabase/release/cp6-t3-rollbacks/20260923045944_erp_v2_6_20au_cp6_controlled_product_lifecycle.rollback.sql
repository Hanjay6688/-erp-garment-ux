-- T3 release variant of supabase/rollbacks/20260923045944_erp_v2_6_20au_cp6_controlled_product_lifecycle.rollback.sql (sha256 6f00891cd095dff0ba74b2cb12ffd430f83e6a0cb28e3b2a461b2dbf99f8d10f), built by scripts/cp6_t3_rollback_acav.py from docs/evidence/cp6-t3/release_pins.json.
-- CP6 AU: exact pre-use restore to AT under closed admission.
begin;
...
create temporary table cp6_au_restore_boundary on commit drop as select boundary_snapshot from erp.cp6_v2620au_rollback_capsule limit 1;
do $pre_use$ declare v_table text;v_hash jsonb;v_after jsonb;b jsonb; begin
 select boundary_snapshot into b from erp.cp6_v2620au_rollback_capsule limit 1;
 v_after:='{}'::jsonb;
 ...
 if (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from supabase_migrations.schema_migrations t where not(version='20260923045944')) is distinct from b->>'platform_before'
  or (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from erp.schema_migrations t where not(version='v2.6.20au')) is distinct from b->>'markers_before'
...
delete from supabase_migrations.schema_migrations where version='20260923045944' and name='erp_v2_6_20au_cp6_controlled_product_lifecycle';
do $catalog_guard$
declare actual jsonb;fingerprint text;object_count bigint;
begin
 select * into actual from (
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7138 or fingerprint is distinct from 'c77ead98a959e03e6e7fea96becafc10d9993054753de5fdfd78fb2f979c2284' then
  raise exception 'AU_RESTORED_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
