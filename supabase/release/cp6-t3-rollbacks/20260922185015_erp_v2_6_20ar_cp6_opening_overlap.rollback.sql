-- T3 release variant of supabase/rollbacks/20260922185015_erp_v2_6_20ar_cp6_opening_overlap.rollback.sql (sha256 732df12f2da63ca605d26f909307ca77499faadf5ef9c12e6e2a47c08fca674d), built by scripts/cp6_t3_rollback_acav.py from docs/evidence/cp6-t3/release_pins.json.
-- CP6 AR: exact pre-use restore to AQ under closed admission.
begin;
...
create temporary table cp6_ar_restore_boundary on commit drop as select boundary_snapshot from erp.cp6_v2620ar_rollback_capsule limit 1;
do $pre_use$ declare v_table text;v_hash jsonb;v_after jsonb;b jsonb; begin
 select boundary_snapshot into b from erp.cp6_v2620ar_rollback_capsule limit 1;
 v_after:='{}'::jsonb;
 ...
 if (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from supabase_migrations.schema_migrations t where not(version='20260922185015')) is distinct from b->>'platform_before'
  or (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from erp.schema_migrations t where not(version='v2.6.20ar')) is distinct from b->>'markers_before'
 then raise exception 'AR_PRIOR_HISTORY_DRIFT';end if;
end $pre_use$;
do $restore_function$ declare r record; begin for r in select object_definition from erp.cp6_v2620ar_rollback_capsule order by object_regidentity loop execute r.object_definition;end loop;end $restore_function$;
drop table erp.cp6_v2620ar_rollback_capsule;
delete from erp.schema_migrations where version='v2.6.20ar';
delete from supabase_migrations.schema_migrations where version='20260922185015' and name='erp_v2_6_20ar_cp6_opening_overlap';
do $catalog_guard$
declare actual jsonb;fingerprint text;object_count bigint;
begin
 select * into actual from (
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7138 or fingerprint is distinct from 'f98542d5e20d22f6322319f16d8ed050fa0f230a6bb901db2b050c6c7d2d779e' then
  raise exception 'AR_RESTORED_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
