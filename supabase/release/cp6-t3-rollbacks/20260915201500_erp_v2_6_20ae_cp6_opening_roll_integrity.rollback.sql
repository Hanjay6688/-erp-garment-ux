-- T3 release variant of supabase/rollbacks/20260915201500_erp_v2_6_20ae_cp6_opening_roll_integrity.rollback.sql (sha256 a869de31481cac1c8a26de17e3be9d47d2b42c18a6756df5cabfc016ebfa0341), built by scripts/cp6_t3_rollback_acav.py from docs/evidence/cp6-t3/release_pins.json.
     ...
     or not exists(select 1 from supabase_migrations.schema_migrations
       where version='20260915201500' and name='erp_v2_6_20ae_cp6_opening_roll_integrity'
         and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')
           in('0c6bbf77a68142ddfdae99761a5cc2cd6e2e538be50b391f3303922c748c39d6'))
     or exists(select 1 from supabase_migrations.schema_migrations where version>'20260915201500') then
...
lock table erp.cp6_v2620ae_rollback_capsule in access exclusive mode;

do $lock_all_erp_v2620ae$
declare v_table text;
begin
  for v_table in select relation.relname from pg_class relation join pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='erp' and relation.relkind in('r','p') and relation.relname not in('schema_migrations','cp6_v2620ae_rollback_capsule') order by relation.relname loop
    execute format('lock table erp.%I in share row exclusive mode',v_table);
  end loop;
end
$lock_all_erp_v2620ae$;

do $restore_guard_v2620ae$
...
begin
  if not exists(select 1 from erp.schema_migrations where version='v2.6.20ad')
     ...
     or exists(select 1 from erp.schema_migrations
       where version not in('v2.6.20ad','v2.6.20ae')
         and installed_at>(select installed_at from erp.schema_migrations where version='v2.6.20ae'))
  ...
  for r in select * from(values
    ('erp.post_opening_balance(uuid)','cc89ac9e978723c70728cc183104490109bf2530ce8dd1a99b314018a5b0f6eb'),
    ('erp.run_v268_financial_report_checks()','6a93242cb0325baee511c2753eb65588600fe8b9a7fc62d423b85f14bf9c71d5')
...
drop table erp.cp6_v2620ae_rollback_capsule;
delete from erp.schema_migrations where version='v2.6.20ae';
delete from supabase_migrations.schema_migrations
where version='20260915201500'
  and name='erp_v2_6_20ae_cp6_opening_roll_integrity'
  and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')
    in('0c6bbf77a68142ddfdae99761a5cc2cd6e2e538be50b391f3303922c748c39d6');

do $postcheck_v2620ae$
...
begin
  ...
  if exists(select 1 from erp.schema_migrations where version='v2.6.20ae')
     or exists(select 1 from supabase_migrations.schema_migrations where name='erp_v2_6_20ae_cp6_opening_roll_integrity')
