-- CP6 AZ rollback: exact pre-use restore of the T3 release package to its predecessor; closed, drained maintenance required.
begin;
-- Built by scripts/cp6_t3_rollback.py from supabase/release/cp6-t3/20260924010300_erp_v2_6_20az_cp6_material_recost_dated_from_movement.sql (sha256 e515d12a995a1806537ac08b7b0da15eb817480ead3103b035fc651b62977b8d) and docs/evidence/cp6-t3/rollback_capture.json (sha256 3b1e34c5c581299dad99b0d084e1b0e9cb14c3f29781badf0f96759a9a54849d).
...
do $platform$ begin
 if not exists(select 1 from erp.schema_migrations where version='v2.6.20az')
  or (select count(*) from supabase_migrations.schema_migrations where name='erp_v2_6_20az_cp6_material_recost_dated_from_movement')<>1
  or not exists(select 1 from supabase_migrations.schema_migrations where version='20260924010300' and name='erp_v2_6_20az_cp6_material_recost_dated_from_movement'
   and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')='e515d12a995a1806537ac08b7b0da15eb817480ead3103b035fc651b62977b8d')
  or exists(select 1 from supabase_migrations.schema_migrations where version>'20260924010300')
 then raise exception 'AZ_ROLLBACK_PLATFORM_OR_SUCCESSOR';end if;
end $platform$;
do $prior_platform$
declare r record;
begin
...
create temporary table cp6_t3_rollback_boundary on commit drop as select boundary_snapshot from erp.cp6_v2620az_rollback_capsule limit 1;
do $pre_use$ declare v_table text;v_hash jsonb;v_after jsonb;b jsonb; begin
 ...
 if b is null or v_after is distinct from b->'after' then raise exception 'AZ_POST_USE_ROLLBACK_REFUSED';end if;
 if (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from supabase_migrations.schema_migrations t where version<>'20260924010300') is distinct from b->>'platform_before'
  or (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from erp.schema_migrations t where version<>'v2.6.20az') is distinct from b->>'markers_before'
 then raise exception 'AZ_PRIOR_HISTORY_DRIFT';end if;
end $pre_use$;
do $restore_function$ declare r record; begin
 for r in select object_definition from erp.cp6_v2620az_rollback_capsule order by object_regidentity loop execute r.object_definition;end loop;
 if exists(select 1 from erp.cp6_v2620az_rollback_capsule x join pg_proc p on p.oid=to_regprocedure(x.object_regidentity)
  ...
  or (select count(*) from erp.cp6_v2620az_rollback_capsule x where to_regprocedure(x.object_regidentity) is not null)<>12
 then raise exception 'AZ_FUNCTION_RESTORE_MISMATCH';end if;
end $restore_function$;
drop table erp.cp6_v2620az_rollback_capsule;
delete from erp.schema_migrations where version='v2.6.20az';
delete from supabase_migrations.schema_migrations where version='20260924010300' and name='erp_v2_6_20az_cp6_material_recost_dated_from_movement';
do $catalog_guard$
declare actual jsonb;fingerprint text;object_count bigint;
begin
 select * into actual from (
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 select count(*),encode(extensions.digest(convert_to(coalesce(string_agg(length(key)::text||':'||key||':'||value,E'\n' order by key collate "C"),''),'UTF8'),'sha256'),'hex') into object_count,fingerprint from jsonb_each_text(actual);
 if object_count<>7297 or fingerprint is distinct from '1c2e99507a2e6fd0b8a2700d7eca65fa7fc6199cb365054d3135c2d8a7782140' then
  raise exception 'AZ_RESTORED_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
do $restored_data$ declare v_table text;v_hash jsonb;v_before jsonb;b jsonb; begin
 ...
 then raise exception 'AZ_EXACT_RESTORE_FAILED';end if;
end $restored_data$;
commit;
