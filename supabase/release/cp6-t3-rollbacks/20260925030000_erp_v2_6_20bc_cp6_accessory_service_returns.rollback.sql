...
create temporary table cp6_t3_rollback_boundary on commit drop as select boundary_snapshot from erp.cp6_v2620bc_rollback_capsule limit 1;
do $pre_use$ declare v_table text;v_hash jsonb;v_after jsonb;b jsonb; begin
 ...
 if b is null or v_after is distinct from b->'after' then raise exception 'BC_POST_USE_ROLLBACK_REFUSED';end if;
 if (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from supabase_migrations.schema_migrations t where version<>'20260925030000') is distinct from b->>'platform_before'
  or (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from erp.schema_migrations t where version<>'v2.6.20bc') is distinct from b->>'markers_before'
 then raise exception 'BC_PRIOR_HISTORY_DRIFT';end if;
end $pre_use$;
do $restore_function$ declare r record; begin
 for r in select object_definition from erp.cp6_v2620bc_rollback_capsule order by object_regidentity loop execute r.object_definition;end loop;
 if exists(select 1 from erp.cp6_v2620bc_rollback_capsule x join pg_proc p on p.oid=to_regprocedure(x.object_regidentity)
  ...
  or (select count(*) from erp.cp6_v2620bc_rollback_capsule x where to_regprocedure(x.object_regidentity) is not null)<>22
 then raise exception 'BC_FUNCTION_RESTORE_MISMATCH';end if;
end $restore_function$;
drop trigger trg_bc_note_reversal on erp.contractor_material_issues;
drop trigger trg_bc_zone_location on erp.locations;
drop trigger trg_bc_zone_movement on erp.material_stock_movements;
drop trigger trg_bc_note_credit_settlement on erp.opening_subledger_settlements;
drop trigger trg_bc_carry_reimbursement on erp.payroll_reimbursements;
...
drop function erp.bc_adjustment_account_v1(uuid,text);
drop function erp.bc_adjustment_value_v1(uuid);
...
drop function erp.bc_close_custody_v1(jsonb,uuid);
drop function erp.bc_count_post_v1(jsonb,uuid);
drop function erp.bc_credit_note_return_v1(jsonb,uuid);
drop function erp.bc_customer_garment_v1(text,jsonb,uuid);
drop function erp.bc_dispose_stock_v1(jsonb,uuid);
drop function erp.bc_enter_context_v1();
drop function erp.bc_free_basis_v1(uuid,uuid,timestamp with time zone);
drop function erp.bc_guard_carry_reimbursement_v1();
drop function erp.bc_guard_note_credit_settlement_v1();
drop function erp.bc_guard_note_reversal_v1();
drop function erp.bc_guard_zone_location_v1();
drop function erp.bc_guard_zone_movement_v1();
drop function erp.bc_import_revision_part_v1(uuid);
drop function erp.bc_import_workspace_v1(uuid);
drop function erp.bc_in_context_v1();
drop function erp.bc_inspect_v1(jsonb,uuid);
drop function erp.bc_internal_use_v1(jsonb,uuid);
drop function erp.bc_leave_context_v1();
drop function erp.bc_lines_v1(jsonb,text[]);
drop function erp.bc_link_v1(uuid,text,uuid);
drop function erp.bc_location_v1(uuid,text[],text);
drop function erp.bc_lock_lot_v1(uuid);
drop function erp.bc_lot_state_text_v1(uuid);
drop function erp.bc_lot_state_v1(uuid);
drop function erp.bc_move_post_v1(text,jsonb,uuid);
drop function erp.bc_new_document_v1(uuid,text,timestamp with time zone,jsonb,jsonb);
drop function erp.bc_note_item_adjustment_v1(uuid);
drop function erp.bc_note_item_allocated_v1(uuid);
drop function erp.bc_note_item_collectible_v1(uuid);
drop function erp.bc_note_item_share_v1(uuid);
drop function erp.bc_note_return_issue_movement_v1(uuid);
drop function erp.bc_opening_note_credit_lines_v1(uuid);
drop function erp.bc_parse_at_v1(text,text);
drop function erp.bc_parse_qty_v1(uuid,jsonb,text);
drop function erp.bc_policy_account_v1(jsonb,text,text[]);
drop function erp.bc_policy_v1(text);
drop function erp.bc_policy_version_v1(text);
drop function erp.bc_post_adjustment_v1(uuid,uuid,timestamp with time zone,text,jsonb,text,text,uuid,text);
drop function erp.bc_post_transfer_v1(uuid,uuid,uuid,timestamp with time zone,jsonb,text);
drop function erp.bc_receive_return_v1(jsonb,uuid);
drop function erp.bc_register_zone_v1(jsonb,uuid);
drop function erp.bc_require_policy_v1(text,text);
drop function erp.bc_require_value_approval_v1(numeric);
drop function erp.bc_resolve_variance_v1(jsonb,uuid);
drop function erp.bc_reverse_v1(jsonb,uuid);
drop function erp.bc_round_note_v1(jsonb,uuid);
drop function erp.bc_set_policy_v1(jsonb,uuid);
drop function erp.bc_stock_at_v1(uuid,uuid);
drop function erp.bc_text_v1(jsonb,text,boolean,integer);
drop function erp.bc_uuid_v1(jsonb,text,boolean);
drop function erp.bc_validate_imports_v1(uuid);
drop function erp.bc_value_custody_v1(jsonb,uuid);
drop function erp.bc_zone_kind_v1(uuid);
drop function erp.get_accessory_service_workspace_v1(jsonb);
drop function erp.save_accessory_service_action_v1(text,jsonb,uuid);
drop function public.erp_get_accessory_service_workspace_v1(jsonb);
drop function public.erp_save_accessory_service_action_v1(text,jsonb,uuid);
drop table erp.bc_free_issue_lines_v1;
drop table erp.bc_note_roundings_v1;
drop table erp.bc_opening_note_lines_v1;
drop table erp.bc_count_variances_v1;
drop table erp.bc_lot_events_v1;
drop table erp.bc_return_lots_v1;
drop table erp.bc_customer_custody_v1;
drop table erp.bc_outstanding_returns_v1;
drop table erp.bc_internal_use_lines_v1;
drop table erp.bc_adjustment_purposes_v1;
drop table erp.bc_document_links_v1;
drop table erp.bc_documents_v1;
drop table erp.bc_execution_context_v1;
drop table erp.bc_accessory_zones_v1;
drop table erp.bc_policy_setting_events_v1;
drop table erp.bc_policy_settings_v1;
drop table erp.cp6_v2620bc_rollback_capsule;
delete from erp.schema_migrations where version='v2.6.20bc';
delete from supabase_migrations.schema_migrations where version='20260925030000' and name='erp_v2_6_20bc_cp6_accessory_service_returns';
do $catalog_guard$
declare actual jsonb;fingerprint text;object_count bigint;
begin
 select * into actual from (
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7848 or fingerprint is distinct from '760f029cbbcaeb2fe81f03795e1544376b94c7b29e5088c9a28bb99bd4fa0dd4' then
  raise exception 'BC_RESTORED_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
do $restored_data$ declare v_table text;v_hash jsonb;v_before jsonb;b jsonb; begin
 ...
 then raise exception 'BC_EXACT_RESTORE_FAILED';end if;
end $restored_data$;
commit;
