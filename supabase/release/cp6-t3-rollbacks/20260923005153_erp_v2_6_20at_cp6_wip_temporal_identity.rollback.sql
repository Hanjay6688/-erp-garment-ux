-- T3 release variant of supabase/rollbacks/20260923005153_erp_v2_6_20at_cp6_wip_temporal_identity.rollback.sql (sha256 873552ff2da76cfee680c7a5f6feb7099e43435e3f411f12a9dbb8b3825018e6), built by scripts/cp6_t3_rollback_acav.py from docs/evidence/cp6-t3/release_pins.json.
-- CP6 AT: exact pre-use restore to AS under closed admission.
begin;
...
do $platform$ begin
 ...
 then raise exception 'AT_ROLLBACK_PLATFORM_OR_SUCCESSOR';end if;
end $platform$;
do $catalog_guard$
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7138 or fingerprint is distinct from 'c77ead98a959e03e6e7fea96becafc10d9993054753de5fdfd78fb2f979c2284' then
  raise exception 'AT_ROLLBACK_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
...
create temporary table cp6_at_restore_boundary on commit drop as select boundary_snapshot from erp.cp6_v2620at_rollback_capsule limit 1;
do $pre_use$ declare v_table text;v_hash jsonb;v_after jsonb;b jsonb; begin
 select boundary_snapshot into b from erp.cp6_v2620at_rollback_capsule limit 1;
 v_after:='{}'::jsonb;
 ...
 if v_after is distinct from b->'after' then raise exception 'AT_POST_USE_ROLLBACK_REFUSED';end if;
 if (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from supabase_migrations.schema_migrations t where not(version='20260923005153')) is distinct from b->>'platform_before'
  or (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from erp.schema_migrations t where not(version='v2.6.20at')) is distinct from b->>'markers_before'
 then raise exception 'AT_PRIOR_HISTORY_DRIFT';end if;
end $pre_use$;
do $restore_function$ declare r record; begin for r in select object_definition from erp.cp6_v2620at_rollback_capsule order by object_regidentity loop execute r.object_definition;end loop;end $restore_function$;
drop table erp.cp6_v2620at_rollback_capsule;
delete from erp.schema_migrations where version='v2.6.20at';
delete from supabase_migrations.schema_migrations where version='20260923005153' and name='erp_v2_6_20at_cp6_wip_temporal_identity';
do $catalog_guard$
declare actual jsonb;fingerprint text;object_count bigint;
begin
 select * into actual from (
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7138 or fingerprint is distinct from '5ef6b37f36838068b1953ce47384ea108d61dd5dee7ecf8cecf3659b3a3b2f18' then
  raise exception 'AT_RESTORED_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
do $restored_data$ declare v_table text;v_hash jsonb;v_before jsonb;b jsonb; begin
 select boundary_snapshot into b from pg_temp.cp6_at_restore_boundary;
 v_before:='{}'::jsonb;
 for v_table in select c.relname from pg_class c join pg_namespace n on n.oid=c.relnamespace
 ...
 then raise exception 'AT_EXACT_RESTORE_FAILED';end if;
end $restored_data$;
commit;
