-- T3 release variant of supabase/rollbacks/20260915031500_erp_v2_6_20ac_cp6_temporal_surface_closure.rollback.sql (sha256 ebf1d0f66fe0b50d41d872c19510adad9af993bd03ae5e513542004b7e5b1acb), built by scripts/cp6_t3_rollback_acav.py from docs/evidence/cp6-t3/release_pins.json.
     ...
     or not exists(select 1 from supabase_migrations.schema_migrations
       where version='20260915031500' and name='erp_v2_6_20ac_cp6_temporal_surface_closure'
         and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')
           in('871fb32b1d4a1e7f9aedfef684b0f2c766092732c38552620eb394061f6240d1'))
...
insert into pg_temp.cp6_ac_expected_views values
 ...
 SELECT m.id AS material_id,
  ...
  ('erp.v_payroll_nota_browser','SELECT n.id AS payroll_id,
    ...
    (n.status IN (''DRAFT'', ''CALCULATED'', ''REVIEW'')) AS can_merge,
    (n.status IN (''APPROVED'', ''PAID'', ''REVERSED'')) AS is_locked,
    n.notes,
    n.created_at,
    n.updated_at
   FROM (((erp.payroll_settlements n
     ...
     LEFT JOIN LATERAL ( SELECT count(*) AS eligible_line_count,
          ...
          WHERE ((q.contractor_id = n.contractor_id) AND ((q.eligible_at)::date <= n.period_end))) e ON (true))','fb234170ab4476b5395ed72752bab1d4e685461ff02cdf8f79150013db3cca0b','SELECT n.id AS payroll_id,
    n.payroll_number,
    n.contractor_id,
    c.contractor_code,
    c.contractor_name,
   ...
   FROM (((erp.payroll_settlements n
     ...
     LEFT JOIN LATERAL ( SELECT count(*) AS eligible_line_count,
            ...
            COALESCE(sum(((q.remaining_qty)::numeric * q.rate_snapshot)), (0)::numeric) AS eligible_amount
           FROM erp.v_payroll_eligible_work_lines q
          WHERE ((q.contractor_id = n.contractor_id) AND (((q.eligible_at) AT TIME ZONE ''Asia/Jakarta'')::date <= n.period_end))) e ON (true))','16f47474d81b89b0639a3c46cd49c90140691926e4cc03d610e493b1677ca226','postgres',array['authenticated=r/postgres','postgres=arwdDxtm/postgres']::text[],array['security_invoker=true']::text[],false,'823d3fdac5ab25a9c92ee5901d9f3601b16eb34174409ee1b4f9f711b5c2ee16'),
