-- T3 release variant of supabase/rollbacks/20260917054049_erp_v2_6_20al_cp6_opening_value_validation.rollback.sql (sha256 10ae9c470f363d778cde1c61f6b4f7388ba72efb30038adbfe5dc300ae687d07), built by scripts/cp6_t3_rollback_acav.py from docs/evidence/cp6-t3/release_pins.json.
-- REVIEWED PRE-USE ROLLBACK: CP6 v2.6.20al -> exact AK.
...
begin
  if (select count(*) from supabase_migrations.schema_migrations where name='erp_v2_6_20al_cp6_opening_value_validation')<>1
     or not exists(select 1 from supabase_migrations.schema_migrations
       where version='20260917054049' and name='erp_v2_6_20al_cp6_opening_value_validation'
         and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')
           in('2d16461cababd4a27827cfb8de01dd993a3a343460cdf049d427b44dd4cb6ea2'))
     or exists(select 1 from supabase_migrations.schema_migrations where version>'20260917054049') then
    raise exception 'AL_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR';
  end if;
end
$platform_guard_v2620al$;
...
lock table erp.cp6_v2620al_rollback_capsule in access exclusive mode;

do $lock_all_erp_v2620al$
declare v_table text;
begin
  for v_table in select relation.relname from pg_class relation join pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='erp' and relation.relkind in('r','p') and relation.relname not in('schema_migrations','cp6_v2620al_rollback_capsule') order by relation.relname loop
    execute format('lock table erp.%I in share row exclusive mode',v_table);
  end loop;
end
$lock_all_erp_v2620al$;

do $restore_guard_v2620al$
...
begin
  if not exists(select 1 from erp.schema_migrations where version='v2.6.20ak')
     ...
     or exists(select 1 from erp.schema_migrations
       where version not in('v2.6.20ak','v2.6.20al')
         and installed_at>(select installed_at from erp.schema_migrations where version='v2.6.20al'))
     or (select count(*) from erp.cp6_v2620al_rollback_capsule)<>2 then
    raise exception 'AL_ROLLBACK_MARKER_CAPSULE_OR_SUCCESSOR';
  end if;
  for r in select * from(values
    ...
    into c from erp.cp6_v2620al_rollback_capsule cap
    ...
    if c.object_regidentity is null
      ...
      raise exception 'AL_TRUSTED_PREDECESSOR_PIN_MISMATCH: %',r.identity;
    end if;
  end loop;
  select boundary_snapshot into v_expected from erp.cp6_v2620al_rollback_capsule limit 1;
  if (select count(*) from (
       select relation.relname from pg_class relation
       join pg_namespace namespace on namespace.oid=relation.relnamespace
       where namespace.nspname='erp' and relation.relkind in('r','p')
         and relation.relname not in('schema_migrations','cp6_v2620al_rollback_capsule')
     ) all_erp_tables)<>224
     or v_expected is null
     or (select count(*) from jsonb_object_keys(v_expected))<>224
     or exists(select 1 from erp.cp6_v2620al_rollback_capsule
       where boundary_snapshot is distinct from v_expected) then
    raise exception 'AL_BOUNDARY_SNAPSHOT_MISMATCH';
  end if;
  for v_table in
    select relation.relname from pg_class relation
    join pg_namespace namespace on namespace.oid=relation.relnamespace
  ...
  loop
    ...
    if v_hash is distinct from v_expected->>v_table then
      raise exception 'AL_POST_USE_ROLLBACK_REFUSED: %',v_table;
    end if;
  end loop;
  for c in select * from erp.cp6_v2620al_rollback_capsule order by object_regidentity loop
    execute c.object_definition;
  end loop;
  for r in select * from(values
    ('erp._validate_migration_batch_base(uuid)','cb72249e0a5ccc835d40c7468601a952a9387186545e34f13391bdf4a17ce0f7'),
    ('erp.post_opening_balance(uuid)','632075e17f4c59ceafe109059bb7105dca72b5b1fcc06e4ff67a9f050ee67543')
    ...
    if v_actual is distinct from r.sha256 then
      raise exception 'AL_ROLLBACK_RESTORE_HASH_MISMATCH: %',r.identity;
    end if;
  end loop;
end
$restore_guard_v2620al$;

drop table erp.cp6_v2620al_rollback_capsule;
delete from erp.schema_migrations where version='v2.6.20al';
delete from supabase_migrations.schema_migrations
where version='20260917054049'
  and name='erp_v2_6_20al_cp6_opening_value_validation'
  and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')
    in('2d16461cababd4a27827cfb8de01dd993a3a343460cdf049d427b44dd4cb6ea2');

do $postcheck_v2620al$
...
begin
  for r in select * from(values
    ...
    if v_actual is distinct from r.sha256 then
      raise exception 'AL_ROLLBACK_POSTCONDITION_FUNCTION_MISMATCH: %',r.identity;
    end if;
  end loop;
  if exists(select 1 from erp.schema_migrations where version='v2.6.20al')
     or exists(select 1 from supabase_migrations.schema_migrations where name='erp_v2_6_20al_cp6_opening_value_validation')
     ...
     or to_regclass('erp.cp6_v2620ak_rollback_capsule') is null then
    raise exception 'AL_ROLLBACK_POSTCONDITION_FAILED';
  end if;
end
$postcheck_v2620al$;
commit;
