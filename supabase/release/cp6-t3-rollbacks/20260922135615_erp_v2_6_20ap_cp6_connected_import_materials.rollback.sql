-- T3 release variant of supabase/rollbacks/20260922135615_erp_v2_6_20ap_cp6_connected_import_materials.rollback.sql (sha256 786e36224e2b607d8d2c27499c823df27026ba6d8f8e88455640998e543074f7), built by scripts/cp6_t3_rollback_acav.py from docs/evidence/cp6-t3/release_pins.json.
-- CP6 AP: exact pre-use restore to AO; closed, drained maintenance required.
begin;
...
begin
 if not exists(select 1 from erp.schema_migrations where version='v2.6.20ap')
  or not exists(select 1 from erp.schema_migrations where version='v2.6.20ao')
  ...
  or exists(select 1 from supabase_migrations.schema_migrations where version>'20260922135615') then raise exception 'AP_ROLLBACK_PLATFORM_OR_SUCCESSOR';end if;
end $platform_guard$;
do $catalog_guard$
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>7138 or fingerprint is distinct from '9db7095e0be59881f23c0422109160ea8bfdf9737662064186bd587e238de096' then
  raise exception 'AP_ROLLBACK_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
...
create temporary table cp6_package_restore_boundary on commit drop as select boundary_snapshot from erp.cp6_v2620ap_rollback_capsule limit 1;
do $pre_use$
declare v_table text;v_hash jsonb;v_after jsonb;b jsonb;
begin
 select boundary_snapshot into b from erp.cp6_v2620ap_rollback_capsule limit 1;
 v_after:='{}'::jsonb;
 ...
 if v_after is distinct from b->'after' then raise exception 'AP_POST_USE_ROLLBACK_REFUSED';end if;
 if (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from supabase_migrations.schema_migrations t where not(version='20260922135615')) is distinct from b->>'platform_before'
  or (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from erp.schema_migrations t where not(version='v2.6.20ap')) is distinct from b->>'markers_before' then raise exception 'AP_PRIOR_MIGRATION_HISTORY_DRIFT';end if;
end $pre_use$;
drop trigger initial_import_po_completion on erp.production_orders;
drop trigger initial_import_bs_disposition on erp.bs_resolutions;
drop trigger pocket_period_work_inverse on erp.work_completion_events;
drop trigger pocket_period_sewing on erp.sewing_terminal_events;
drop trigger pocket_period_stock_inverse on erp.material_stock_movements;
drop trigger pocket_period_new_source on erp.pocket_fabric_usage;
drop trigger pocket_period_recost on erp.material_adjustment_revaluation_facts;
drop trigger pocket_period_immutable on erp.pocket_period_events;
drop trigger pocket_period_immutable on erp.pocket_period_destinations;
drop trigger pocket_period_immutable on erp.pocket_period_sources;
drop trigger pocket_period_immutable on erp.pocket_periods;
drop trigger pocket_fabric_stock_inverse on erp.material_stock_movements;
drop trigger pocket_fabric_cutting_guard on erp.cutting_group_rolls;
drop trigger pocket_fabric_usage_immutable on erp.pocket_fabric_usage;
drop trigger pocket_fabric_material_immutable on erp.pocket_fabric_materials;
drop trigger initial_prepayment_mapping_guard on erp.accounting_account_mappings;
drop trigger initial_prepayment_cash_alias_guard on erp.cash_accounts;
drop trigger initial_prepayment_account_guard on erp.chart_accounts;
drop trigger initial_prepayment_payment_guard on erp.opening_subledger_settlements;
drop trigger initial_prepayment_payment_guard on erp.vendor_payments;
drop trigger initial_prepayment_payment_guard on erp.sales_payments;
drop trigger initial_prepayment_payment_guard on erp.supplier_payments;
drop trigger initial_prepayment_immutable on erp.initial_import_prepayment_events;
drop trigger initial_prepayment_immutable on erp.initial_import_prepayment_payments;
drop trigger initial_prepayment_immutable on erp.initial_import_prepayments;
drop trigger trg_opening_cash_advance_balance on erp.opening_subledger_balances;
drop trigger trg_opening_cash_advance_deduction on erp.payroll_deductions;
drop trigger initial_import_receipt_header_guard on erp.material_purchase_headers;
do $restore_functions$ declare r record;begin for r in select object_definition from erp.cp6_v2620ap_rollback_capsule order by object_regidentity loop execute r.object_definition;end loop;end $restore_functions$;
drop function erp.complete_initial_import_wip_v1(jsonb);
drop function erp.guard_initial_import_po_completion_v1();
drop function erp.sync_initial_import_bs_disposition_v1();
drop function erp.sync_initial_import_bs_value_v1(uuid,date);
drop function erp.recost_initial_import_origins_v1(uuid);
drop function erp.refresh_initial_import_fg_cost_v1(uuid,numeric,date);
drop function erp.validate_initial_import_cost_origins_v1(uuid);
drop function erp.initial_import_production_rows_v1(uuid);
drop function erp.initial_import_lot_cost_v1(uuid);
drop function erp.initial_import_source_value_v1(uuid);
drop function erp.link_initial_import_production_v1(uuid,uuid,uuid);
drop function erp.validate_initial_import_production_v1(uuid);
drop function erp.set_opening_cash_advance_payroll_v1(uuid,uuid,numeric,bigint);
drop function erp.guard_opening_cash_advance_balance_v1();
drop function erp.check_opening_cash_advance_payroll_v1(uuid);
drop function erp.guard_opening_cash_advance_deduction_v1();
drop function erp.guard_initial_import_receipt_header_v1();
drop function erp.apply_initial_import_receipts_v1(uuid);
drop function erp.link_initial_import_stock_source_v1(uuid,uuid,uuid);
drop function erp.validate_initial_import_receipts_v1(uuid);
drop function erp.check_initial_import_receipt_v1(uuid,uuid);
drop function erp.validate_initial_import_financial_sources_v1(uuid);
drop function erp.check_initial_import_financial_source_v1(uuid,uuid,uuid);
drop function erp.validate_initial_import_master_row_v1(uuid,text,jsonb);
drop function erp.validate_initial_import_totals_v1(uuid);
drop function public.erp_save_initial_import_action_v1(text,jsonb,uuid);
drop function public.erp_get_initial_import_workspace_v1(uuid);
drop function erp.save_initial_import_action_v1(text,jsonb,uuid);
drop function erp.get_initial_import_workspace_v1(uuid);
drop function erp.initial_import_revision_v1(uuid);
drop function erp.opening_cash_advance_state_v1(uuid);
drop function erp.initial_prepayment_checks_v1();
drop function erp.manage_initial_prepayment_v1(jsonb);
drop function erp.apply_initial_prepayments_v1(uuid);
drop function erp.validate_initial_prepayments_v1(uuid);
drop function erp.check_initial_prepayment_v1(uuid,uuid);
drop function erp.guard_initial_prepayment_payment_v1();
drop function erp.guard_initial_prepayment_account_v1();
drop function erp.guard_initial_prepayment_immutable_v1();
drop function erp.initial_prepayment_funding_v1(uuid,text,uuid,numeric,date,date);
drop function erp.initial_prepayment_targets_v1(uuid);
drop function erp.initial_prepayment_account_v1(uuid);
drop function erp.initial_prepayment_state_v1(uuid);
drop function erp.initial_prepayment_payments_v1(uuid);
drop function public.erp_save_pocket_fabric_action_v1(text,jsonb,uuid);
drop function public.erp_get_pocket_fabric_workspace_v1(text);
drop function erp.pocket_fabric_checks_v1();
drop function erp.save_pocket_fabric_action_v1(text,jsonb,uuid);
drop function erp.get_pocket_fabric_workspace_v1(text);
drop function erp.guard_pocket_fabric_v1();
drop function erp.pocket_fabric_roll_revision_v1(uuid);
drop function public.erp_preview_pocket_fabric_period_v1(date,date);
drop function erp.pocket_period_checks_v1();
drop function erp.guard_pocket_period_v1();
drop function erp.save_pocket_period_action_v1(text,jsonb,uuid);
drop function erp.sync_pocket_period_v1(uuid,date,text,text);
drop function erp.preview_pocket_period_v1(date,date);
drop function erp.pocket_lot_cost_v1(uuid);
drop function erp.pocket_period_book_v1(uuid);
drop function erp.pocket_period_target_v1(uuid,boolean);
drop function erp.pocket_period_state_v1(uuid);
drop function erp.pocket_period_manifest_v1(date,date);
drop function erp.pocket_period_amount_v1(uuid,bigint,bigint);
drop function erp.pocket_period_total_v1(uuid);
drop function erp.pocket_period_active_v1(uuid);
drop function erp.pocket_period_lock_v1();
drop function public.erp_save_accessory_issue_action_v1(text,jsonb,uuid);
drop function public.erp_get_accessory_issue_workspace_v1(jsonb);
drop function erp.save_accessory_issue_action_v1(text,jsonb,uuid);
drop function erp.get_accessory_issue_workspace_v1(jsonb);
drop function erp.accessory_issue_quote_v1(uuid,uuid,timestamp with time zone);
alter table erp.payroll_deductions drop constraint payroll_cash_advance_source_check;
alter table erp.payroll_deductions drop constraint payroll_deductions_deduction_type_check;
alter table erp.payroll_deductions add constraint payroll_deductions_deduction_type_check CHECK (((deduction_type)::text = ANY (ARRAY[('MATERIAL_KASBON'::character varying)::text, ('BS_COMPENSATION'::character varying)::text, ('PENALTY'::character varying)::text, ('OTHER'::character varying)::text])));
...
drop table erp.initial_import_wip_output_reversals;
drop table erp.initial_import_bs_value_events;
drop table erp.initial_import_wip_outputs;
drop table erp.initial_import_origin_cost_events;
drop table erp.initial_import_cost_origins;
drop table erp.initial_import_production_sources;
drop table erp.pocket_period_events;
drop table erp.pocket_period_destinations;
drop table erp.pocket_period_sources;
drop table erp.pocket_periods;
drop table erp.pocket_fabric_execution_context;
drop table erp.pocket_fabric_usage;
drop table erp.pocket_fabric_materials;
drop table erp.initial_import_prepayment_events;
drop table erp.initial_import_prepayment_payments;
drop table erp.initial_import_prepayments;
drop table erp.initial_import_receipt_lines;
drop table erp.initial_import_receipt_headers;
drop table erp.initial_import_opening_stock_sources;
drop table erp.initial_import_financial_sources;
drop table erp.cp6_v2620ap_rollback_capsule;
delete from erp.schema_migrations where version='v2.6.20ap';
delete from supabase_migrations.schema_migrations where version='20260922135615' and name='erp_v2_6_20ap_cp6_connected_import_materials';
do $catalog_guard$
declare actual jsonb;fingerprint text;object_count bigint;
begin
 select * into actual from (
...
select coalesce(jsonb_object_agg(k,encode(extensions.digest(convert_to(v::text,'UTF8'),'sha256'),'hex')),'{}'::jsonb) from objects
 ...
 if object_count<>6636 or fingerprint is distinct from 'd4928be2a2744508d8b7acaabd0227538cc6245d73d6f34e3134b240a50d5fa0' then
  raise exception 'AP_RESTORED_CATALOG_DRIFT';
 end if;
end $catalog_guard$;
do $restore_data$
...
begin
 ...
 if v_before is distinct from b->'before' or (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from supabase_migrations.schema_migrations t where not(false)) is distinct from b->>'platform_before'
  or (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from erp.schema_migrations t where not(false)) is distinct from b->>'markers_before' then raise exception 'AP_EXACT_RESTORE_FAILED';end if;
end $restore_data$;
commit;
