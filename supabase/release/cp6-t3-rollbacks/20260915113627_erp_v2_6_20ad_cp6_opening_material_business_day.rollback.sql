-- T3 release variant of supabase/rollbacks/20260915113627_erp_v2_6_20ad_cp6_opening_material_business_day.rollback.sql (sha256 38eb8025f07fa9a49f221fa274e56242c5c525947c3d24dc98a735d02b75512c), built by scripts/cp6_t3_rollback_acav.py from docs/evidence/cp6-t3/release_pins.json.
-- REVIEWED PRE-USE ROLLBACK: CP6 v2.6.20ad -> exact AC.
     ...
     or not exists(select 1 from supabase_migrations.schema_migrations
       where version='20260915113627'
         ...
         and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')
           in('58f8f1050e6c325339d7400a43b8c8f3d2375d2067f0c418d3eedfe85430a563'))
     or exists(select 1 from supabase_migrations.schema_migrations
       where version>'20260915113627') then
    raise exception 'AD_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR';
  end if;
end
$platform_guard_v2620ad$;
...
lock table erp.cp6_v2620ad_rollback_capsule in access exclusive mode;

do $lock_all_erp_v2620ad$
declare v_table text;
begin
  for v_table in select relation.relname from pg_class relation join pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='erp' and relation.relkind in('r','p') and relation.relname not in('schema_migrations','cp6_v2620ad_rollback_capsule') order by relation.relname loop
    execute format('lock table erp.%I in share row exclusive mode',v_table);
  end loop;
end
$lock_all_erp_v2620ad$;

do $restore_guard_v2620ad$
...
begin
  ...
  loop
    ...
    into c
    from erp.cp6_v2620ad_rollback_capsule cap
    ...
    if c.object_regidentity is null
      ...
      raise exception 'AD_TRUSTED_PREDECESSOR_PIN_MISMATCH: %',r.identity;
    end if;
  end loop;

  select boundary_snapshot into v_expected
  from erp.cp6_v2620ad_rollback_capsule limit 1;
  if (select count(*) from (select relation.relname from pg_class relation join pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='erp' and relation.relkind in('r','p') and relation.relname not in('schema_migrations','cp6_v2620ad_rollback_capsule') order by relation.relname) all_erp_tables)<>216
     or v_expected is null
     or (select count(*) from jsonb_object_keys(v_expected))<>216
     or exists(select 1 from erp.cp6_v2620ad_rollback_capsule
       where boundary_snapshot is distinct from v_expected) then
    raise exception 'AD_BOUNDARY_SNAPSHOT_MISMATCH';
  end if;
  for v_table in select relation.relname from pg_class relation join pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='erp' and relation.relkind in('r','p') and relation.relname not in('schema_migrations','cp6_v2620ad_rollback_capsule') order by relation.relname loop
    ...
    if v_hash is distinct from v_expected->>v_table then
      raise exception 'AD_POST_USE_ROLLBACK_REFUSED: %',v_table;
    end if;
  end loop;

  for c in select * from erp.cp6_v2620ad_rollback_capsule
    order by object_regidentity
  loop
    execute c.object_definition;
  end loop;
  for r in select * from(values
    ('erp.post_opening_balance(uuid)','d73391c79e725c14aab7370cf5f5dfc0bd2134a02480e5693c37dc9c25bf5829'),
    ('erp.run_v268_financial_report_checks()','416b8c53665c4c2b814d4621935a9291797fdbf49067deb0a910d16dc90685fd')
  ...
  loop
    ...
    if v_actual is distinct from r.sha256 then
      raise exception 'AD_ROLLBACK_RESTORE_HASH_MISMATCH: %',r.identity;
    end if;
  end loop;
end
$restore_guard_v2620ad$;

drop table erp.cp6_v2620ad_rollback_capsule;
delete from erp.schema_migrations where version='v2.6.20ad';
...
begin
  ...
  loop
    ...
    if v_actual is distinct from r.sha256 then
      raise exception 'AD_ROLLBACK_POSTCONDITION_FUNCTION_MISMATCH: %',r.identity;
    end if;
  end loop;
  if exists(select 1 from erp.schema_migrations where version='v2.6.20ad')
     or exists(select 1 from supabase_migrations.schema_migrations
       where name='erp_v2_6_20ad_cp6_opening_material_business_day')
     or to_regclass('erp.cp6_v2620ad_rollback_capsule') is not null
     or not exists(select 1 from erp.schema_migrations where version='v2.6.20ac')
     ...
     or to_regclass('erp.cp6_v2620y_rollback_capsule') is null
    ...
    raise exception 'AD_ROLLBACK_POSTCONDITION_FAILED';
  end if;
end
$postcheck_v2620ad$;
commit;
