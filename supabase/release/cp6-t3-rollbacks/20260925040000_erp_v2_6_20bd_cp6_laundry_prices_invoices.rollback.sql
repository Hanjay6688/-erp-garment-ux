...
do $platform$ begin
 if not exists(select 1 from erp.schema_migrations where version='v2.6.20bd')
  ...
  or not exists(select 1 from supabase_migrations.schema_migrations where version='20260925040000' and name='erp_v2_6_20bd_cp6_laundry_prices_invoices'
   and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')='84ef761ed9ea175fe8ae5f26a0ac1ad4fcaebc082b698c5409bdc8d64eb69ee0')
...
do $restore_function$ declare r record; begin
 ...
 if exists(select 1 from erp.cp6_v2620bd_rollback_capsule x join pg_proc p on p.oid=to_regprocedure(x.object_regidentity)
  ...
  or (select count(*) from erp.cp6_v2620bd_rollback_capsule x where to_regprocedure(x.object_regidentity) is not null)<>20
