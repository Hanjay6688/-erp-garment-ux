...
do $platform$ begin
 if not exists(select 1 from erp.schema_migrations where version='v2.6.20bd')
  ...
  or not exists(select 1 from supabase_migrations.schema_migrations where version='20260925040000' and name='erp_v2_6_20bd_cp6_laundry_prices_invoices'
   and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')='0935ffb9bdabad7592d93683d2474a3701bfaa82fb788dd49e0d007da1b830a2')
...
create temporary table cp6_t3_rollback_boundary on commit drop as select boundary_snapshot from erp.cp6_v2620bd_rollback_capsule limit 1;
do $pre_use$ declare v_table text;v_hash jsonb;v_after jsonb;b jsonb; begin
 ...
 if b is null or v_after is distinct from b->'after' then raise exception 'BD_POST_USE_ROLLBACK_REFUSED';end if;
 if (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from supabase_migrations.schema_migrations t where version<>'20260925040000') is distinct from b->>'platform_before'
  or (select encode(extensions.digest(convert_to(coalesce(jsonb_agg(to_jsonb(t) order by version),'[]'::jsonb)::text,'UTF8'),'sha256'),'hex') from erp.schema_migrations t where version<>'v2.6.20bd') is distinct from b->>'markers_before'
 then raise exception 'BD_PRIOR_HISTORY_DRIFT';end if;
end $pre_use$;
do $restore_function$ declare r record; begin
 for r in select object_definition from erp.cp6_v2620bd_rollback_capsule order by object_regidentity loop execute r.object_definition;end loop;
 if exists(select 1 from erp.cp6_v2620bd_rollback_capsule x join pg_proc p on p.oid=to_regprocedure(x.object_regidentity)
  ...
  or (select count(*) from erp.cp6_v2620bd_rollback_capsule x where to_regprocedure(x.object_regidentity) is not null)<>21
...
drop function erp.bd_compute_pricing_v1(jsonb,jsonb);
...
drop function erp.bd_invoice_billed_v1(uuid,text);
...
drop function erp.bd_lot_laundry_unknown_v1(uuid);
drop function erp.bd_manage_opening_claim_v1(jsonb,uuid);
drop function erp.bd_opening_billed_v1(uuid);
drop function erp.bd_opening_claim_days_v1(uuid);
drop function erp.bd_opening_claim_held_asof_v1(uuid,date);
drop function erp.bd_opening_claim_held_qty_v1(uuid);
drop function erp.bd_opening_claim_lost_qty_v1(uuid);
drop function erp.bd_opening_claim_recovered_v1(uuid);
drop function erp.bd_opening_invoiced_v1(uuid);
...
drop function erp.bd_post_invoice_v1(jsonb,uuid);
drop function erp.bd_post_priced_delivery_v1(jsonb,uuid);
drop function erp.bd_priced_line_json_v1(uuid);
drop function erp.bd_process_rate_at_v1(uuid,uuid,timestamp with time zone);
...
drop function erp.bd_set_charge_price_v1(jsonb,uuid);
...
drop table erp.bd_opening_laundry_claim_events_v1;
drop table erp.bd_opening_laundry_claims_v1;
drop table erp.bd_laundry_receipt_allocations_v1;
drop table erp.bd_laundry_size_estimates_v1;
drop table erp.bd_laundry_charge_shares_v1;
drop table erp.bd_laundry_charge_lines_v1;
drop table erp.bd_laundry_priced_lines_v1;
drop table erp.bd_requests_v1;
drop table erp.bd_laundry_scoped_rates_v1;
drop table erp.bd_laundry_package_rates_v1;
drop table erp.bd_laundry_package_components_v1;
drop table erp.bd_laundry_packages_v1;
drop table erp.bd_laundry_component_rates_v1;
drop table erp.bd_laundry_components_v1;
drop table erp.bd_laundry_vendor_terms_v1;
drop table erp.bd_execution_context_v1;
drop table erp.bd_policy_setting_events_v1;
drop table erp.bd_policy_settings_v1;
drop table erp.cp6_v2620bd_rollback_capsule;
delete from erp.schema_migrations where version='v2.6.20bd';
delete from supabase_migrations.schema_migrations where version='20260925040000' and name='erp_v2_6_20bd_cp6_laundry_prices_invoices';
do $catalog_guard$
declare actual jsonb;fingerprint text;object_count bigint;
begin
 select * into actual from (
...
do $restored_data$ declare v_table text;v_hash jsonb;v_before jsonb;b jsonb; begin
 ...
 then raise exception 'BD_EXACT_RESTORE_FAILED';end if;
end $restored_data$;
commit;
