-- T3 release variant of supabase/rollbacks/20260917033516_erp_v2_6_20ak_cp6_import_reference_preview.rollback.sql (sha256 f20722e58d8de938d2d25b1ec4c29b61f7bb38ac3a12b9633aedb22e2c7b5bba), built by scripts/cp6_t3_rollback_acav.py from docs/evidence/cp6-t3/release_pins.json.
-- REVIEWED PRE-USE ROLLBACK: CP6 v2.6.20ak -> exact AJ.
     ...
     or not exists(select 1 from supabase_migrations.schema_migrations
       where version='20260917033516' and name='erp_v2_6_20ak_cp6_import_reference_preview'
         and encode(extensions.digest(convert_to(array_to_string(statements,E'\n'),'UTF8'),'sha256'),'hex')
           in('9d06a91bd849c80e1ef99a6ac8996863527a30ef3b4079188be58bd8f3d4311d'))
...
lock table erp.cp6_v2620ak_rollback_capsule in access exclusive mode;

do $lock_all_erp_v2620ak$
declare v_table text;
begin
  for v_table in select relation.relname from pg_class relation join pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='erp' and relation.relkind in('r','p') and relation.relname not in('schema_migrations','cp6_v2620ak_rollback_capsule') order by relation.relname loop
    execute format('lock table erp.%I in share row exclusive mode',v_table);
  end loop;
end
$lock_all_erp_v2620ak$;

do $restore_guard_v2620ak$
...
begin
  if not exists(select 1 from erp.schema_migrations where version='v2.6.20aj')
     or not exists(select 1 from erp.schema_migrations where version='v2.6.20ak')
     or exists(select 1 from erp.schema_migrations
       where version not in('v2.6.20aj','v2.6.20ak')
         and installed_at>(select installed_at from erp.schema_migrations where version='v2.6.20ak'))
     or (select count(*) from erp.cp6_v2620ak_rollback_capsule)<>8 then
    raise exception 'AK_ROLLBACK_MARKER_CAPSULE_OR_SUCCESSOR';
  end if;
  for r in select * from(values
    ...
    into c from erp.cp6_v2620ak_rollback_capsule cap
    ...
    if c.object_regidentity is null
      ...
      raise exception 'AK_TRUSTED_PREDECESSOR_PIN_MISMATCH: %',r.identity;
    end if;
  end loop;
  select boundary_snapshot into v_expected from erp.cp6_v2620ak_rollback_capsule limit 1;
  if (select count(*) from (
       select relation.relname from pg_class relation
       join pg_namespace namespace on namespace.oid=relation.relnamespace
       where namespace.nspname='erp' and relation.relkind in('r','p')
         and relation.relname not in('schema_migrations','cp6_v2620ak_rollback_capsule')
     ) all_erp_tables)<>223
     or v_expected is null
     or (select count(*) from jsonb_object_keys(v_expected))<>223
     or exists(select 1 from erp.cp6_v2620ak_rollback_capsule
       where boundary_snapshot is distinct from v_expected) then
    raise exception 'AK_BOUNDARY_SNAPSHOT_MISMATCH';
  end if;
  for v_table in
    select relation.relname from pg_class relation
    join pg_namespace namespace on namespace.oid=relation.relnamespace
  ...
  loop
    ...
    if v_hash is distinct from v_expected->>v_table then
      raise exception 'AK_POST_USE_ROLLBACK_REFUSED: %',v_table;
    end if;
  end loop;
  for c in select * from erp.cp6_v2620ak_rollback_capsule order by object_regidentity loop
    execute c.object_definition;
  end loop;
  for r in select * from(values
    ('erp._validate_migration_batch_base(uuid)','eb6fc2bc6cf419a72be75891d145782dbb1dec06186243834c2a444fc1fa9801'),
    ('erp.validate_migration_opening_stock_costs(uuid)','8223a856b0a0cd66d29a9eba31ebb6644434f934d910f184947a35ae0ffa77a0'),
    ('erp.prepare_migration_opening_balance(uuid,text)','89f419571e5de43434008e5090b1d4808089411ba839a53172354a145ec97323'),
    ('erp.apply_migration_master_rows(uuid)','6fd720835a487197a9b0d4b6246d9669ba5f472c7135993ab64cd5d9970eb1c9'),
    ('erp.apply_migration_open_pos(uuid)','49dd168a2dc3df0be6fcec9c962446d91e5724f9a838dc9616d894717c9844f7'),
    ('erp.finalize_migration_batch(uuid)','b2ca642c739fda50ffe2e5fb7d165e9a3370bcc55fad2e256c358d403f1436e3'),
    ('erp.stage_migration_row(uuid,text,integer,text,jsonb,jsonb)','0a35df9e1c385ca27f2c34f17de8e22023b58e2f6c42d9788260d0ebd69cecaa'),
    ('erp.post_opening_balance(uuid)','6460ab913c9a0614df6a622ff2a223df21c13d1361e14e6089c09375f7a95b40')
    ...
    if v_actual is distinct from r.sha256 then
      raise exception 'AK_ROLLBACK_RESTORE_HASH_MISMATCH: %',r.identity;
    end if;
  end loop;
end
$restore_guard_v2620ak$;

drop table erp.cp6_v2620ak_rollback_capsule;
delete from erp.schema_migrations where version='v2.6.20ak';
...
begin
  for r in select * from(values
    ...
    if v_actual is distinct from r.sha256 then
      raise exception 'AK_ROLLBACK_POSTCONDITION_FUNCTION_MISMATCH: %',r.identity;
    end if;
  end loop;
  if exists(select 1 from erp.schema_migrations where version='v2.6.20ak')
     or exists(select 1 from supabase_migrations.schema_migrations where name='erp_v2_6_20ak_cp6_import_reference_preview')
     ...
     or to_regclass('erp.cp6_v2620aj_rollback_capsule') is null then
    raise exception 'AK_ROLLBACK_POSTCONDITION_FAILED';
  end if;
end
$postcheck_v2620ak$;
commit;
