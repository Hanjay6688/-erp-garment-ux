...
function LaundryHistory({ workspace, writerLocked, canReverse, onAction }: {
  ...
  return <section className="clq-history">
    ...
    {workspace.deliveries.length === 0 ? <div className="clq-empty"><History/><strong>Belum ada pengiriman CP6</strong><small>Data lama yang belum punya hubungan lengkap tetap dipisahkan dan tidak ditebak.</small></div> : workspace.deliveries.map((delivery) => <article key={delivery.delivery_id}>
      ...
      {delivery.receipts.map((receipt) => <div className="clq-reversal" key={receipt.id}><span><small>{receipt.number} · versi {receipt.row_version}</small><strong>{status(receipt.status)} · {formatCp6WibDateTime(receipt.physical_at)}</strong></span><input aria-label={`Alasan reversal ${receipt.number}`} value={reasons[receipt.id] ?? ''} disabled={writerLocked || !canReverse || receipt.status !== 'POSTED'} onChange={(event) => setReason(receipt.id, event.target.value)} placeholder="Alasan pembatalan · wajib"/><button disabled={writerLocked || !canReverse || receipt.status !== 'POSTED' || (reasons[receipt.id] ?? '').trim().length < 4} onClick={() => void onAction('REVERSE_RECEIPT', { receipt_id: receipt.id, reason: reasons[receipt.id].trim() }, receipt.row_version, () => setReason(receipt.id, ''))}><Undo2/> Batalkan penerimaan</button></div>)}
      <div className="clq-reversal"><span><small>GAGAL CUCI TANPA TAGIHAN</small><strong>Batalkan surat kirim, lalu buat pengiriman baru</strong></span><input aria-label={`Alasan reversal ${delivery.delivery_number}`} value={reasons[delivery.delivery_id] ?? ''} disabled={writerLocked || !canReverse || !delivery.reversible} onChange={(event) => setReason(delivery.delivery_id, event.target.value)} placeholder="Bukti seluruh barang kembali"/><button disabled={writerLocked || !canReverse || !delivery.reversible || (reasons[delivery.delivery_id] ?? '').trim().length < 4} onClick={() => void onAction('REVERSE_DELIVERY', { delivery_id: delivery.delivery_id, reason: reasons[delivery.delivery_id].trim() }, delivery.row_version, () => setReason(delivery.delivery_id, ''))}><RotateCcw/> Batalkan pengiriman</button></div>
    </article>)}
  </section>
}
