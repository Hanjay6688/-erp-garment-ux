...
export default function ConnectedPickupPage() {
  ...
  const fetchQueue = useCallback(async (nextFilter: PickupFilter, nextPattern: string, nextQuery: string) => {
    setLoading(true)
    setError('')
  ...
  useEffect(() => {
    ...
    setContractorId(selected.assigned_contractor_id ?? selected.pickup?.contractor_id ?? queue?.contractors[0]?.id ?? '')
  ...
  }, [contractorSignature, selectedGroupId, selectedGroupVersion, selectedPickupVersion]) // eslint-disable-line react-hooks/exhaustive-deps

  const changeFilter = async (next: PickupFilter) => {
    setFilter(next)
    setSelectedId('')
    await fetchQueue(next, patternId, query)
  }
  const changePattern = async (next: string) => {
    setPatternId(next)
    setSelectedId('')
    await fetchQueue(filter, next, query)
  }
  const refresh = () => fetchQueue(filter, patternId, query)
  ...
  const draftValid = Boolean(selected && editable && contractorId && pickedAtValid && !overAllocated && batchCount >= 1)
  ...
  const save = async (action: 'SAVE_DRAFT' | 'POST') => {
    if (saving || !mutationAllowed || (action === 'POST' ? !postValid || !canPost : !draftValid)) return
    setSaving(true)
    setError('')
    setNotice('')
    try {
  ...
  const removeDraft = async () => {
    if (!selected || !pickupId || pickupVersion === null || saving || !canEdit) return
    ...
    <div className="cpick-layout">
      ...
      <main className="cpick-workspace">
        {!selected ? <div className="cpick-empty large">Pilih Potongan dari antrean.</div> : <>
          ...
          <section className="cpick-setup"><label>Mandor<select value={contractorId} disabled={!editable || Boolean(selected.assigned_contractor_id)} onChange={(event) => setContractorId(event.target.value)}><option value="">Pilih Mandor…</option>{queue?.contractors.map((contractor) => <option value={contractor.id} key={contractor.id}>{contractor.code} · {contractor.name}</option>)}</select><small>{selected.assigned_contractor_id ? `Terkunci mengikuti PO · ${selected.assigned_contractor_name ?? 'Mandor terpasang'}` : 'Akan menjadi Mandor kanonik PO saat pickup diposting.'}</small></label><label>Waktu fisik diambil<input type="datetime-local" value={pickedUpAt} disabled={!editable} onChange={(event) => setPickedUpAt(event.target.value)}/></label><label>Jumlah batch<div><button type="button" disabled={!editable || batchCount <= 1} onClick={() => resizeBatches(batchCount - 1)}>−</button><strong>{batchCount}</strong><button type="button" disabled={!editable || batchCount >= 12} onClick={() => resizeBatches(batchCount + 1)}>+</button></div></label><fieldset disabled={!editable}><legend>Susun awal</legend><button type="button" className={mode === 'ROLL' ? 'active' : ''} onClick={() => changeMode('ROLL')}>Per roll</button><button type="button" className={mode === 'SIZE' ? 'active' : ''} onClick={() => changeMode('SIZE')}>Per size</button></fieldset></section>
