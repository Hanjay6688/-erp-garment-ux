...
function QcHistory({ workspace, writerLocked, canReverse, onAction }: {
  ...
  return <section className="clq-history">
    ...
    {workspace.qc_history.length === 0 ? <div className="clq-empty"><History/><strong>Belum ada finalisasi CP6</strong><small>QC lama yang belum punya hubungan lengkap tetap dipisahkan dan tidak ditebak.</small></div> : workspace.qc_history.map((row) => <article key={row.qc_inspection_id}><header><div><small>{formatCp6WibDateTime(row.physical_at)} · versi {row.row_version}</small><strong>{row.inspection_number} · {row.po_number}</strong><span>{row.location_name} · Good {row.good_qty_pcs} · BS {row.bs_qty_pcs}</span></div><em>{status(row.status)}</em></header><div className="clq-reversal"><span><small>KOREKSI TRANSAKSI TERSIMPAN</small><strong>Membalik stok, reimbursement, HPP, jurnal, dan laporan sebagai riwayat baru</strong></span><input aria-label={`Alasan reversal ${row.inspection_number}`} value={reasons[row.qc_inspection_id] ?? ''} disabled={writerLocked || !canReverse || row.status !== 'POSTED'} onChange={(event) => setReasons((current) => ({ ...current, [row.qc_inspection_id]: event.target.value }))} placeholder="Alasan pembatalan · wajib"/><button disabled={writerLocked || !canReverse || row.status !== 'POSTED' || (reasons[row.qc_inspection_id] ?? '').trim().length < 4} onClick={() => void onAction('REVERSE_FINAL_SKU', { qc_inspection_id: row.qc_inspection_id, reason: reasons[row.qc_inspection_id].trim() }, row.row_version, () => setReasons((current) => ({ ...current, [row.qc_inspection_id]: '' }))) }><Undo2/> Batalkan finalisasi</button></div></article>)}
  </section>
}
