...
function invoiceStatusLabel(status:string){
  if(status==='WAITING_ALLOCATION')return 'MENUNGGU ALOKASI'
  return status
}

function InvoiceLifecycle({compact=false}:{compact?:boolean}){
  return <section className={`panel biz-invoice-lifecycle ${compact?'compact':''}`} aria-label="Lifecycle invoice"><article className="active"><i>01</i><span><small>DRAFT</small><strong>Bebas diedit</strong><em>On hand dan available belum berubah.</em></span></article><ArrowRight/><article><i>02</i><span><small>ALOKASI / STOCK COMMIT</small><strong>FIFO lalu stock-out sekali</strong><em>WAITING_ALLOCATION belum mengurangi stok.</em></span></article><ArrowRight/><article><i>03</i><span><small>POSTED</small><strong>Terkunci & terbentuk AR</strong><em>Tidak ada stock-out kedua saat posting.</em></span></article></section>
}
  ...
  return <>
    ...
    <section className="biz-sales-builder">
      ...
      <aside className="panel biz-review-card">
        ...
        <section><p><Check/> Draft tidak memotong On Hand / Available</p><p><Check/> FIFO + stock-out terjadi satu kali saat commit</p><p><Check/> Posted hanya mengunci dan membentuk AR/COGS</p><p><Check/> Koreksi posted lewat retur atau reversal</p></section>
    ...
    <section className="panel biz-history-block"><header><div><span>INVOICE TERBARU</span><strong>Draft, menunggu alokasi, dan posted</strong></div><History/></header><div className="biz-table"><div className="biz-table-head"><span>Nomor / waktu</span><span>Pelanggan</span><span>SKU / qty</span><span>Nilai</span><span>Status</span></div>{invoiceHistory.map((item)=><article key={item.number}><span><strong>{item.number}</strong><small>{item.day} · {item.time}</small></span><strong>{item.customer}</strong><span>{item.sku}<small>{item.qty} pcs · {qtyLabel(item.qty)}</small></span><b>{money(item.gross)}</b><Pill tone={statusTone(item.status)}>{invoiceStatusLabel(item.status)}</Pill></article>)}</div></section>

    {reviewOpen&&<ReviewModal title="Simpan sebagai Draft" description="Draft boleh diedit lagi dan belum membuat mutasi stok, AR, revenue, atau COGS." onClose={()=>setReviewOpen(false)} actionLabel="Simpan draft simulasi" action={()=>{setReviewOpen(false);setNotice('Draft invoice tersimpan di layar. Stok, AR, revenue, dan COGS belum berubah.')}}><div className="biz-impact-grid"><article><span>Stock On Hand</span><strong>0 pcs</strong><small>Tidak ada stock ledger</small></article><article><span>Available stock</span><strong>Tetap</strong><small>Belum ada allocation FIFO</small></article><article><span>AR / COGS</span><strong>Belum terbentuk</strong><small>Baru saat dokumen committed / posted</small></article></div><div className="biz-guard"><ShieldCheck/><span><strong>Jawaban singkat: Draft tidak minus stok.</strong> Stock-out dibuat tepat satu kali setelah allocation berhasil. Perubahan status menjadi Posted tidak boleh membuat stock-out kedua.</span></div></ReviewModal>}
...
function AllocationWorkspace({ onNavigate }: { onNavigate: SalesPageProps['onNavigate'] }) {
  ...
  return <>
    ...
    {selectedInvoice&&<ReviewModal title={selectedInvoice.number} description={`${selectedInvoice.customer} · ${selectedInvoice.day} ${selectedInvoice.time}`} onClose={()=>setSelectedInvoice(null)} actionLabel={selectedInvoice.status==='DRAFT'?'Buka draft':selectedInvoice.status==='WAITING_ALLOCATION'?'Lihat status alokasi':'Buat draft koreksi'} disabled={['POSTED','PAID'].includes(selectedInvoice.status)&&selectedInvoice.paid>0} action={()=>{if(selectedInvoice.status==='DRAFT'){setSelectedInvoice(null);onNavigate('sales-invoice');return}if(selectedInvoice.status==='WAITING_ALLOCATION'){setNotice(`${selectedInvoice.number} masih menunggu FIFO; belum ada stock-out.`);setSelectedInvoice(null);return}setNotice(`Draft reversal + invoice pengganti untuk ${selectedInvoice.number} dibuat sebagai simulasi. Dokumen posted asal tetap utuh.`);setSelectedInvoice(null)}}><div className="biz-invoice-detail-summary"><article><span>STATUS</span><strong><Pill tone={statusTone(selectedInvoice.status)}>{invoiceStatusLabel(selectedInvoice.status)}</Pill></strong><small>{['POSTED','PAID'].includes(selectedInvoice.status)?'Read-only':'Masih dapat diproses'}</small></article><article><span>QTY / SKU</span><strong>{selectedInvoice.qty} pcs</strong><small>{selectedInvoice.sku}</small></article><article><span>NET INVOICE</span><strong>{money(selectedInvoice.gross-selectedInvoice.returns)}</strong><small>Dibayar {money(selectedInvoice.paid)}</small></article><article><span>DAMPAK STOK</span><strong>{selectedInvoice.status==='DRAFT'||selectedInvoice.status==='WAITING_ALLOCATION'?'0 pcs':`− ${selectedInvoice.qty} pcs`}</strong><small>{selectedInvoice.status==='DRAFT'?'Draft belum menyentuh ledger':selectedInvoice.status==='WAITING_ALLOCATION'?'FIFO belum committed':'Sudah dicatat sekali saat commit'}</small></article></div><div className="biz-invoice-event-log"><header><span>DOCUMENT HISTORY</span><strong>Jejak status dan stock event</strong></header><article><Clock3/><span><strong>Draft dibuat</strong><small>{selectedInvoice.day} · {selectedInvoice.time} · stok tidak berubah</small></span></article>{selectedInvoice.status!=='DRAFT'&&<article><PackageCheck/><span><strong>{selectedInvoice.status==='WAITING_ALLOCATION'?'Menunggu allocation FIFO':'Allocation & stock-out committed'}</strong><small>{selectedInvoice.status==='WAITING_ALLOCATION'?'Belum ada stock ledger':'Satu stock event · per SKU, size, dan lot'}</small></span></article>}{['POSTED','PAID'].includes(selectedInvoice.status)&&<article><ShieldCheck/><span><strong>Invoice Posted · dokumen dikunci</strong><small>AR, revenue, COGS terbentuk tanpa stock-out kedua</small></span></article>}</div>{['POSTED','PAID'].includes(selectedInvoice.status)&&<div className="biz-invoice-correction-actions"><button type="button" className="soft-btn" onClick={()=>{setSelectedInvoice(null);onNavigate('sales-returns')}}><Undo2/> Retur fisik</button><span><strong>Edit langsung dinonaktifkan.</strong><small>Koreksi nominal/qty memakai reversal + draft pengganti. Retur fisik memakai dokumen retur.</small></span></div>}{selectedInvoice.paid>0&&['POSTED','PAID'].includes(selectedInvoice.status)&&<div className="biz-guard warn"><AlertTriangle/><span><strong>Reversal diblokir karena sudah ada pembayaran {money(selectedInvoice.paid)}.</strong><small>Void/reverse payment terkait lebih dulu atau gunakan retur/koreksi yang tidak merusak settlement.</small></span></div>}</ReviewModal>}
  </>
}
