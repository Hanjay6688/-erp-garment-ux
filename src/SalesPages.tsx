...
function InvoiceLifecycle({compact=false}:{compact?:boolean}){
  return <section className={`panel biz-invoice-lifecycle ${compact?'compact':''}`} aria-label="Lifecycle invoice"><article className="active"><i>01</i><span><small>DRAFT + AUTO RESERVE</small><strong>Cek stok saat barang dipasang</strong><em>On hand tetap; available langsung berkurang.</em></span></article><ArrowRight/><article><i>02</i><span><small>REVIEW POST</small><strong>FIFO sudah terlihat</strong><em>Kurang stok memblokir Post, bukan masuk antrean alokasi.</em></span></article><ArrowRight/><article><i>03</i><span><small>POSTED</small><strong>Stock-out sekali & invoice terkunci</strong><em>AR, revenue, COGS, dan stok committed atomically.</em></span></article></section>
}
  ...
  return <>
    ...
    <section className="biz-sales-builder">
      ...
      <aside className="panel biz-review-card">
        ...
        <section><p><Check/> SKU/size langsung dicek terhadap available</p><p><Check/> Draft auto-reserve; On Hand belum berkurang</p><p><Check/> Post membuat stock-out, AR, revenue, dan COGS sekali</p><p><Check/> Koreksi posted lewat retur atau reversal</p></section>
    ...
    {reviewOpen&&<ReviewModal title="Simpan Draft & reserve stok" description="Draft tetap dapat diedit. Sistem menyimpan allocation FIFO sebagai reservation; stok fisik belum keluar." onClose={()=>setReviewOpen(false)} actionLabel="Simpan draft + reserve" action={()=>{setReviewOpen(false);setNotice(`Draft tersimpan dan ${totalQty} pcs di-reserve otomatis. On Hand tetap; Available berkurang. Stock-out baru saat Post.`)}}><div className="biz-impact-grid"><article><span>Stock On Hand</span><strong>Tetap</strong><small>Belum ada stock ledger keluar</small></article><article><span>Available stock</span><strong>− {totalQty} pcs</strong><small>Reservation per SKU, size, dan lot FIFO</small></article><article><span>AR / COGS</span><strong>Belum terbentuk</strong><small>Dibentuk bersama stock-out saat Post</small></article></div><div className="biz-guard"><ShieldCheck/><span><strong>Draft hanya mengunci ketersediaan.</strong> Edit qty menyesuaikan reservation; hapus Draft melepaskannya. Post mengubah reservation menjadi stock-out tepat satu kali.</span></div></ReviewModal>}
...
function AllocationWorkspace({ onNavigate }: { onNavigate: SalesPageProps['onNavigate'] }) {
  ...
  return <>
    ...
    {selectedInvoice&&<ReviewModal
      ...
      description={selectedInvoice.status==='DRAFT'?`${selectedInvoice.customer} · reservation FIFO siap diubah menjadi stock-out`:`${selectedInvoice.customer} · ${selectedInvoice.day} ${selectedInvoice.time}`}
      ...
      action={()=>{
        if(selectedInvoice.status==='DRAFT'){
          ...
          setNotice(`${invoiceNumber} dipost: reservation menjadi stock-out ${invoiceQty} pcs tepat satu kali; AR, revenue, dan COGS ikut terbentuk.`)
    ...
    >
      <div className="biz-invoice-detail-summary">
        ...
        <article><span>ON HAND / AVAILABLE</span><strong>{selectedInvoice.status==='DRAFT'?`Tetap / − ${selectedInvoice.qty}`:`− ${selectedInvoice.qty} pcs`}</strong><small>{selectedInvoice.status==='DRAFT'?'Reserved otomatis; belum stock-out':'Stock-out sudah dicatat satu kali'}</small></article>
      </div>
      <div className="biz-invoice-event-log">
        ...
        <article><Clock3/><span><strong>Draft dibuat</strong><small>{selectedInvoice.day} · {selectedInvoice.time} · On Hand tetap</small></span></article>
        <article><Boxes/><span><strong>FIFO di-reserve otomatis</strong><small>Available − {selectedInvoice.qty} pcs · per SKU, size, dan lot</small></span></article>
        {['POSTED','PAID'].includes(selectedInvoice.status)&&<article><PackageCheck/><span><strong>Post + stock-out committed</strong><small>Satu stock event; reservation dilepas menjadi pengeluaran fisik</small></span></article>}
      ...
      {selectedInvoice.status==='DRAFT'&&<div className="biz-invoice-correction-actions"><button type="button" className="soft-btn" onClick={()=>{setSelectedInvoice(null);onNavigate('sales-invoice')}}><ReceiptText/> Buka & edit Draft</button><span><strong>Tidak ada status Waiting Allocation.</strong><small>Edit qty menyesuaikan reservation; hapus Draft melepaskan reservation.</small></span></div>}
