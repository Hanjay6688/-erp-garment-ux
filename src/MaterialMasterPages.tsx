...
type AccessoryMaster={id:string;code:string;name:string;category:string;stockUom:string;purchaseUom:string;conversion:number;movingAverage:number;stock:number;active:boolean;rowVersion:number}
...
const accessorySeeds:AccessoryMaster[]=[
  {id:'acc-btn-silver',code:'ACC-BTN-017-SLV',name:'Kancing Jeans 17 mm · Silver',category:'Kancing',stockUom:'pcs',purchaseUom:'gross',conversion:144,movingAverage:620,stock:4800,active:true,rowVersion:7},
  {id:'acc-btn-metal',code:'ACC-BTN-017-MTL',name:'Kancing Jeans 17 mm · Metal',category:'Kancing',stockUom:'pcs',purchaseUom:'gross',conversion:144,movingAverage:690,stock:3200,active:true,rowVersion:5},
  {id:'acc-zip-14',code:'ACC-ZIP-014-BLK',name:'Resleting 14 cm · Black',category:'Resleting',stockUom:'pcs',purchaseUom:'lusin',conversion:12,movingAverage:5200,stock:2200,active:true,rowVersion:9},
  {id:'acc-label-vivo',code:'ACC-LBL-VIVO',name:'Label Woven Vivo',category:'Label',stockUom:'pcs',purchaseUom:'roll',conversion:1000,movingAverage:530,stock:8000,active:true,rowVersion:3},
]
...
function AccessoryWorkspace(){
  ...
  const [priceKind,setPriceKind]=useState<'SELLING'|'REIMBURSEMENT'>('SELLING')
  const [historyKind,setHistoryKind]=useState<'SELLING'|'REIMBURSEMENT'>('SELLING')
  const [editMode,setEditMode]=useState<'create'|'edit'|null>(null)
  ...
  const displayedHistory=historyKind==='SELLING'?selectedRate.sellingPrices:selectedRate.reimbursementPrices
  const openPrice=(kind:'SELLING'|'REIMBURSEMENT')=>{
    const entry=kind==='SELLING'?sellingEntry:reimbursementEntry
  ...
  const savePrice=()=>{
    ...
    setRates((current)=>current.map((rate)=>rate.category===selected.category?{...rate,[key]:[{id:`${priceKind.toLowerCase()}-${Date.now()}`,amount,uom:priceUom,effectiveFrom:effective,note:note.trim(),status:'CURRENT'},...rate[key].map((row)=>row.status==='CURRENT'?{...row,status:'SUPERSEDED' as const,effectiveTo:effective}:row)]}:rate))
    setPriceOpen(false);setHistoryKind(priceKind);setNotice(`${priceKind==='SELLING'?'Harga jual ke mandor':'Tarif reimburse'} kategori ${selected.category} ${money(amount)}/${priceUom} tersimpan sebagai simulasi. HPP ${selected.code} tetap mengikuti moving average pembelian.`)
  }
  ...
  const saveAccessory=()=>{
    ...
    if(editMode==='create'){
      const next:AccessoryMaster={id:`acc-sim-${Date.now()}`,code:editCode.trim(),name:editName.trim(),category:editCategory,stockUom:editStockUom.trim(),purchaseUom:editPurchaseUom.trim(),conversion,movingAverage:0,stock:0,active:editActive,rowVersion:1}
  ...
  const saveCategory=()=>{
    ...
    setCategory(value);setCategoryOpen(false);setNewCategory('');setNotice(`Kategori ${value} dibuat sebagai simulasi. Tetapkan dua harga kategori dan tambahkan barang detail sebelum dipakai pada BOM.`)
  }
  ...
  return <>
    <MasterHero kind="AKSESORI" title="Aksesori, UOM & Harga Mandor" description="HPP tetap milik SKU detail dari moving average pembelian. Harga jual ke mandor dan reimburse adalah dua rate kategori dengan nominal serta UOM independen."/>
    {tabs}
    ...
    <section className="panel mm-layout"><aside><header><div><span>BROWSER DETAIL AKSESORI</span><strong>{visible.length} barang</strong></div><Package/></header><label className="mm-search"><Search/><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Cari kode, nama, kategori..."/></label><div>{visible.map((item)=><button className={item.id===selected.id?'active':''} key={item.id} onClick={()=>{setSelectedId(item.id);setNotice('')}}><i><Package/></i><span><strong>{item.name}</strong><small>{item.code} · {item.category}</small><em>{item.stock.toLocaleString('id-ID')} {item.stockUom} On Hand</em></span><StatePill active={item.active}/></button>)}</div><footer><button onClick={()=>openAccessoryEditor('create')}><Plus/> Tambah detail</button><small>Kategori dan detail dipisah; hard delete tidak dipakai.</small></footer></aside><main>
      ...
      <div className="mm-rule"><ShieldCheck/><span><strong>Varian fisik tetap jelas; rate tidak dicampur dengan HPP.</strong><small>Nota Ambil mencatat SKU detail yang diambil. Kasbon memakai harga jual kategori; reimburse memakai Good FG × BOM kategori; valuasi persediaan tetap dari moving average harga beli SKU detail.</small></span></div>
      <section className="mm-history"><header><div><span>PRICE HISTORY · KATEGORI {selected.category.toUpperCase()}</span><strong>{historyKind==='SELLING'?'Harga jual ke mandor':'Tarif reimburse'} berdasarkan waktu</strong></div><nav className="mm-history-tabs"><button className={historyKind==='SELLING'?'active':''} onClick={()=>setHistoryKind('SELLING')}>Jual ke mandor</button><button className={historyKind==='REIMBURSEMENT'?'active':''} onClick={()=>setHistoryKind('REIMBURSEMENT')}>Reimburse</button></nav></header><div className="mm-history-head"><span>Effective</span><span>Harga / UOM</span><span>Catatan</span><span>Status</span></div>{displayedHistory.map((item)=><article key={item.id}><span><strong>{item.effectiveFrom}</strong><small>{item.effectiveTo?`s.d. ${item.effectiveTo}`:'Tanpa akhir'}</small></span><b>{money(item.amount)} / {item.uom??selected.stockUom}</b><span>{item.note}</span><em className={item.status.toLowerCase()}>{item.status}</em></article>)}{displayedHistory.length===0&&<div className="mm-history-empty">Belum ada versi harga untuk kategori ini.</div>}</section>
      {notice&&<div className="mm-notice"><CheckCircle2/> {notice}</div>}
    ...
    {priceOpen&&<MasterModal title={priceKind==='SELLING'?'Set harga jual ke mandor':'Set tarif reimburse mandor'} description={`${selected.category} · ditampilkan pada SKU ${selected.code}`} onClose={()=>setPriceOpen(false)} action={savePrice} disabled={Number(price.replace(/\D/g,''))<=0||!priceUom||!effective||note.trim().length<4} actionLabel="Simpan versi simulasi"><div className="mm-form-grid three"><label><span>NOMINAL</span><div><b>Rp</b><input autoFocus inputMode="numeric" value={formatMoneyInput(price)} onChange={(event)=>setPrice(cleanMoneyInput(event.target.value))}/></div></label><label><span>UOM HARGA</span><select value={priceUom} onChange={(event)=>setPriceUom(event.target.value)}><option value="pcs">PCS</option><option value="lusin">LUSIN</option><option value="gross">GROSS</option><option value="pack">PACK</option><option value="box">BOX</option></select></label><label><span>BERLAKU MULAI</span><input type="date" value={effective} onChange={(event)=>setEffective(event.target.value)}/></label></div><label className="mm-modal-note"><span>CATATAN SINGKAT</span><textarea value={note} onChange={(event)=>setNote(event.target.value)} placeholder="Alasan dan sumber perubahan rate..."/></label><div className="mm-modal-impact"><Coins/><span><strong>Nominal dan UOM disimpan bersama sebagai snapshot</strong><small>Contoh resleting: jual Rp29.900/lusin dan reimburse Rp2.500/pcs. Nota posted tetap membaca versi lama; HPP SKU tidak ikut berubah.</small></span></div></MasterModal>}
