...
const masterConfigs:Record<BusinessMasterView,MasterConfig>={
  ...
  'master-workforce':{view:'master-workforce',eyebrow:'MANDOR & PEKERJA',title:'Mandor & Pekerja',description:'Kelola identitas, aturan absensi, dan flag Special penerima WIP/payroll tanpa hardcode nama Mandor atau menghilangkan histori pembayaran.',browserLabel:'BROWSER TENAGA KERJA',singular:'tenaga kerja',icon:UsersRound,writerStatus:'Read-ready · safe writer tenaga kerja belum lengkap',integrityTitle:'Mandor dengan WIP atau payroll tidak boleh dihapus.',integrityCopy:'Flag Special memilih komisi Special SKU dan harga Special aksesori pada transaksi baru. Snapshot batch dan Nota FG lama tidak dihitung ulang.',emptyDependencyCopy:'Tenaga kerja baru belum memiliki WIP, payroll, atau kasbon.',fields:[{key:'role',label:'Peran',type:'select',options:['Mandor','Pekerja Jahit','Pekerja Finishing','QC']},{key:'special',label:'Mandor Special',type:'boolean'},{key:'phone',label:'Telepon',type:'text'},{key:'bank',label:'Rekening / cara bayar',type:'text'},{key:'attendance',label:'Kebijakan absensi',type:'select',options:['Tidak wajib','Wajib']},{key:'teamSize',label:'Jumlah anggota tim',type:'number',suffix:'orang'}],seeds:workforceSeeds,metrics:(items)=>commonMetrics(items,{label:'MANDOR SPECIAL',value:String(items.filter((item)=>item.values.role==='Mandor'&&item.values.special==='true').length),note:'Dibaca dari checkbox, bukan nama',tone:'warn'})},
