...
type FieldType='text'|'number'|'select'|'sizes'|'date'
...
type CategoryRate={category:string;uom:string;mandorCharge:number;reimbursement:number;effectiveFrom:string;version:number}
type MasterEntity={
...
const categoryRateSeeds:CategoryRate[]=[
  {category:'Kancing',uom:'pcs',mandorCharge:495,reimbursement:500,effectiveFrom:'2026-08-01',version:4},
  {category:'Resleting',uom:'pcs',mandorCharge:6000,reimbursement:6200,effectiveFrom:'2026-08-01',version:3},
  {category:'Label',uom:'pcs',mandorCharge:750,reimbursement:800,effectiveFrom:'2026-08-01',version:5},
]
...
function ProductCostPanel({entity,categoryRates,onEditBom,onEditCategories}:{entity:MasterEntity;categoryRates:CategoryRate[];onEditBom:()=>void;onEditCategories:()=>void}){
  ...
  const reimbursement=(entity.bom??[]).reduce((total,line)=>total+line.quantity*(categoryRates.find((rate)=>rate.category===line.category)?.reimbursement??0),0)
  return <section className="md-product-cost"><header><div><span>MASTER TARIF SKU</span><strong>Snapshot biaya kerja & BOM kategori</strong></div><Calculator/></header><div className="md-rate-cards"><article><small>HARGA JUAL / PCS</small><strong>{idr(entity.values.sellingPrice)}</strong><em>Invoice baru</em></article><article><small>UPAH JAHIT / PCS</small><strong>{idr(entity.values.sewingRate)}</strong><em>Tarif kerja</em></article><article><small>KOMISI / PCS</small><strong>{idr(entity.values.commissionRate)}</strong><em>Tarif mandor</em></article><article className="total"><small>TOTAL KERJA / PCS</small><strong>{idr(String(sewing+commission))}</strong><em>Jahit + komisi</em></article></div><div className="md-bom-head"><div><span>MASTER BOM PER SKU</span><strong>{entity.bom?.length??0} kategori · reimburse standar {idr(String(reimbursement))} / Good FG</strong></div><div><button type="button" className="soft-btn" onClick={onEditCategories}><Coins/> Tarif kategori</button><button type="button" className="soft-btn" onClick={onEditBom}><Pencil/> Edit BOM SKU</button></div></div><div className="md-bom-table"><div className="head"><span>KATEGORI BOM</span><span>QTY / GOOD FG</span><span>HARGA TAGIH MANDOR</span><span>TARIF REIMBURSE</span><span>BUFFER</span></div>{(entity.bom??[]).map((line)=>{const rate=categoryRates.find((item)=>item.category===line.category);const buffer=(rate?.reimbursement??0)-(rate?.mandorCharge??0);return <article key={line.id}><span><strong>{line.category}</strong><small>Stok detail & HPP tetap terpisah</small></span><b>{line.quantity} {line.uom}</b><span><strong>{idr(String(rate?.mandorCharge??0))}/{line.uom}</strong><small>Semua qty aktual yang diambil</small></span><span><strong>{idr(String(rate?.reimbursement??0))}/{line.uom}</strong><small>Good FG × BOM</small></span><em className={buffer>=0?'positive':'negative'}>{buffer>=0?'+':''}{idr(String(buffer))}</em></article>})}</div><div className="md-cost-rule"><ShieldCheck/><span><strong>Kancing: tagih mandor Rp495, reimburse Rp500.</strong><small>Nominal boleh berbeda untuk buffer defect pabrik. BOM kategori tidak memindahkan stok; HPP tetap memakai moving average barang detail, dan pemakaian berlebih tidak menambah reimburse.</small></span></div></section>
}
...
function CategoryRateModal({rates,onClose,onSave}:{rates:CategoryRate[];onClose:()=>void;onSave:(rates:CategoryRate[])=>void}){
  ...
  const update=(category:string,key:'mandorCharge'|'reimbursement'|'effectiveFrom',value:string)=>setDrafts((current)=>current.map((item)=>item.category===category?{...item,[key]:value}:item))
  ...
  return <div className="md-backdrop" role="presentation"><section className="md-modal wide" role="dialog" aria-modal="true" aria-label="Tarif kategori aksesori"><ModalHeader eyebrow="TARIF AKSESORI PER KATEGORI" title="Tagihan mandor & reimburse" copy="Dua nominal independen; HPP pembelian dan moving average tidak ikut berubah." onClose={onClose}/><div className="md-category-editor"><div className="head"><span>KATEGORI</span><span>HARGA TAGIH MANDOR</span><span>TARIF REIMBURSE</span><span>BERLAKU MULAI</span></div>{drafts.map((item)=><article key={item.category}><span><strong>{item.category}</strong><small>v{item.version} · per {item.uom}</small></span><MoneyEditorInput value={item.mandorCharge} onChange={(value)=>update(item.category,'mandorCharge',value)}/><MoneyEditorInput value={item.reimbursement} onChange={(value)=>update(item.category,'reimbursement',value)}/><input type="date" value={item.effectiveFrom} onChange={(event)=>update(item.category,'effectiveFrom',event.target.value)}/></article>)}</div><label className="md-modal-note"><span>CATATAN SINGKAT</span><textarea value={note} onChange={(event)=>setNote(event.target.value)}/></label><div className="md-editor-warning"><Coins/><span><strong>Contoh Kancing: Rp495 vs Rp500</strong><small>Rp495 menilai kasbon dari kuantitas aktual yang diambil. Rp500 membayar standar BOM dari Good FG untuk memberi buffer defect pabrik.</small></span></div><footer><button type="button" className="soft-btn" onClick={onClose}>Batal</button><button type="button" className="primary-btn" disabled={!valid} onClick={()=>onSave(drafts.map((item)=>({...item,mandorCharge:Number(item.mandorCharge),reimbursement:Number(item.reimbursement),version:item.version+1})))}>Simpan versi tarif</button></footer></section></div>
}

type BulkRateKey='sellingPrice'|'sewingRate'|'commissionRate'
const bulkRateFields:[BulkRateKey,string][]=[['sellingPrice','Harga jual'],['sewingRate','Upah jahit'],['commissionRate','Komisi']]
...
function BulkRateModal({entities,selectedIds,onClose,onApply}:{entities:MasterEntity[];selectedIds:string[];onClose:()=>void;onApply:(values:Partial<Record<BulkRateKey,string>>,effectiveFrom:string,note:string)=>void}){
  ...
  const [enabled,setEnabled]=useState<Record<BulkRateKey,boolean>>({sellingPrice:true,sewingRate:false,commissionRate:false})
  const [values,setValues]=useState<Record<BulkRateKey,string>>({sellingPrice:selected[0]?.values.sellingPrice??'',sewingRate:selected[0]?.values.sewingRate??'',commissionRate:selected[0]?.values.commissionRate??''})
