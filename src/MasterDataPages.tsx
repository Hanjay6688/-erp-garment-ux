...
type CategoryRate={category:string;chargeUom:string;reimbursementUom:string;mandorCharge:number;reimbursement:number;effectiveFrom:string;version:number;configured:boolean}
type MasterEntity={
...
const categoryRateSeeds:CategoryRate[]=[
  {category:'Kancing',chargeUom:'pcs',reimbursementUom:'pcs',mandorCharge:495,reimbursement:500,effectiveFrom:'2026-08-01',version:4,configured:true},
  {category:'Centang',chargeUom:'pcs',reimbursementUom:'pcs',mandorCharge:200,reimbursement:200,effectiveFrom:'2026-08-01',version:1,configured:true},
  {category:'Kulit',chargeUom:'pcs',reimbursementUom:'pcs',mandorCharge:1_000,reimbursement:1_000,effectiveFrom:'2026-08-01',version:1,configured:true},
  {category:'Sleting',chargeUom:'lusin',reimbursementUom:'pcs',mandorCharge:29_900,reimbursement:2_500,effectiveFrom:'2026-08-01',version:4,configured:true},
  {category:'Plat',chargeUom:'pcs',reimbursementUom:'pcs',mandorCharge:500,reimbursement:500,effectiveFrom:'2026-08-01',version:1,configured:true},
  {category:'Hang Tag',chargeUom:'lusin',reimbursementUom:'pcs',mandorCharge:7_150,reimbursement:600,effectiveFrom:'2026-08-01',version:1,configured:true},
  {category:'Lock Pin',chargeUom:'pcs',reimbursementUom:'pcs',mandorCharge:300,reimbursement:300,effectiveFrom:'2026-08-01',version:1,configured:true},
  {category:'Kain Kantong',chargeUom:'pcs',reimbursementUom:'pcs',mandorCharge:0,reimbursement:0,effectiveFrom:'',version:0,configured:false},
  {category:'Label',chargeUom:'pcs',reimbursementUom:'pcs',mandorCharge:0,reimbursement:0,effectiveFrom:'',version:0,configured:false},
  {category:'Kain Keras',chargeUom:'pcs',reimbursementUom:'pcs',mandorCharge:0,reimbursement:0,effectiveFrom:'',version:0,configured:false},
]

const defaultBom=(prefix:string):BomLine[]=>[
  {id:`${prefix}-button`,category:'Kancing',quantity:1,uom:'pcs'},
  {id:`${prefix}-zipper`,category:'Sleting',quantity:1,uom:'pcs'},
  {id:`${prefix}-hang-tag`,category:'Hang Tag',quantity:1,uom:'pcs'},
]
...
const masterConfigs:Record<BusinessMasterView,MasterConfig>={
  'master-products':{view:'master-products',eyebrow:'PRODUK & SKU',title:'Produk, Tarif & BOM',description:'Identitas dibaca Merek → Nomor SKU → Model. Nomor yang sama boleh dipakai merek berbeda; pola tidak disimpan di SKU dan hanya ditarik dari lineage produksi.',browserLabel:'BROWSER SKU',singular:'produk',icon:Package,writerStatus:'Read-ready · writer produk masih simulasi',integrityTitle:'Identitas dan tarif SKU yang sudah bergerak tidak ditimpa.',integrityCopy:'Dalam satu merek, varian size untuk satu nomor SKU wajib tetap satu model dan warna; exact size tidak boleh dobel pada periode yang sama. Perubahan identitas, BOM, atau nominal menjadi versi penerus; FG, payroll, invoice, dan lineage pola lama tetap membaca snapshot sebelumnya.',emptyDependencyCopy:'SKU baru belum memiliki BOM, WIP, stok, payroll, atau invoice.',fields:[{key:'brand',label:'Merek',type:'select',options:['Vivo','Widie']},{key:'model',label:'Model',type:'text'},{key:'color',label:'Warna',type:'text'},{key:'sizes',label:'Ukuran aktif',type:'sizes',options:allSizes},{key:'sellingPrice',label:'Harga jual / pcs',type:'number',money:true},{key:'sewingRate',label:'Upah jahit / pcs',type:'number',money:true},{key:'commissionRate',label:'Komisi base / pcs',type:'number',money:true},{key:'specialCommissionRate',label:'Komisi Special / pcs',type:'number',money:true,optional:true,placeholder:'Kosong = tidak tersedia'},{key:'effectiveFrom',label:'Berlaku mulai',type:'date'}],seeds:productSeeds,metrics:(items)=>commonMetrics(items,{label:'BOM KATEGORI',value:'7 aktif · 3 future',note:'Pembayaran memakai kategori besar',tone:'warn'})},
...
const productIdentity=(entity:MasterEntity)=>`${entity.values.brand||'—'} · ${entity.code||'—'} · ${entity.values.model||'—'}`
...
function MasterEditor({mode,config,entity,onClose,onSave}:{mode:'create'|'edit';config:MasterConfig;entity?:MasterEntity;onClose:()=>void;onSave:(draft:EditorDraft)=>void}){
  ...
  const renderField=(field:FieldDefinition,autoFocus=false)=>field.type==='sizes'
    ? <div className="md-size-field" key={field.key}><span>{field.label.toUpperCase()}</span><SizeSelector value={draft.values[field.key]??''} onChange={(value)=>update(field.key,value)}/></div>
    : field.type==='boolean'
      ? <label className="md-boolean-field" key={field.key}><span>{field.label.toUpperCase()}</span><input type="checkbox" checked={draft.values[field.key]==='true'} disabled={draft.values.role!=='Mandor'} onChange={(event)=>update(field.key,String(event.target.checked))}/><span className="md-boolean-control"><i>{draft.values[field.key]==='true'&&<Check/>}</i><b>{draft.values[field.key]==='true'?'Special aktif':'Mandor regular'}</b><small>{draft.values.role==='Mandor'?'Pakai komisi Special SKU dan harga Special aksesori.':'Hanya tersedia untuk peran Mandor.'}</small></span></label>
      : <label key={field.key}><span>{field.label.toUpperCase()}{field.optional?' · OPSIONAL':''}</span>{field.type==='select'?<select autoFocus={autoFocus} value={draft.values[field.key]??''} onChange={(event)=>update(field.key,event.target.value)}>{field.options?.map((option)=><option key={option}>{option}</option>)}</select>:field.money?<MoneyEditorInput autoFocus={autoFocus} value={draft.values[field.key]??''} onChange={(value)=>update(field.key,value)}/>:<div><input autoFocus={autoFocus} type={field.type==='date'?'date':'text'} inputMode={field.type==='number'?'numeric':undefined} placeholder={field.placeholder} value={draft.values[field.key]??''} onChange={(event)=>update(field.key,field.type==='number'?event.target.value.replace(/\D/g,''):event.target.value)}/>{field.suffix&&<em>{field.suffix}</em>}</div>}</label>
  const brandField=config.fields.find((field)=>field.key==='brand')
  const modelField=config.fields.find((field)=>field.key==='model')
  const remainingProductFields=config.fields.filter((field)=>!['brand','model'].includes(field.key))
  ...
  return <div className="md-backdrop" role="presentation" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose()}}><section className="md-modal" role="dialog" aria-modal="true" aria-label={`${mode==='create'?'Tambah':'Edit'} ${config.singular}`}>
    <ModalHeader eyebrow={mode==='create'?'MASTER BARU':'EDIT BER-VERSI'} title={mode==='create'?`Tambah ${config.singular}`:`Edit ${isProduct&&entity?productIdentity(entity):entity?.name}`} copy={mode==='create'?'Record baru dimulai tanpa dependency transaksi.':`row-version ${entity?.rowVersion} akan bertambah saat tersimpan.`} onClose={onClose}/>
    <div className="md-editor-grid">{isProduct?<>{brandField&&renderField(brandField,true)}<label><span>NOMOR SKU</span><input value={draft.code} onChange={(event)=>setDraft((current)=>({...current,code:event.target.value}))}/></label>{modelField&&renderField(modelField)}<label><span>NAMA PRODUK / DESKRIPSI</span><input value={draft.name} onChange={(event)=>setDraft((current)=>({...current,name:event.target.value}))}/></label>{remainingProductFields.map((field)=>renderField(field))}</>:<><label><span>KODE</span><input autoFocus value={draft.code} onChange={(event)=>setDraft((current)=>({...current,code:event.target.value}))}/></label><label><span>NAMA</span><input value={draft.name} onChange={(event)=>setDraft((current)=>({...current,name:event.target.value}))}/></label>{config.fields.map((field)=>renderField(field))}</>}</div>
...
function BomEditorModal({entity,categoryRates,onClose,onSave}:{entity:MasterEntity;categoryRates:CategoryRate[];onClose:()=>void;onSave:(lines:BomLine[])=>void}){
  ...
  const available=categoryRates.filter((rate)=>rate.configured&&!lines.some((line)=>line.category===rate.category))
  ...
  return <div className="md-backdrop" role="presentation"><section className="md-modal wide" role="dialog" aria-modal="true" aria-label="Edit BOM SKU"><ModalHeader eyebrow="MASTER BOM PER SKU" title={productIdentity(entity)} copy="BOM menentukan standar reimburse per Good FG; tidak mengurangi stock card." onClose={onClose}/><div className="md-bom-editor"><div className="head"><span>KATEGORI</span><span>QTY / GOOD FG</span><span>UOM</span><span/></div>{lines.map((line)=><article key={line.id}><select aria-label={`Kategori ${line.category}`} value={line.category} onChange={(event)=>{const rate=categoryRates.find((item)=>item.category===event.target.value);setLines((current)=>current.map((item)=>item.id===line.id?{...item,category:event.target.value,uom:rate?.reimbursementUom??item.uom}:item))}}>{categoryRates.filter((rate)=>rate.configured).map((rate)=><option disabled={lines.some((item)=>item.id!==line.id&&item.category===rate.category)} key={rate.category}>{rate.category}</option>)}</select><input aria-label={`Quantity ${line.category}`} type="number" min="0.01" step="0.01" value={line.quantity} onChange={(event)=>setLines((current)=>current.map((item)=>item.id===line.id?{...item,quantity:Number(event.target.value)}:item))}/><span>{line.uom}</span><button type="button" aria-label={`Hapus ${line.category} dari BOM`} onClick={()=>setLines((current)=>current.filter((item)=>item.id!==line.id))}><Trash2/></button></article>)}</div><button type="button" className="md-add-line" disabled={available.length===0} onClick={addLine}><Plus/> Tambah kategori BOM</button><div className="md-editor-warning"><ShieldCheck/><span><strong>Versi penerus, bukan overwrite</strong><small>Kategori future tidak dapat masuk BOM sampai tarifnya disahkan. WIP/payroll lama tetap membaca snapshot BOM sebelumnya.</small></span></div><footer><button type="button" className="soft-btn" onClick={onClose}>Batal</button><button type="button" className="primary-btn" disabled={!valid} onClick={()=>onSave(lines)}>Simpan BOM simulasi</button></footer></section></div>
}

type CategoryRateDraft={category:string;chargeUom:string;reimbursementUom:string;mandorCharge:string;reimbursement:string;effectiveFrom:string;version:number;configured:boolean}
...
function CategoryRateModal({rates,onClose,onSave}:{rates:CategoryRate[];onClose:()=>void;onSave:(rates:CategoryRate[])=>void}){
  ...
  const valid=note.trim().length>=4&&drafts.filter((item)=>item.configured).every((item)=>Number(item.mandorCharge)>0&&Number(item.reimbursement)>0&&item.effectiveFrom)
  return <div className="md-backdrop" role="presentation"><section className="md-modal wide" role="dialog" aria-modal="true" aria-label="Tarif kategori aksesori"><ModalHeader eyebrow="TARIF AKSESORI PER KATEGORI" title="Tagihan mandor & reimburse" copy="Dua nominal dan dua UOM independen; HPP pembelian serta moving average tidak ikut berubah." onClose={onClose}/><div className="md-category-editor"><div className="head"><span>KATEGORI</span><span>HARGA TAGIH MANDOR</span><span>TARIF REIMBURSE</span><span>BERLAKU MULAI</span></div>{drafts.map((item)=><article key={item.category}><span><strong>{item.category}</strong><small>{item.configured?`versi ${item.version}`:'FUTURE · belum pernah ditagih'}</small></span>{item.configured?<><div className="md-category-price"><MoneyEditorInput value={item.mandorCharge} onChange={(value)=>update(item.category,'mandorCharge',value)}/><select aria-label={`UOM harga jual ${item.category}`} value={item.chargeUom} onChange={(event)=>update(item.category,'chargeUom',event.target.value)}><option value="pcs">PCS</option><option value="lusin">LUSIN</option><option value="gross">GROSS</option><option value="pack">PACK</option><option value="box">BOX</option></select></div><div className="md-category-price"><MoneyEditorInput value={item.reimbursement} onChange={(value)=>update(item.category,'reimbursement',value)}/><select aria-label={`UOM reimburse ${item.category}`} value={item.reimbursementUom} onChange={(event)=>update(item.category,'reimbursementUom',event.target.value)}><option value="pcs">PCS</option><option value="lusin">LUSIN</option><option value="gross">GROSS</option><option value="pack">PACK</option><option value="box">BOX</option></select></div><input type="date" value={item.effectiveFrom} onChange={(event)=>update(item.category,'effectiveFrom',event.target.value)}/></>:<><div className="md-category-price"><strong>Belum ada tarif</strong></div><div className="md-category-price"><strong>Biaya lain-lain</strong></div><span>Aktifkan di versi CP berikut</span></>}</article>)}</div><label className="md-modal-note"><span>CATATAN SINGKAT</span><textarea value={note} onChange={(event)=>setNote(event.target.value)}/></label><div className="md-editor-warning"><Coins/><span><strong>Kancing Rp495/pcs vs Rp500/pcs · Sleting Rp29.900/lusin vs Rp2.500/pcs</strong><small>Tagihan menilai kuantitas aktual yang diambil. Reimburse membayar kategori besar berdasarkan BOM Good FG; kategori future tetap nonaktif sampai tarif disahkan.</small></span></div><footer><button type="button" className="soft-btn" onClick={onClose}>Batal</button><button type="button" className="primary-btn" disabled={!valid} onClick={()=>onSave(drafts.map((item)=>({...item,mandorCharge:Number(item.mandorCharge),reimbursement:Number(item.reimbursement),version:item.configured?item.version+1:item.version})))}>Simpan versi tarif</button></footer></section></div>
}
...
function MasterWorkspace({config}:{config:MasterConfig}){
  ...
  return <>
    ...
    <section className="panel md-workspace"><aside>
      ...
      <header className="md-detail-head"><div><span>IDENTITAS MASTER</span><h2>{isProduct?productIdentity(selected):selected.name}</h2><p>{isProduct?`${selected.name} · `:`${selected.code} · `}terakhir diperbarui {selected.updatedAt}</p></div><div><MasterState active={selected.active}/><small>row-version {selected.rowVersion}</small></div></header>
