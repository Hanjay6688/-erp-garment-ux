...
const fromDocument = (d: AccessoryDocument): Form => ({ id:d.id,version:d.row_version,number:d.number,contractor:d.contractor_id,
  location:d.location_id,po:d.po_id ?? '',at:d.physical_local,notes:d.notes,lines:d.items.map(i => ({ material_id:i.material_id,
    qty:displayDecimal(i.qty),mode:i.free ? 'FREE' : i.manual_price === null ? 'MASTER' : 'MANUAL',manual_price:i.free ? '' : i.manual_price ?? '' })) })
      ...
      {isDraft && <>
        ...
        <div className="initial-import-table"><table><thead><tr><th>Aksesori / stok gudang</th><th>Jumlah fisik</th><th>Dasar harga</th><th>Harga eceran / pcs</th><th>Tagihan</th><th/></tr></thead><tbody>{form.lines.map((line,index) => {
          ...
          return <tr key={line.material_id}><td>{m?.name ?? document?.items.find(i => i.material_id===line.material_id)?.name ?? 'Aksesori belum dimuat'}<br/>{m && <small>Stok: {displayDecimal(m.stock)} PCS · {m.master_price === null ? 'Harga master belum tersedia' : `${displayMoney(micro(m.master_price))} / ${m.price_unit}`}</small>}</td>
            ...
            <td><select aria-label={`Dasar harga ${index+1}`} disabled={!editable} value={line.mode} onChange={e => updateLine(line.material_id,{mode:e.target.value as 'MASTER' | 'MANUAL' | 'FREE'})}><option value="MANUAL">Eceran per pcs</option><option value="MASTER">Harga master</option>{(m?.free || line.mode === 'FREE') && <option value="FREE">Gratis Special (kebijakan owner)</option>}</select></td>
            <td>{line.mode==='MANUAL' ? <input aria-label={`Harga per PCS ${index+1}`} inputMode="decimal" disabled={!editable} value={line.manual_price} onChange={e => updateLine(line.material_id,{manual_price:e.target.value})}/> : line.mode==='FREE' ? 'Gratis: kategori pada daftar Special owner' : 'Mengikuti master pada waktu ambil'}</td>
