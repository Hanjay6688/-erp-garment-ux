...
const accessories: Accessory[] = [
  { id:'ACC-KNC-17', name:'Kancing Jeans 17 mm', category:'Kancing', supplier:'Inti Metal', unit:'pcs', physical:12480, supplierReturned:0, minimum:6000, avgCost:1850, location:'Aksesori A-01', status:'Aman', lastMovement:'Keluar ke Mandor Asep · 10:22' },
  { id:'ACC-RVT-CU', name:'Rivet Copper', category:'Rivet', supplier:'Inti Metal', unit:'pcs', physical:7160, supplierReturned:480, minimum:5000, avgCost:940, location:'Aksesori A-02', status:'Menipis', lastMovement:'Retur supplier RTS-260827-012 · kemarin' },
  { id:'ACC-ZIP-14', name:'Resleting YKK 14 cm', category:'Resleting', supplier:'Sinar Zip', unit:'pcs', physical:4380, supplierReturned:0, minimum:2500, avgCost:7100, location:'Aksesori B-01', status:'Aman', lastMovement:'Terima PO-BELI-260827-018 · kemarin' },
  { id:'ACC-HTG-WD', name:'Hangtag Widie', category:'Label & tag', supplier:'Cetak Jaya', unit:'pcs', physical:2260, supplierReturned:240, minimum:2500, avgCost:620, location:'Aksesori C-02', status:'Perlu pesan', lastMovement:'Retur supplier RTS-260826-009 · 16:18' },
  { id:'ACC-KRT-32', name:'Karet Pinggang 32 mm', category:'Karet', supplier:'Maju Elastik', unit:'meter', physical:1860, supplierReturned:0, minimum:900, avgCost:3950, location:'Aksesori B-03', status:'Aman', lastMovement:'Retur Mandor Asep · kemarin' },
  { id:'ACC-LBL-VV', name:'Label Woven Vivo', category:'Label & tag', supplier:'Cetak Jaya', unit:'pcs', physical:3820, supplierReturned:0, minimum:2000, avgCost:780, location:'Aksesori C-01', status:'Aman', lastMovement:'Keluar ke Mandor Intan · kemarin' },
]
...
const statusTone = (status: string): Tone => status === 'Aman' || status === 'Healthy' || status === 'Available' || status === 'Selesai' || status === 'ACTUAL' ? 'good' : status === 'Menipis' || status === 'Low' || status === 'In Use' || status === 'Menunggu review' || status === 'ESTIMATED' ? 'warn' : status === 'Perlu pesan' || status === 'Perlu cek' || status === 'Review' || status === 'Selisih ditemukan' ? 'danger' : 'neutral'
  ...
  return <div className="wh-return-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="wh-return-modal" role="dialog" aria-modal="true" aria-labelledby="supplier-return-title" onMouseDown={(event)=>event.stopPropagation()}>
      ...
      {target&&<div className="wh-return-form">
        <div className="wh-return-identity"><span><small>ITEM / ROLL</small><strong>{target.label}</strong><em>{target.id} · {target.supplier}</em></span><span><small>STOCK ON HAND</small><strong>{formatNumber(target.maxQty)} {target.unit}</strong><em>Saldo ledger yang boleh diretur sekarang</em></span></div>
...
function WarehouseDashboard({ onNavigate }: { onNavigate: (view: WarehouseView) => void }) {
  ...
  return <>
    ...
    <section className="wh-dashboard-grid">
      <div className="panel wh-zone-panel"><header><div><span>AREA GUDANG</span><h2>Buka dari pekerjaan fisiknya</h2><p>Angka utama adalah Stock On Hand dari mutasi posted—tanpa layer reservasi palsu.</p></div><Warehouse/></header><div className="wh-zone-grid">
...
function AccessoriesPage() {
  ...
  const coverage = selected.minimum ? selected.physical/selected.minimum : 0
  ...
  return <>
    <section className="hero-copy compact wh-hero"><div><div className="eyebrow">GUDANG · KOMPONEN</div><h1>Aksesori</h1><p>Qty langsung per unit. Stock On Hand, minimum stok, mutasi posted, dan moving average terlihat tanpa reservation/ATP yang belum ada di backend.</p></div><div className="wh-rule"><Boxes/><div><span>POLA INPUT</span><strong>Qty langsung, bukan gulungan</strong><small>PCS, meter, set, atau unit master item.</small></div></div></section>
    ...
    <section className="wh-metrics"><MetricCard label="Stock On Hand" value="31.960 unit" note="Saldo dari mutasi posted" icon={Package}/><MetricCard label="Di bawah minimum" value="1 item" note="Hangtag Widie · kurang 240 pcs" tone="warn" icon={AlertTriangle}/><MetricCard label="Moving Average" value="6 item" note="Valuasi aktif per item" tone="good" icon={Tag}/><MetricCard label="Returned to Supplier" value="720 pcs" note="2 dokumen retur supplier" tone="danger" icon={RotateCcw}/></section>
    <div className="panel wh-workspace">
      ...
      <div className="wh-master-detail accessory">
        ...
        <main className="wh-detail"><header className="wh-detail-head"><div><span>ITEM TERPILIH</span><h2>{selected.name}</h2><p>{selected.id} · {selected.category} · {selected.supplier}</p></div><div className="wh-detail-total"><span>STOCK ON HAND</span><strong>{formatNumber(available,0)} {selected.unit}</strong><small>{money(selected.avgCost)} / {selected.unit} moving average</small></div></header>
          <div className="wh-accessory-facts"><div><MapPin/><span><small>LOCATION</small><strong>{selected.location}</strong></span></div><div><Boxes/><span><small>STOCK ON HAND</small><strong>{formatNumber(selected.physical,0)} {selected.unit}</strong></span></div><div><Tag/><span><small>VALUATION</small><strong>Moving average</strong></span></div><div className="returned"><RotateCcw/><span><small>RETURNED TO SUPPLIER</small><strong>{formatNumber(selected.supplierReturned,0)} {selected.unit}</strong></span></div><div className={coverage<1?'danger':'good'}><ShieldCheck/><span><small>MINIMUM</small><strong>{formatNumber(selected.minimum,0)} {selected.unit}</strong></span></div></div>
          <div className={`wh-stock-health ${coverage<1?'danger':coverage<1.5?'warn':'good'}`}><div><span>STOCK HEALTH</span><strong>{coverage<1?'Below minimum':coverage<1.5?'Near minimum':'Safe on hand'}</strong><small>Stock On Hand {formatNumber(available,0)} dibanding minimum {formatNumber(selected.minimum,0)} {selected.unit}.</small></div><b>{Math.round(coverage*100)}%</b></div>
