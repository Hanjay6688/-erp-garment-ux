...
function ProductionBalances({ batch, locked, manage }: { batch: Batch; locked: boolean; manage: (payload: Record<string, Json>) => void }) {
  ...
  return <section className="panel initial-import-advances" aria-label="Saldo fisik produksi awal">
    ...
    <p>{(selected.stage === 'LAUNDRY' ? selected.vendor_name : selected.contractor_name ?? selected.vendor_name) ?? 'Pemegang mengikuti rincian saldo'} · Awal {selected.qty_pcs} pcs · Sisa {selected.remaining_qty_pcs} pcs</p>
    ...
    {selected.balance_type === 'BS' ? <p>Lanjutkan pemeriksaan, rework, atau pengeluaran melalui halaman BS/Rework.</p> : <>
      <div className="initial-import-toolbar"><label>Hasil baik (pcs)<input aria-label="Hasil WIP baik" inputMode="numeric" value={qty} disabled={locked} onChange={e => setQty(e.target.value)}/></label><label>Kode produk jadi<input aria-label="Produk hasil WIP" value={sku} disabled={locked} onChange={e => setSku(e.target.value)}/></label><label>Kode merek produk<input aria-label="Merek hasil WIP" value={brand} disabled={locked} onChange={e => setBrand(e.target.value)}/></label><label>Kode gudang barang jadi<input aria-label="Gudang hasil WIP" value={location} disabled={locked} onChange={e => setLocation(e.target.value)}/></label><label>Tanggal hasil<input aria-label="Tanggal hasil WIP" type="date" value={date} disabled={locked} onChange={e => setDate(e.target.value)}/></label><label>Catatan pemeriksaan<input aria-label="Catatan hasil WIP" value={reason} disabled={locked} onChange={e => setReason(e.target.value)}/></label><button type="button" disabled={locked || !valid} onClick={() => send({ operation:'COMPLETE', qty_pcs:qty, product_sku:sku.trim(), ...(brand.trim() ? { brand_code:brand.trim() } : {}), location_code:location.trim(), date })}>Sahkan hasil WIP awal</button></div>
