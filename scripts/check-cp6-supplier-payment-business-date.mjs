...
let previous=-1
for(const token of ['Apply v2.6.20r receipt invoice dependency','Reproduce independent R supplier payment business-date','Apply v2.6.20s supplier payment canonical business date','Run post-CP6 real Auth','Prove native S supplier payment canonical business date','Qualify exact F G H I J K L M N O P Q R S T U V and W rollback','Prove trusted S capsule','Prove trusted R capsule']) {const at=workflow.indexOf(token);assert.ok(at>previous,token);previous=at}
for(const token of ["len(schedules['cases'])==360","'expected':90,'observed':90","setup_unit['completed_case_count']==setup_unit['expected_case_count']==188","sum(c['upgrade_guard']['invalid_history_refused'] for c in s_before['cases'].values())==4",'CP6_V2620W_RUNTIME_MANIFEST.json','CP6_V2620S_SUPPLIER_PAYMENT_BUSINESS_DATE_REGRESSION.json','PHYSICAL_DISPOSABLE_CP6_AUTH_CLONE_AFTER_V2620W','CP6_M_RACE_SOURCE_GENERATION: W',"rr['source_generation']=='W'"])assert.ok(workflow.includes(token),token)
...
console.log(JSON.stringify({status:'PASS',classification:'STATIC_SOURCE_CONTRACT_NOT_NATIVE_PROOF',replaced_functions:3,boundary_tables:69,native_cases:12,known_r_paths:6,controls:6,maintenance_schedules:360,writer_body_entries:90,production_go:false}))
