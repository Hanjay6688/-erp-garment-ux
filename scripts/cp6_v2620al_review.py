#!/usr/bin/env python3
"""AL writer gate: original oracles, new recovery scenarios, exact restoration."""
...
def install():
 ...
 with psycopg.connect(ADMIN) as conn,conn.cursor() as cur:
  ...
  save('AK_BASELINE',dict(catalog=function_catalog(cur),boundary=snapshot(cur)))
...
def rework_case(cur,today,initial_good,first_route,failed_wash,mixed_rates=False):
 ...
 cur.execute('select public.erp_record_sewing_terminal_v1(%s::jsonb,%s)',
  (json.dumps(dict(work_completion_id=str(f['completion']),qty_pcs=10,reason='AL ten original physical pieces')),uuid.uuid4()))
 actors.admin(cur)
 ...
 product=base.create_product(cur,'AL-'+uuid.uuid4().hex[:12])
 ...
 sent=laundry_action(cur,'POST_DELIVERY',dict(distribution_batch_id=f['batch'],vendor_id=base.VENDOR,
  wash_process_id=base.BASE_PROCESS,target_dyeing_color='AL-NAVY',physical_at=production.at(day,11),
  reason='AL ten physical pieces',lines=[dict(size_id=base.SIZE,qty_sent_pcs=10)]),base.group_version(cur,f['group']))['delivery_id']
 if failed_wash:
  laundry_action(cur,'POST_FAILED_WASH',dict(delivery_id=sent,wash_process_id=base.BASE_PROCESS,
   custody_outcome='RETRY_AT_VENDOR',physical_at=production.at(day,11,30).isoformat(),reason='AL genuine paid first attempt',
 ...
 if mixed_rates:
  ...
  cur.execute("insert into erp.wash_processes(id,process_code,process_name,is_active) values(%s,%s,'AL actual eleven wash',true)",(process,'AL-'+uuid.uuid4().hex[:16]))
  cur.execute("insert into erp.laundry_vendor_rate_versions(vendor_id,wash_process_id,rate_per_pcs,effective_from,notes) values(%s,%s,11,%s,'AL independently priced second receipt')",(base.VENDOR,process,production.at(day,0)))
  receipt_sources=[]
  for i,process_id in enumerate((base.BASE_PROCESS,process)):
   receipt=laundry_action(cur,'POST_RECEIPT',dict(delivery_id=sent,wash_process_id=process_id,
    physical_at=production.at(day,12,i*15).isoformat(),reason='AL two separately priced physical receipt groups',
  ...
  received=laundry_action(cur,'POST_RECEIPT',dict(delivery_id=sent,wash_process_id=base.BASE_PROCESS,
   physical_at=production.at(day,12),reason='AL ten original physical receipts',
 ...
 laundry_action(cur,'POST_FINAL_SKU',dict(cutting_group_id=f['group'],destination_location_id=base.LOCATION,
  physical_at=production.at(day,13).isoformat(),reason='AL exact original Good and BS split',good_qty_pcs=initial_good,completion_mode='ALL_READY',
 ...
 for index,route in enumerate((first_route,'LAUNDRY' if first_route=='CONTRACTOR' else 'CONTRACTOR')):
  ...
  payload=dict(rework_number='AL-'+uuid.uuid4().hex[:20],bs_case_id=bs,destination_type=route,
   ...
   accessory_bom_version_id=bom,accessory_bom_item_ids=[],change_reason='AL four physical pieces, no new original work entitlement',
  ...
  bs_action(cur,'SAVE_REWORK',dict(id=order,action='SAVE',qty_good_returned=1,qty_bs_returned=1,
   return_fg_location_id=base.LOCATION,change_reason='AL two cumulative returns, two still away'),version(cur,'rework_orders',order))
  ...
  payload=dict(rework_order_id=order,qty_good=2,qty_bs=sent_qty-2,completed_at=production.at(day,14+index,30),
   return_fg_location_id=base.LOCATION,change_reason='AL four returned, two recover to Good')
 ...
 cur.execute('savepoint al_source_refusal')
 try:laundry_action(cur,'REVERSE_FINAL_SKU',dict(qc_inspection_id=inspection,reason='AL source still has active recoveries'),iv)
 except psycopg.Error as exc:refused=dict(sqlstate=exc.sqlstate,error=str(exc))
 finally:
  cur.execute('rollback to savepoint al_source_refusal');actors.admin(cur);cur.execute('release savepoint al_source_refusal')
 ...
 bs_action(cur,'REVERSE_REWORK_COMPLETION',dict(rework_order_id=orders[0],change_reason='AL linked correction of first recovery'),version(cur,'rework_orders',orders[0]))
 total_good-=2;state=po_state(cur,po)
 expected_fg=original_fg+(total_good-initial_good)*unit
...
# BEGIN AL POSTED IMPORT ORACLE
def crossflow_case(cur,day,name,fn):
 # AL locks/checks the migration batch before the opening header. A replay of
...
def run_cases():
 ...
 with psycopg.connect(ADMIN) as conn,conn.cursor() as cur:
  ...
  for name,fn in specs:
   actors.admin(cur);before=actors.boundary(cur);cur.execute('savepoint al_case')
   try:row=fn(cur)
   except Exception as exc:row=dict(status='INCOMPLETE',error=str(exc),traceback=traceback.format_exc())
   finally:cur.execute('rollback to savepoint al_case');actors.admin(cur);cur.execute('release savepoint al_case')
...
def original_schedules(head,tree):
 ...
 try:
  ...
  with old.connect('ak-original-foundation') as conn,conn.cursor() as cur:
   assert len(runtime.verified_successor(cur))==690
...
def concurrency():
 ...
 result.update(tested_runtime_head=head,tested_runtime_tree=tree,runtime_generation='AL',independent_acceptance=False)
 with psycopg.connect(ADMIN) as conn,conn.cursor() as cur:
  assert len(runtime.verified_successor(cur))==690
...
def rollback():
 ...
 spec=spec_from_file_location('al_maintenance_controller',runtime.ROOT/'scripts/cp6_preuse_rollback_maintenance.py')
 maintenance=module_from_spec(spec);spec.loader.exec_module(maintenance)
 maintenance.TARGETS['AL']['rollback']=runtime.reviewed_local_rollback()
 before=json.loads((ROOT/'AK_BASELINE.json').read_text())
 controls={
  'FUNCTION':("alter function erp._validate_migration_batch_base(uuid) set work_mem='64MB'",'AL_TRUSTED_PREDECESSOR_PIN_MISMATCH'),
  'ACL':('grant execute on function erp._validate_migration_batch_base(uuid) to anon','AL_TRUSTED_PREDECESSOR_PIN_MISMATCH'),
  'OWNER':('alter function erp._validate_migration_batch_base(uuid) owner to supabase_admin','AL_TRUSTED_PREDECESSOR_PIN_MISMATCH'),
  'EXTRA_TABLE':('create table erp.cp6_al_unexpected(id integer)','AL_BOUNDARY_SNAPSHOT_MISMATCH'),
  'MISSING_TABLE':('drop table erp.cp6_v2620ac_rollback_capsule','AL_BOUNDARY_SNAPSHOT_MISMATCH'),
  'POST_USE':("insert into erp.locations(location_code,location_name,location_type) values('AL-POST-USE','AL probe','FG_WAREHOUSE')",'AL_POST_USE_ROLLBACK_REFUSED'),
  'CAPSULE':(f"update {runtime.CAPSULE} set object_definition=object_definition||E'\\n-- changed'",'AL_TRUSTED_PREDECESSOR_PIN_MISMATCH'),
  'PLATFORM':(f"update supabase_migrations.schema_migrations set statements=array['wrong'] where name='{runtime.NAME}'",'AL_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR')}
 atomic={}
 with psycopg.connect(ADMIN) as conn,conn.cursor() as cur:
  ...
  for name,(statement,expected) in controls.items():
   boundary,catalog=actors.boundary(cur),function_catalog(cur);cur.execute('savepoint al_refusal')
   try:
   ...
   finally:cur.execute('rollback to savepoint al_refusal;release savepoint al_refusal')
 ...
 operation=maintenance.run_maintenance_rollback(target_name='AL',target_pgurl=URL,
 ...
 assert before==actual,'AL rollback did not restore the exact AK boundary'
 ...
 save('EXACT_AK_RESTORE',result);return result
...
if __name__=='__main__':
 ...
 # A completed business ledger may retain the known date-policy HOLD. The
 # workflow checks its exact 230-case counts and never promotes HOLD to PASS.
 complete_hold=phase=='business' and result['status']=='HOLD' and result.get('counts')==dict(PASS=179,CONTROL_PASS=39,BUG_PROVEN=0,GAP_PROVEN=0,DATE_POLICY_REVIEW_REQUIRED=12,INCOMPLETE=0,FAIL=0)
 raise SystemExit(0 if complete_hold or result['status'] in ('PASS','WRITER_PASS','PASS_REVIEWED_SCOPE') else 1)
