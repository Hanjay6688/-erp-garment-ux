...
for(const token of ['MATERIAL_A_FIRST','MATERIAL_B_FIRST','MATERIAL_A_ABORT','pg_blocking_pids(pid)',
  "SOURCE_GENERATION = 'Z'", 'matrix.verify_setup_source(SOURCE_GENERATION)', "'source_generation': SOURCE_GENERATION",'first.rollback()','idempotent_replay']) assert.ok(races.includes(token),token)
...
for(const token of ["len(schedules['cases'])==420","'expected':105,'observed':105",
  ...
  'CP6_V2620Z_RUNTIME_MANIFEST.json','CP6_V2620T_MATERIAL_ADJUSTMENT_REVALUATION_REGRESSION.json',
  'PHYSICAL_DISPOSABLE_CP6_AUTH_CLONE_AFTER_V2620Z','CP6_M_RACE_SOURCE_GENERATION: Z',
...
console.log(JSON.stringify({status:'PASS',classification:'STATIC_SOURCE_CONTRACT_NOT_NATIVE_PROOF',
  ...
  known_s_paths:15,controls:6,shared_document_native_races:3,maintenance_schedules:420,
  writer_body_entries:105,production_go:false}))
