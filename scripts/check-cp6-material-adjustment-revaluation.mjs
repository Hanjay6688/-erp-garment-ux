...
for(const token of ['MATERIAL_A_FIRST','MATERIAL_B_FIRST','MATERIAL_A_ABORT','pg_blocking_pids(pid)',
  "SOURCE_GENERATION = 'X'", 'matrix.verify_setup_source(SOURCE_GENERATION)', "'source_generation': SOURCE_GENERATION",'first.rollback()','idempotent_replay']) assert.ok(races.includes(token),token)
...
for(const token of ["len(schedules['cases'])==380","'expected':95,'observed':95",
  "setup_unit['completed_case_count']==setup_unit['expected_case_count']==208",
  'CP6_V2620X_RUNTIME_MANIFEST.json','CP6_V2620T_MATERIAL_ADJUSTMENT_REVALUATION_REGRESSION.json',
  'PHYSICAL_DISPOSABLE_CP6_AUTH_CLONE_AFTER_V2620X','CP6_M_RACE_SOURCE_GENERATION: X',
...
console.log(JSON.stringify({status:'PASS',classification:'STATIC_SOURCE_CONTRACT_NOT_NATIVE_PROOF',
  ...
  known_s_paths:15,controls:6,shared_document_native_races:3,maintenance_schedules:380,
  writer_body_entries:95,production_go:false}))
