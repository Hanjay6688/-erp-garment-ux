#!/usr/bin/env python3
"""Trusted R capsule guards and maintenance-only exact R-to-Q restore."""
...
ROLLBACK = Path(
    'supabase/rollbacks/20260912114635_erp_v2_6_20r_cp6_receipt_invoice_dependency.rollback.sql'
)
REPORT = Path('cp6-proof/CP6_V2620R_ROLLBACK_GUARDS.json')
MAINTENANCE_REPORT = Path('cp6-proof/V2620R_MAIN_MAINTENANCE_ROLLBACK.json')
CLONE_ROOT = Path('cp6-proof/R_TRUSTED_CAPSULE_GUARD')
DIRECT_GUARD_NAMES = (
...
def verify_capsule_fault() -> dict[str, Any]:
    ...
    with n_guards.disposable_clone_confirmation(matrix.CLONE):
        try:
            matrix.prepare('R', 'REPORT', CLONE_ROOT, source_generation='R')
            coherent_capsule_fault(matrix.CLONE)
            rejection = None
            try:
                maintenance.run_maintenance_rollback(
                    target_name='R', target_pgurl=matrix.CLONE,
...
def direct_guards(target_pgurl: str) -> list[dict[str, Any]]:
    ...
    with psycopg.connect(target_pgurl, autocommit=False) as conn:
        ...
        cases = (
            ('platform_bytes',
             "update supabase_migrations.schema_migrations set statements=array['tampered R'] where name='erp_v2_6_20r_cp6_receipt_invoice_dependency'",
             'R_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR', None),
            ('successor',
             ...
             'R_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR', None),
            ('definition_drift',
             'alter function erp.reverse_material_purchase(uuid,text) cost 999',
             'R_TRUSTED_PREDECESSOR_PIN_MISMATCH', None),
            ('acl_drift',
             'grant execute on function erp.reverse_material_purchase(uuid,text) to anon',
             'R_TRUSTED_PREDECESSOR_PIN_MISMATCH', None),
            ('boundary_drift',
             "update erp.cp6_v2620r_rollback_capsule set boundary_snapshot=boundary_snapshot-'supplier_payments'",
             'R_BOUNDARY_SNAPSHOT_MISMATCH', None),
            ('post_install_boundary_history',
             "update erp.cp6_v2620q_rollback_capsule set boundary_snapshot=boundary_snapshot||'{\"r_post_use_probe\":true}'::jsonb",
             'R_POST_USE_ROLLBACK_REFUSED', None),
            ('coherent_capsule_and_checksum', None,
             'R_TRUSTED_PREDECESSOR_PIN_MISMATCH', coherent),
        )
        if tuple(case[0] for case in cases) != DIRECT_GUARD_NAMES:
            raise AssertionError('Direct R guard case manifest drift')
...
def verify_extra_preflight_guards() -> list[dict[str, Any]]:
    ...
    with n_guards.disposable_clone_confirmation(matrix.CLONE):
        for name, mutation, expected in EXTRA_FAULTS:
            ...
            try:
                matrix.prepare('R', 'REPORT', folder, source_generation='R')
                with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                ...
                if rejection is None or expected not in rejection:
                    raise AssertionError(
                        'R_EXTRA_PREFLIGHT_WRONG_REJECTION: ' + name
                    )
                observed = matrix.read_json_if_present(
                ...
                if observed.get('admission_closed') or observed.get('rollback_started'):
                    raise AssertionError(
                        'R_EXTRA_PREFLIGHT_MUTATED_ADMISSION: ' + name
                    )
                ...
                with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                    marker = conn.execute("""select count(*)
                      from erp.schema_migrations where version='v2.6.20r'""").fetchone()
                if admission != (True,) or marker != (1,):
                    raise AssertionError(
                        'R_EXTRA_PREFLIGHT_STATE_CHANGED: ' + name
                    )
                results.append({
...
def verify_exact_q_restore(target_pgurl: str) -> dict[str, Any]:
    expected = maintenance.TRUSTED_FUNCTIONS['R']
    with psycopg.connect(target_pgurl, autocommit=False) as conn, conn.cursor() as cur:
        ...
        cur.execute("""select
          not exists(select 1 from erp.schema_migrations where version='v2.6.20r')
          and exists(select 1 from erp.schema_migrations where version='v2.6.20q')
          and not exists(select 1 from supabase_migrations.schema_migrations
            where name='erp_v2_6_20r_cp6_receipt_invoice_dependency')
          and to_regclass('erp.cp6_v2620r_rollback_capsule') is null
          and to_regclass('erp.cp6_v2620q_rollback_capsule') is not null
    ...
    return {
        'generation': 'Q', 'restored_function_count': len(actual),
...
def run() -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        'classification': (
            'DISPOSABLE_NATIVE_R_TRUSTED_GUARDS_AND_CLOSED_ADMISSION_RESTORE'
        ),
    ...
    maintenance_result = maintenance.run_maintenance_rollback(
        target_name='R', target_pgurl=target_pgurl,
    ...
    result.update(
        status='PASS',
        exact_pre_use_restore=verify_exact_q_restore(target_pgurl),
        maintenance={
            'status': 'PASS', 'target': 'R',
...
def main() -> None:
    ...
    try:
        ...
        result = {
            'head': os.environ.get('GITHUB_SHA', 'LOCAL_UNBOUND'),
            'status': 'FAIL',
            'error_code': 'V2620R_ROLLBACK_GUARD_FAILED',
