"""Closed-admission AT install/restore; exact AS history and data remain pinned."""
from pathlib import Path
import json,os,subprocess,time
import psycopg
from psycopg import sql
import cp6_at_build as build
import cp6_as_runtime as predecessor
...
EXPECTED_PINS='87ced118da7193cf274027e91e09cc858d6a60720b7e1ae58d367d5f5576ad6c'
def pins():
    predecessor.pins()
    assert sha(build.PINS.read_bytes())==EXPECTED_PINS,'AT_SOURCE_PINS_DRIFT'
    p=json.loads(build.PINS.read_text())
    assert sha(Path(build.__file__).read_bytes())==p['builder_sha256'],'AT_BUILDER_DRIFT'
    assert sha(Path('scripts/cp6_at_definitions.py').read_bytes())==p['definitions_sha256'],'AT_DEFINITIONS_DRIFT'
    for key in ('migration','rollback'):assert sha(Path(p[key]).read_bytes())==p[key+'_sha256'],'AT_SQL_DRIFT'
    return p

def verified(cur):
    p=pins();base,capture,before,after,_=build.model()
    cur.execute("set local search_path='';set local timezone='UTC'")
    assert function_pins(cur)==after['functions'],'AT_FUNCTION_OWNER_ACL_DRIFT'
    assert cur.execute(build.INVENTORY_SQL).fetchone()[0]==after['objects'],'AT_FULL_CATALOG_DRIFT'
    cur.execute(build.prior_guard(base,capture),prepare=False)
    cur.execute(build.package.capsule_guard('AT',p),prepare=False)
    ...
    return dict(stage='AT',function_count=len(after['functions']),object_count=len(after['objects']),full_catalog_verified=True,
...
def change(kind,pg,control_url):
    ...
    with psycopg.connect(pg,autocommit=True,application_name='cp6-at-'+kind) as conn,psycopg.connect(control_url,autocommit=True) as control:
        ...
        try:
            ...
            while True:
                ...
                if sessions:
                    ...
                    assert time.monotonic()<deadline,('AT_REQUIRES_DRAINED_DATABASE',sessions)
...
def advisors(pg,stage,path,baseline=None):
    def snapshot():
        with psycopg.connect(pg) as conn,conn.cursor() as cur:
            (predecessor.verified(cur) if stage=='AS' else verified(cur))
    ...
    assert r.returncode==0,'AT_ADVISORS_FAILED'
    if r.stdout.strip():findings=json.loads(r.stdout)
    else:
        assert 'No issues found' in r.stderr.splitlines(),'AT_ADVISORS_MISSING_RESULT'
        findings=[]
    assert isinstance(findings,list) and all(isinstance(x,dict) and x.get('name') and x.get('level') for x in findings)
    assert snapshot()==before,'AT_ADVISORS_CHANGED_BOUNDARY'
...
def qualify(pg,admin,out):
    """The two pre-use cycles operate on the same seeded, nonempty AS clone."""
    ...
    try:
        baseline=advisors(pg,'AS',out/'AT_ADVISORS_BEFORE.json')
        for _ in range(2):
            ...
            with psycopg.connect(admin) as conn,conn.cursor() as cur:
                ...
                else:raise AssertionError('AT_OPEN_ROLLBACK_ADMITTED')
            assert snapshot()==boundary
            restored=change('rollback',pg,control);assert snapshot()==before
            report['cycles'].append(dict(install=installed,open_rollback_refused_atomically=True,restore=restored,exact_as_data_and_history_restored=True))
        report['final_install']=change('install',pg,control)
        security=advisors(pg,'AT',out/'AT_ADVISORS_AFTER.json',baseline)
        ...
        report['status']='PASS'
    finally:(out/'AT_PACKAGE.json').write_text(json.dumps(report,indent=2,default=str)+'\n')

def refuse_post_use(pg,admin,out):
    """A used AT clone cannot be restored even after linked business reversals."""
    ...
    with psycopg.connect(pg,autocommit=True) as conn,psycopg.connect(control_url,autocommit=True) as control:
        ...
        try:
            ...
            while control.execute('select 1 from pg_stat_activity where datname=%s and pid<>%s',('cp6_rollback',pid)).fetchone():
                assert time.monotonic()<deadline,'AT_POST_USE_DRAIN_TIMEOUT'
                time.sleep(.05)
            with conn.transaction(),conn.cursor() as cur:
                verified(cur);before=dict(data=data(cur),platform=platform(cur))
                try:
                    with conn.transaction():cur.execute(prior.sql_body(build.ROLLBACK.read_text()),prepare=False)
                except psycopg.Error as exc:assert 'AT_POST_USE_ROLLBACK_REFUSED' in str(exc)
                else:raise AssertionError('AT_USED_ROLLBACK_ADMITTED')
    ...
    (out/'AT_POST_USE_REFUSAL.json').write_text(json.dumps(dict(status='PASS',all_erp_data_unchanged=True,platform_unchanged=True,production_go=False,independent_acceptance=False),indent=2)+'\n')
