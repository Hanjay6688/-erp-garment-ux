#!/usr/bin/env python3
"""Twenty AI schedules using the unchanged established concurrency oracle."""
...
ROOT = Path("cp6-proof/AI_MAINTENANCE_ROLLBACK")
REPORT = ROOT / "manifest.json"


def prepare(target, operation, folder, *, source_generation):
    """Bind the old oracle's fixture setup to AI -> AH, with exact verification."""
    if target != "AI" or source_generation != "AI":
        raise AssertionError("AI_SCHEDULE_REQUIRES_EXACT_EDGE")
    matrix.command(
    ...
    with matrix.legacy.connect("ai-clone-source") as conn, conn.cursor() as cur:
        if len(runtime.verified_successor(cur)) != 690:
            raise AssertionError("AI_CLONE_RUNTIME_MISMATCH")
    matrix.maintenance_strip("AI", folder)
    with matrix.legacy.connect("ai-predecessor-fixture") as conn, conn.cursor() as cur:
    ...
    with matrix.legacy.connect("ai-exact-platform-ledger") as conn, conn.cursor() as cur:
        cur.execute(
        ...
        if len(runtime.verified_successor(cur)) != 690:
            raise AssertionError("AI_SEEDED_RUNTIME_MISMATCH")
        conn.commit()
    capsule = matrix.legacy.capture_capsule("AI")
    if len(capsule) != 3:
        raise AssertionError("AI_SCHEDULE_CAPSULE_CARDINALITY")
...
def run():
    ...
    if actual != expected or os.environ.get("CP6_AI_MAINTENANCE_CONFIRM") != "cp6_rollback":
        raise AssertionError("AI_MAINTENANCE_EXACT_DISPOSABLE_ENVIRONMENT_REQUIRED")
    with psycopg.connect(matrix.SOURCE) as conn, conn.cursor() as cur:
        if len(runtime.verified_successor(cur)) != 690:
            raise AssertionError("AI_MAINTENANCE_SOURCE_MISMATCH")
    matrix.TARGETS["AI"] = (runtime.STAMP, runtime.NAME, runtime.VERSION, "v2.6.20ah", 3)
    original_prepare = matrix.prepare
    matrix.prepare = prepare
    result = {
        "format": "CP6_AI_MAINTENANCE_SCHEDULES_V1", "status": "INCOMPLETE",
    ...
    try:
        for operation in matrix.OPERATIONS:
            for mode in matrix.MODES:
                name = f"AI-{operation}-{mode}"
                folder = ROOT / name
                folder.mkdir(parents=True, exist_ok=True)
                try:
                    evidence = matrix.run_case("AI", operation, mode, folder, source_generation="AI")
                except Exception as exc:
                    evidence = {
                        "status": "FAIL", "target": "AI", "operation": operation,
