#!/usr/bin/env python3
"""Trusted O capsule guards and maintenance-only exact O-to-N restore."""
...
REPORT = Path('cp6-proof/CP6_V2620O_ROLLBACK_GUARDS.json')
MAINTENANCE_REPORT = Path('cp6-proof/V2620O_MAIN_MAINTENANCE_ROLLBACK.json')
CLONE_ROOT = Path('cp6-proof/O_TRUSTED_CAPSULE_GUARD')
DIRECT_GUARD_NAMES = (
...
def verify_capsule_fault() -> dict[str, Any]:
    ...
    with n_guards.disposable_clone_confirmation(matrix.CLONE):
        try:
            matrix.prepare('O', 'REPORT', CLONE_ROOT, source_generation='O')
            coherent_capsule_fault(matrix.CLONE)
            rejection = None
            try:
                maintenance.run_maintenance_rollback(
                    target_name='O', target_pgurl=matrix.CLONE,
            ...
            if rejection is None or 'TRUSTED_PREDECESSOR_PIN_MISMATCH' not in rejection:
                raise AssertionError('O coherent capsule fault was not rejected')
            ...
            if observed.get('admission_closed') or observed.get('rollback_started'):
                raise AssertionError('O trust failure crossed mutation boundary')
            with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                marker = conn.execute("""select count(*) from erp.schema_migrations
                  where version='v2.6.20o'""").fetchone()
            if marker != (1,):
                raise AssertionError('O trust guard changed installed generation')
            return {
                'target': 'O', 'status': 'PASS',
...
def direct_guards(target_pgurl: str) -> list[dict[str, Any]]:
    ...
    with psycopg.connect(target_pgurl, autocommit=False) as conn:
        ...
        cases = (
            ('platform_bytes',
             "update supabase_migrations.schema_migrations set statements=array['tampered O'] where name='erp_v2_6_20o_cp6_supplier_return_document_allocation'",
             'O_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR', None),
            ('successor',
             "insert into supabase_migrations.schema_migrations(version,statements,name) values('99999999999999',array['successor'],'o_successor_probe')",
             'O_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR', None),
            ('definition_drift',
             'alter function erp.post_material_supplier_return(uuid) cost 999',
             'O_TRUSTED_PREDECESSOR_PIN_MISMATCH', None),
            ('acl_drift',
             'grant execute on function erp.post_material_supplier_return(uuid) to anon',
             'O_TRUSTED_PREDECESSOR_PIN_MISMATCH', None),
            ('boundary_drift',
             "update erp.cp6_v2620o_rollback_capsule set boundary_snapshot=boundary_snapshot-'supplier_payments'",
             'O_BOUNDARY_SNAPSHOT_MISMATCH', None),
            ('post_install_boundary_history',
             "update erp.cp6_v2620n_rollback_capsule set boundary_snapshot=boundary_snapshot||'{\"o_post_use_probe\":true}'::jsonb",
             'O_POST_USE_ROLLBACK_REFUSED', None),
            ('coherent_capsule_and_checksum', None,
             'O_TRUSTED_PREDECESSOR_PIN_MISMATCH', coherent),
        )
        if tuple(case[0] for case in cases) != DIRECT_GUARD_NAMES:
            raise AssertionError('Direct O guard case manifest drift')
...
def verify_extra_preflight_guards() -> list[dict[str, Any]]:
    ...
    with n_guards.disposable_clone_confirmation(matrix.CLONE):
        for name, mutation, expected in EXTRA_FAULTS:
            ...
            try:
                matrix.prepare('O', 'REPORT', folder, source_generation='O')
                with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                ...
                if rejection is None or expected not in rejection:
                    raise AssertionError(
                        'O_EXTRA_PREFLIGHT_WRONG_REJECTION: ' + name
                    )
                observed = matrix.read_json_if_present(
                ...
                if observed.get('admission_closed') or observed.get('rollback_started'):
                    raise AssertionError(
                        'O_EXTRA_PREFLIGHT_MUTATED_ADMISSION: ' + name
                    )
                ...
                with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                    marker = conn.execute("""select count(*)
                      from erp.schema_migrations where version='v2.6.20o'""").fetchone()
                if admission != (True,) or marker != (1,):
                    raise AssertionError(
                        'O_EXTRA_PREFLIGHT_STATE_CHANGED: ' + name
                    )
                results.append({
...
def verify_exact_n_restore(target_pgurl: str) -> dict[str, Any]:
    expected = maintenance.TRUSTED_FUNCTIONS['O']
    with psycopg.connect(target_pgurl, autocommit=False) as conn, conn.cursor() as cur:
        ...
        cur.execute("""select
          not exists(select 1 from erp.schema_migrations where version='v2.6.20o')
          and exists(select 1 from erp.schema_migrations where version='v2.6.20n')
          ...
          and to_regclass('erp.cp6_v2620o_rollback_capsule') is null
          and to_regclass('erp.cp6_v2620n_rollback_capsule') is not null
        ...
        if cur.fetchone()[0] is not True:
            raise AssertionError('O rollback metadata/schema residue')
    return {
        'generation': 'N', 'restored_function_count': len(actual),
...
def run() -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        'classification': (
            'DISPOSABLE_NATIVE_O_TRUSTED_GUARDS_AND_CLOSED_ADMISSION_RESTORE'
        ),
    ...
    maintenance_result = maintenance.run_maintenance_rollback(
        target_name='O', target_pgurl=target_pgurl,
    ...
    if maintenance_result['status'] != 'PASS':
        raise AssertionError('O maintenance rollback failed')
    required_phases = {
    ...
    if not required_phases.issubset(observed_phases):
        raise AssertionError('O maintenance phase evidence incomplete')
    result.update(
        status='PASS',
        exact_pre_use_restore=verify_exact_n_restore(target_pgurl),
        maintenance={
            'status': 'PASS', 'target': 'O',
...
def main() -> None:
    ...
    try:
        ...
        result = {
            'head': os.environ.get('GITHUB_SHA', 'LOCAL_UNBOUND'),
            'status': 'FAIL',
            'error_code': 'V2620O_ROLLBACK_GUARD_FAILED',
