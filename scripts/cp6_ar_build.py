"""Three-function successor: serialize and reject cross-route opening overlap."""
from pathlib import Path
import copy,json
import cp6_ao_ap_build as package
import cp6_ao_ap_inventory as inventory
import cp6_aq_build as aq
from cp6_ar_definitions import OLD,FUNCTIONS

ROOT=Path(__file__).resolve().parents[1]
STAMP='20260922185015'
NAME='erp_v2_6_20ar_cp6_opening_overlap'
VERSION='v2.6.20ar'
CAP='erp.cp6_v2620ar_rollback_capsule'
MIGRATION=Path(f'supabase/migrations/{STAMP}_{NAME}.sql')
ROLLBACK=Path(f'supabase/rollbacks/{STAMP}_{NAME}.rollback.sql')
PINS=Path('docs/evidence/cp6-ar-pins.json')
PROVENANCE=Path('docs/evidence/cp6-ar-cli-provenance.json')
BASE_PINS_SHA='2f58f2b3ef00367e04c4ed2aa2f9418642452220b15139936c30608f700f8f9a'
q,j,sha=package.q,package.j,inventory.sha
INVENTORY_SQL=aq.INVENTORY_SQL.replace("'cp6_v2620aq_rollback_capsule')","'cp6_v2620aq_rollback_capsule','cp6_v2620ar_rollback_capsule')")
assert INVENTORY_SQL!=aq.INVENTORY_SQL

def model():
    assert sha(aq.PINS.read_bytes())==BASE_PINS_SHA
    aq_pin=json.loads(aq.PINS.read_text())
    assert sha(Path(aq.__file__).read_bytes())==aq_pin['builder_sha256']
    for key in ('migration','rollback'):assert sha(Path(aq_pin[key]).read_bytes())==aq_pin[key+'_sha256']
    base,capture,_,before,_=aq.model();after=copy.deepcopy(before);functions=[]
    ...
    pin=dict(version=VERSION,stamp=STAMP,name=NAME,migration=str(MIGRATION),rollback=str(ROLLBACK),capsule=CAP,previous='AQ',functions=functions)
...
def prior_guard(base,capture):
    ...
    result=aq.prior_guard(base,capture)+package.capsule_guard('AQ',pin)
    result+=f"""do $aq_platform$ begin
...
def build():
    base,capture,before,after,pin=model();prev=json.loads(aq.PINS.read_text());cap_name=CAP.split('.')[1]
    ...
    s='-- CP6 AR: cross-route opening identity and shared lock order.\n'+package.START
...
"""+guard(before['objects'],'AR_PREDECESSOR')+prior_guard(base,capture)
...
"""+''.join(new.rstrip(';\n')+';\n' for new in FUNCTIONS.values())+guard(after['objects'],'AR_INSTALLED')
...
do $after_data$ declare v_table text;v_hash jsonb;v_after jsonb; begin
 ...
 if v_after is distinct from (select boundary_snapshot->'before' from {CAP} limit 1) then raise exception 'AR_INSTALL_CHANGED_DATA';end if;
 update {CAP} set boundary_snapshot=boundary_snapshot||jsonb_build_object('after',v_after);
end $after_data$;
insert into erp.schema_migrations(version,description) values({q(VERSION)},'Prevent overlapping opening balances across legacy and import routes');
"""+package.capsule_guard('AR',pin)+'commit;\n'
    MIGRATION.write_text(s);pin['migration_sha256']=sha(s)
    s='-- CP6 AR: exact pre-use restore to AQ under closed admission.\n'+package.START
...
"""+guard(after['objects'],'AR_ROLLBACK')+prior_guard(base,capture)+package.capsule_guard('AR',pin)
    s+=f"""create temporary table cp6_ar_restore_boundary on commit drop as select boundary_snapshot from {CAP} limit 1;
do $pre_use$ declare v_table text;v_hash jsonb;v_after jsonb;b jsonb; begin
 select boundary_snapshot into b from {CAP} limit 1;
 {package.data_sql('v_after',['schema_migrations',cap_name])}
 if v_after is distinct from b->'after' then raise exception 'AR_POST_USE_ROLLBACK_REFUSED';end if;
 ...
 then raise exception 'AR_PRIOR_HISTORY_DRIFT';end if;
end $pre_use$;
...
"""+guard(before['objects'],'AR_RESTORED')
    s+=f"""do $restored_data$ declare v_table text;v_hash jsonb;v_before jsonb;b jsonb; begin
 select boundary_snapshot into b from pg_temp.cp6_ar_restore_boundary;
 ...
 then raise exception 'AR_EXACT_RESTORE_FAILED';end if;
end $restored_data$;
commit;
"""
    ROLLBACK.write_text(s);pin['rollback_sha256']=sha(s)
    pin.update(builder_sha256=sha(Path(__file__).read_bytes()),definitions_sha256=sha(Path('scripts/cp6_ar_definitions.py').read_bytes()),base_pins_sha256=BASE_PINS_SHA,production_go=False,independent_acceptance=False)
