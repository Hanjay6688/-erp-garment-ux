...
async function mapUser(owner,u,active=true){
  const role=await rpc(owner.token,'erp_save_role_v1',{p_payload:{code:'AP_'+randomBytes(6).toString('hex').toUpperCase(),name:'Disposable AP reader',
...
async function importFlow(){
  ...
  assert.deepEqual(native('state').ledger,before.ledger)
...
async function accessoryFlow(){
  ...
  assert.equal(native('state').accessory_stock,before.accessory_stock);assert.deepEqual(native('state').ledger,before.ledger)
  ...
  assert.equal(Number(native('state').accessory_stock),300);assert.deepEqual(native('state').ledger,before.ledger)
...
async function pocketFlow(){
  ...
  await mutate(page,'Sahkan pembatalan alokasi','pocket_fabric','CANCEL_PERIOD');assert.deepEqual(native('state').ledger,issued.ledger);assert.equal(native('state').pocket_stock,issued.pocket_stock)
  ...
  assert.equal(Number(native('state').pocket_stock),20);assert.deepEqual(native('state').ledger,before.ledger)
