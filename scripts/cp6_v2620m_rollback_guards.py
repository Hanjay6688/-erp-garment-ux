#!/usr/bin/env python3
"""Trusted M capsule guards plus maintenance-only exact M-to-L restore."""
...
REPORT = Path('cp6-proof/CP6_V2620M_ROLLBACK_GUARDS.json')
MAINTENANCE_REPORT = Path('cp6-proof/V2620M_MAIN_MAINTENANCE_ROLLBACK.json')
CLONE_ROOT = Path('cp6-proof/M_TRUSTED_CAPSULE_GUARD')
DIRECT_M_GUARD_NAMES = (
    ...
    with psycopg.connect(pgurl, autocommit=False) as conn, conn.cursor() as cur:
        cur.execute(
            """select object_regidentity,object_definition,definition_sha256
               from erp.cp6_v2620m_rollback_capsule
        ...
        cur.execute(
            """update erp.cp6_v2620m_rollback_capsule
...
def verify_m_capsule_fault() -> dict[str, Any]:
    """Return public verification outcomes, never capsule contents or credentials."""
    CLONE_ROOT.mkdir(parents=True, exist_ok=True)
    marker = maintenance.TARGETS['M']['marker']
    with disposable_clone_confirmation(matrix.CLONE):
        try:
            matrix.prepare('M', 'REPORT', CLONE_ROOT, source_generation='M')
            ...
            if rejection is None or 'TRUSTED_PREDECESSOR_PIN_MISMATCH' not in rejection:
                raise AssertionError(f'M coherent capsule fault was not rejected: {rejection}')
            ...
            if observed.get('admission_closed') or observed.get('rollback_started'):
                raise AssertionError(f'M trust failure crossed mutation boundary: {observed}')
            ...
            if marker_count != 1:
                raise AssertionError('M trusted guard changed installed generation')
            return {
                'target': 'M',
                'status': 'PASS',
...
def direct_m_guards(target_pgurl: str) -> list[dict[str, Any]]:
    ...
    with psycopg.connect(target_pgurl, autocommit=False) as conn:
        ...
        cases = (
            (
                'platform_bytes',
                "update supabase_migrations.schema_migrations set statements=array['tampered M'] where name='erp_v2_6_20m_cp6_subledger_exact_cent_closure'",
            ...
            (
                'definition_drift',
                'alter function erp._v268_financial_report_checks_pre_scope() cost 999',
                'TRUSTED_PREDECESSOR_PIN_MISMATCH',
                None,
            ),
            (
                'acl_drift',
                'grant execute on function erp._v268_financial_report_checks_pre_scope() to anon',
                'TRUSTED_PREDECESSOR_PIN_MISMATCH',
                None,
            ),
            (
                'boundary_drift',
                "update erp.cp6_v2620m_rollback_capsule set boundary_snapshot=boundary_snapshot-'sales_payments'",
        ...
        if tuple(case[0] for case in cases) != DIRECT_M_GUARD_NAMES:
            raise AssertionError('Direct M guard case manifest drift')
    ...
    return [
        ...
        for name in DIRECT_M_GUARD_NAMES
    ]


def verify_exact_l_restore(target_pgurl: str) -> dict[str, Any]:
    ...
    with psycopg.connect(target_pgurl, autocommit=False) as conn, conn.cursor() as cur:
        ...
        cur.execute(
            """select not exists(select 1 from erp.schema_migrations where version='v2.6.20m')
              and exists(select 1 from erp.schema_migrations where version='v2.6.20l')
              ...
              and to_regclass('erp.cp6_v2620m_rollback_capsule') is null
              ...
              and to_regclass('erp.cp6_v2620l_rollback_capsule') is not null"""
        )
        if cur.fetchone()[0] is not True:
            raise AssertionError('M rollback metadata/schema residue')
    return {
        'generation': 'L',
...
def run() -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        'classification': 'DISPOSABLE_NATIVE_M_TRUSTED_GUARDS_AND_CLOSED_ADMISSION_RESTORE',
        'rollback_sha256': hashlib.sha256(ROLLBACK.read_bytes()).hexdigest(),
        'production_go': False,
    }
    result['trusted_capsule_guard'] = verify_m_capsule_fault()
    result['guards'] = direct_m_guards(target_pgurl)
    ...
    if maintenance_result['status'] != 'PASS':
        raise AssertionError(f'M maintenance rollback failed: {maintenance_result}')
    required_phases = {
    ...
    if not required_phases.issubset(observed_phases):
        raise AssertionError('M maintenance phase evidence incomplete')
    result.update(
        status='PASS',
        exact_pre_use_restore=verify_exact_l_restore(target_pgurl),
        maintenance={
            'status': 'PASS',
            'target': 'M',
...
def main() -> None:
    ...
    try:
        ...
        result = {
            'head': os.environ.get('GITHUB_SHA', 'LOCAL_UNBOUND'),
            'status': 'FAIL',
            'error_code': 'V2620M_ROLLBACK_GUARD_FAILED',
