...
def runtime(cur):
    ...
    return {'engine': engine, 'versions': versions + (['v2.6.20k'] if k_successor else []) + (['v2.6.20l'] if l_successor else []) + (['v2.6.20m'] if m_successor else []) + (['v2.6.20n'] if any('pre_n_installed_sha256' in x for x in m_successor.values()) else []) + (['v2.6.20o'] if any('pre_o_installed_sha256' in x for x in m_successor.values()) else []) + (['v2.6.20p'] if any('pre_p_installed_sha256' in x for x in m_successor.values()) else []), 'capsule': definitions,
