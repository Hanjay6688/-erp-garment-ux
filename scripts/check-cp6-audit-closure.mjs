...
for (const token of [
  ...
  'negative_mutation_transaction_residue_zero', 'auth_identity_and_clone_transient_residue_zero',
  'explicit_private_schema_and_sql_acl_denial', "'Accept-Profile': 'erp'",
  'confirm_high_risk: true',
  "assert.notEqual(new URL(restBaseUrl).hostname, 'vlxdhpkjeevubjxexnfo.supabase.co'",
  'production_go: false',
...
for (const token of [
  ...
  "po.po_number='CP6-MX-REVQC_INVOICE_REV'", "qh.status<>'REVERSED'",
  "execute 'set local role authenticated'", 'public.erp_get_laundry_qc_workspace_v1',
  "execute 'reset role'",
  'idx_products_qc_model_size_effective_v2620b',
  'public.erp_search_final_sku_products_v1', 'v_generated_seen<>700',
  'Beyond-first-500 SKU posted through authoritative facade', "'ROLLBACK_ONLY'",
