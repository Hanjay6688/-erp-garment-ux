...
let previous=-1
for(const token of ['Apply v2.6.20q exact supplier invoice quantity','Reproduce independent Q receipt invoice dependency','Apply v2.6.20r receipt invoice dependency','Run post-CP6 real Auth','Prove native R receipt invoice dependency','Prove native R invoice posting versus receipt reversal','Qualify exact F G H I J K L M N O P Q R S and T rollback','Prove trusted R capsule','Prove trusted Q capsule']) {const at=workflow.indexOf(token);assert.ok(at>previous,token);previous=at}
for(const token of ["len(schedules['cases'])==300", "'expected':75,'observed':75", "setup_unit['completed_case_count']==setup_unit['expected_case_count']==134", "sum(c['upgrade_guard']['invalid_history_refused'] for c in r_before['cases'].values())==4", "CP6_V2620T_RUNTIME_MANIFEST.json", "CP6_V2620R_RECEIPT_INVOICE_DEPENDENCY_REGRESSION.json", "PHYSICAL_DISPOSABLE_CP6_AUTH_CLONE_AFTER_V2620T", "CP6_M_RACE_SOURCE_GENERATION: T", "len(rr['cases'])==2"]) assert.ok(workflow.includes(token),token)
...
for(const token of ['INVOICE_POST_FIRST','RECEIPT_REVERSE_FIRST','pg_blocking_pids(pid)','contender_business_boundary_unchanged',"matrix.verify_setup_source('T')"]) assert.ok(races.includes(token),token)
console.log(JSON.stringify({status:'PASS',classification:'STATIC_SOURCE_CONTRACT_NOT_NATIVE_PROOF',replaced_functions:3,boundary_tables:68,native_cases:13,known_q_paths:5,controls:8,receipt_invoice_native_races:2,maintenance_schedules:300,writer_body_entries:75,production_go:false}))
