#!/usr/bin/env python3
"""Twenty AE schedules using the unchanged established concurrency oracle."""
...
ROOT = Path("cp6-proof/AE_MAINTENANCE_ROLLBACK")
REPORT = ROOT / "manifest.json"


def prepare(target, operation, folder, *, source_generation):
    """Bind the old oracle's fixture setup to AE -> AD, with exact verification."""
    if target != "AE" or source_generation != "AE":
        raise AssertionError("AE_SCHEDULE_REQUIRES_EXACT_EDGE")
    matrix.command(
    ...
    with matrix.legacy.connect("ae-clone-source") as conn, conn.cursor() as cur:
        if len(runtime.verified_successor(cur)) != 276:
            raise AssertionError("AE_CLONE_RUNTIME_MISMATCH")
    matrix.maintenance_strip("AE", folder)
    with matrix.legacy.connect("ae-predecessor-fixture") as conn, conn.cursor() as cur:
    ...
    with matrix.legacy.connect("ae-exact-platform-ledger") as conn, conn.cursor() as cur:
        cur.execute(
        ...
        if len(runtime.verified_successor(cur)) != 276:
            raise AssertionError("AE_SEEDED_RUNTIME_MISMATCH")
        conn.commit()
    capsule = matrix.legacy.capture_capsule("AE")
    if len(capsule) != 2:
        raise AssertionError("AE_SCHEDULE_CAPSULE_CARDINALITY")
...
def run():
    ...
    if actual != expected or os.environ.get("CP6_AE_MAINTENANCE_CONFIRM") != "cp6_rollback":
        raise AssertionError("AE_MAINTENANCE_EXACT_DISPOSABLE_ENVIRONMENT_REQUIRED")
    with psycopg.connect(matrix.SOURCE) as conn, conn.cursor() as cur:
        if len(runtime.verified_successor(cur)) != 276:
            raise AssertionError("AE_MAINTENANCE_SOURCE_MISMATCH")
    matrix.TARGETS["AE"] = (runtime.STAMP, runtime.NAME, runtime.VERSION, "v2.6.20ad", 2)
    original_prepare = matrix.prepare
    matrix.prepare = prepare
    result = {
        "format": "CP6_AE_MAINTENANCE_SCHEDULES_V1", "status": "INCOMPLETE",
    ...
    try:
        for operation in matrix.OPERATIONS:
            for mode in matrix.MODES:
                name = f"AE-{operation}-{mode}"
                folder = ROOT / name
                folder.mkdir(parents=True, exist_ok=True)
                try:
                    evidence = matrix.run_case("AE", operation, mode, folder, source_generation="AE")
                except Exception as exc:
                    evidence = {
                        "status": "FAIL", "target": "AE", "operation": operation,
