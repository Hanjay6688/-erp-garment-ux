#!/usr/bin/env python3
"""Restore AG to the exact 533-function / 221-table AF predecessor."""
...
ROOT = Path("cp6-proof/writer-ag")
REPORT = ROOT / "AG_EXACT_AF_RESTORE.json"
...
def run() -> dict:
    ...
    if os.environ.get("PGURL") != EXPECTED_URL:
        raise AssertionError("AG_EXACT_DISPOSABLE_RESTORE_REQUIRED")
    if (
        os.environ.get("CP6_MAINTENANCE_CONFIRM_DATABASE") != "postgres"
        or os.environ.get("CP6_MAINTENANCE_ALLOW_SYSTEM_DATABASE") != "postgres"
    ):
        raise AssertionError("AG_EXPLICIT_DISPOSABLE_SYSTEM_DATABASE_REQUIRED")
    with psycopg.connect(EXPECTED_URL) as connection, connection.cursor() as cur:
        if len(runtime.verified_successor(cur)) != 690:
            raise AssertionError("AG_RESTORE_SOURCE_MISMATCH")

    controls = {
        "FUNCTION_CONFIGURATION": ("alter function erp.guard_child_by_parent_status() set work_mem='64MB'", "AG_TRUSTED_PREDECESSOR_PIN_MISMATCH"),
        "FUNCTION_ACL": ("grant execute on function erp.guard_child_by_parent_status() to anon", "AG_TRUSTED_PREDECESSOR_PIN_MISMATCH"),
        "FUNCTION_OWNER": ("alter function erp.guard_child_by_parent_status() owner to supabase_admin", "AG_TRUSTED_PREDECESSOR_PIN_MISMATCH"),
        "UNEXPECTED_TABLE": ("create table erp.cp6_ag_unexpected(id integer)", "AG_BOUNDARY_SNAPSHOT_MISMATCH"),
        "MISSING_TABLE": ("drop table erp.cp6_v2620ab_rollback_capsule", "AG_BOUNDARY_SNAPSHOT_MISMATCH"),
        "POST_USE": ("insert into erp.locations(location_code,location_name,location_type) values('AG-ROLLBACK-PROBE','AG probe','FG_WAREHOUSE')", "AG_POST_USE_ROLLBACK_REFUSED"),
        "CAPSULE_DEFINITION": (f"update {runtime.CAPSULE} set object_definition=object_definition||E'\\n-- altered'", "AG_TRUSTED_PREDECESSOR_PIN_MISMATCH"),
        "PLATFORM_SOURCE": (f"update supabase_migrations.schema_migrations set statements=array['wrong'] where name='{runtime.NAME}'", "AG_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR"),
    }
    ...
    with psycopg.connect(EXPECTED_URL.replace("postgres:postgres@", "supabase_admin:postgres@")) as connection, connection.cursor() as cur:
        ...
        for name, (mutation, error) in controls.items():
            ...
            cur.execute("savepoint ag_atomic")
            try:
                ...
                cur.execute("rollback to savepoint ag_atomic;release savepoint ag_atomic")
    ...
    if len(atomic["cases"]) == 8 and all(item["status"] == "PASS" for item in atomic["cases"].values()):
        atomic["status"] = "PASS"
    (ROOT / "AG_ROLLBACK_ATOMIC_CONTROLS.json").write_text(json.dumps(atomic, indent=2) + "\n")
    if atomic["status"] != "PASS":
        raise AssertionError("AG_ROLLBACK_ATOMIC_CONTROLS_FAILED:" + json.dumps(atomic))
    ...
    result = maintenance.run_maintenance_rollback(
        ...
        report_path=ROOT / "AG_MAIN_MAINTENANCE_ROLLBACK.json",
        drain_timeout=10,
        natural_grace=0,
    ...
    if not required.issubset({item["phase"] for item in result["phases"]}):
        raise AssertionError("AG_MAINTENANCE_PHASES_INCOMPLETE")

    before_catalog = json.loads((ROOT / "AF_COMPLETE_CATALOG.json").read_text())
    before_boundary = json.loads((ROOT / "AF_COMPLETE_BOUNDARY.json").read_text())
    ...
    final = {
        ...
        "new_edge": "AG -> AF", "production_go": False,
    }
