#!/usr/bin/env python3
"""Trusted X capsule guards and maintenance-only exact X-to-W restore."""
...
def coherent_capsule_fault(pgurl: str) -> None:
    with psycopg.connect(pgurl, autocommit=False) as conn, conn.cursor() as cur:
        cur.execute("""select object_regidentity,object_definition
          from erp.cp6_v2620x_rollback_capsule
...
def verify_capsule_fault() -> dict[str, Any]:
    ...
    with n_guards.disposable_clone_confirmation(matrix.CLONE):
        try:
            matrix.prepare('X', 'REPORT', CLONE_ROOT, source_generation='X')
            coherent_capsule_fault(matrix.CLONE)
            rejection = None
            try:
                maintenance.run_maintenance_rollback(
                    target_name='X', target_pgurl=matrix.CLONE,
...
def direct_guards(target_pgurl: str) -> list[dict[str, Any]]:
    ...
    with psycopg.connect(target_pgurl, autocommit=False) as conn:
        ...
        cases = (
            ('platform_bytes',
             "update supabase_migrations.schema_migrations set statements=array['tampered W'] where name='erp_v2_6_20x_cp6_internal_role_fail_closed'",
             'X_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR', None),
            ('successor',
             ...
             'X_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR', None),
            ('definition_drift',
             'alter function erp.require_internal() cost 999',
             'X_TRUSTED_PREDECESSOR_PIN_MISMATCH', None),
            ('acl_drift',
             'grant execute on function erp.require_internal() to anon',
             'X_TRUSTED_PREDECESSOR_PIN_MISMATCH', None),
            ('boundary_drift',
             "update erp.cp6_v2620x_rollback_capsule set boundary_snapshot=boundary_snapshot-'supplier_payments'",
             'X_BOUNDARY_SNAPSHOT_MISMATCH', None),
...
def verify_extra_preflight_guards() -> list[dict[str, Any]]:
    ...
    with n_guards.disposable_clone_confirmation(matrix.CLONE):
        for name, mutation, expected in EXTRA_FAULTS:
            ...
            try:
                matrix.prepare('X', 'REPORT', folder, source_generation='X')
                with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                ...
                if rejection is None or expected not in rejection:
                    raise AssertionError(
                        'X_EXTRA_PREFLIGHT_WRONG_REJECTION: ' + name
                    )
                observed = matrix.read_json_if_present(
                ...
                if observed.get('admission_closed') or observed.get('rollback_started'):
                    raise AssertionError(
                        'X_EXTRA_PREFLIGHT_MUTATED_ADMISSION: ' + name
                    )
                ...
                with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                    marker = conn.execute("""select count(*)
                      from erp.schema_migrations where version='v2.6.20x'""").fetchone()
                if admission != (True,) or marker != (1,):
                    raise AssertionError(
                        'X_EXTRA_PREFLIGHT_STATE_CHANGED: ' + name
                    )
                results.append({
...
def verify_exact_w_restore(target_pgurl: str) -> dict[str, Any]:
    expected = maintenance.TRUSTED_FUNCTIONS['X']
    with psycopg.connect(target_pgurl, autocommit=False) as conn, conn.cursor() as cur:
        ...
        cur.execute("""select
          not exists(select 1 from erp.schema_migrations where version='v2.6.20x')
          and exists(select 1 from erp.schema_migrations where version='v2.6.20w')
          and not exists(select 1 from supabase_migrations.schema_migrations
            where name='erp_v2_6_20x_cp6_internal_role_fail_closed')
          and to_regclass('erp.cp6_v2620x_rollback_capsule') is null
    ...
    return {
        'generation': 'W', 'restored_function_count': len(actual),
...
def run() -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        'classification': (
            'DISPOSABLE_NATIVE_X_TRUSTED_GUARDS_AND_CLOSED_ADMISSION_RESTORE'
        ),
    ...
    maintenance_result = maintenance.run_maintenance_rollback(
        target_name='X', target_pgurl=target_pgurl,
    ...
    result.update(
        status='PASS',
        exact_pre_use_restore=verify_exact_w_restore(target_pgurl),
        maintenance={
            'status': 'PASS', 'target': 'X',
