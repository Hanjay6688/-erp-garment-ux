...
def refused(cur,operation):
 actors.admin(cur);before=actors.boundary(cur);cur.execute('savepoint al_reject');error=None
 try: operation()
 except psycopg.Error as exc:error=dict(sqlstate=exc.sqlstate,message=exc.diag.message_primary)
 finally:cur.execute('rollback to savepoint al_reject');actors.admin(cur);cur.execute('release savepoint al_reject')
...
def batch_new(cur,today):
 owner(cur);return cur.execute('select erp.create_migration_batch(%s,%s,null,null)',
  ('AL-'+uuid.uuid4().hex,gaps.production.at(today-timedelta(days=1),0))).fetchone()[0]
...
def master_reference(cur,today,entity,field):
 batch=batch_new(cur,today);actors.admin(cur);tag='AL-'+uuid.uuid4().hex[:14]
...
def staged_material(cur,today,invalid=False,duplicate=False):
 batch=batch_new(cur,today);tag='AL-'+uuid.uuid4().hex[:14];actors.admin(cur)
 location='RAW-'+tag
...
def run(old=False):
 ...
 report=dict(status='INCOMPLETE',writer_head=head,writer_tree=tree,runtime_generation='AK' if old else 'AL',
