...
assert.equal(evidence.counts.navigation_labels + 2, navMap.size)
assert.equal(evidence.counts.protected_routes + 2, pageMap.size)
assert.equal(navMap.get('Kain kantong'), 'warehouse.stock.adjust')
assert.equal(pageMap.get('pocket-fabric'), 'warehouse.stock.adjust')
...
assert.ok(rpcBoundaries.has('src/ConnectedAccessoryIssuePage.tsx:erp_save_accessory_issue_action_v1'))
assert.ok(rpcBoundaries.has('src/ConnectedAccessoryServicePage.tsx:erp_get_accessory_service_workspace_v1'))
assert.ok(rpcBoundaries.has('src/ConnectedAccessoryServicePage.tsx:erp_save_accessory_service_action_v1'))
assert.equal(rpcBoundaries.size, evidence.counts.browser_rpc_boundaries + 13)
