...
for (const token of [
  "'U': ('20260913070000'", "'expected_case_count': 340",
  "source_generation: str = 'V'",
]) assert.ok(matrix.includes(token), token)
const setup = read('scripts/cp6_rollback_setup_unit.py')
for (const token of ["'T', 'U'", 'assert len(cases) == 169', "'expected_case_count': 169"]) {
...
for (const path of [
  ...
  for (const token of ["SOURCE_GENERATION = 'V'",
...
for (const folder of ['R_RECEIPT_INVOICE_NATIVE_RACES', 'T_MATERIAL_ADJUSTMENT_NATIVE_RACES']) {
  const command = `test "$(jq -r '.source_generation' cp6-proof/${folder}/manifest.json)" = 'V'`
...
for (const token of [
  ...
  'CP6_V2620V_RUNTIME_MANIFEST.json',
...
console.log(JSON.stringify({
  ...
  known_t_paths: 4, controls: 1, maintenance_schedules: 340,
  writer_body_entries: 85, rollback_setup_units: 169, production_go: false,
}))
