#!/usr/bin/env python3
"""Twenty AF schedules using the unchanged established concurrency oracle."""
...
ROOT = Path("cp6-proof/AF_MAINTENANCE_ROLLBACK")
REPORT = ROOT / "manifest.json"


def prepare(target, operation, folder, *, source_generation):
    """Bind the old oracle's fixture setup to AF -> AE, with exact verification."""
    if target != "AF" or source_generation != "AF":
        raise AssertionError("AF_SCHEDULE_REQUIRES_EXACT_EDGE")
    matrix.command(
    ...
    with matrix.legacy.connect("af-clone-source") as conn, conn.cursor() as cur:
        if len(runtime.verified_successor(cur)) != 278:
            raise AssertionError("AF_CLONE_RUNTIME_MISMATCH")
    matrix.maintenance_strip("AF", folder)
    with matrix.legacy.connect("af-predecessor-fixture") as conn, conn.cursor() as cur:
    ...
    with matrix.legacy.connect("af-exact-platform-ledger") as conn, conn.cursor() as cur:
        cur.execute(
        ...
        if len(runtime.verified_successor(cur)) != 278:
            raise AssertionError("AF_SEEDED_RUNTIME_MISMATCH")
        conn.commit()
    capsule = matrix.legacy.capture_capsule("AF")
    if len(capsule) != 2:
        raise AssertionError("AF_SCHEDULE_CAPSULE_CARDINALITY")
...
def run():
    ...
    if actual != expected or os.environ.get("CP6_AF_MAINTENANCE_CONFIRM") != "cp6_rollback":
        raise AssertionError("AF_MAINTENANCE_EXACT_DISPOSABLE_ENVIRONMENT_REQUIRED")
    with psycopg.connect(matrix.SOURCE) as conn, conn.cursor() as cur:
        if len(runtime.verified_successor(cur)) != 278:
            raise AssertionError("AF_MAINTENANCE_SOURCE_MISMATCH")
    matrix.TARGETS["AF"] = (runtime.STAMP, runtime.NAME, runtime.VERSION, "v2.6.20ae", 2)
    original_prepare = matrix.prepare
    matrix.prepare = prepare
    result = {
        "format": "CP6_AF_MAINTENANCE_SCHEDULES_V1", "status": "INCOMPLETE",
    ...
    try:
        for operation in matrix.OPERATIONS:
            for mode in matrix.MODES:
                name = f"AF-{operation}-{mode}"
                folder = ROOT / name
                folder.mkdir(parents=True, exist_ok=True)
                try:
                    evidence = matrix.run_case("AF", operation, mode, folder, source_generation="AF")
                except Exception as exc:
                    evidence = {
                        "status": "FAIL", "target": "AF", "operation": operation,
