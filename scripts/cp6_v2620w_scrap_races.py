#!/usr/bin/env python3
"""Real scrap document locks, duplicate post and writer-abort qualification."""
import json
import os
import threading
...
import cp6_v2620w_runtime as runtime
import cp6_v2620w_scrap_business_date_regression as proof
...
def session(name):
    ...
    if params != dict(user='postgres',password='postgres',host='127.0.0.1',port='54322',dbname='cp6_rollback'):
        raise AssertionError('W_RACE_CANONICAL_CLONE_REQUIRED')
    conn = psycopg.connect(**dict(params,user='supabase_admin'),application_name='cp6-w-'+name)
    with conn.cursor() as cur:
        ...
        if not proof.one(cur,'select rolsuper from pg_roles where rolname=current_user'):
            raise AssertionError('W_RACE_EXISTING_DISPOSABLE_ADMIN_REQUIRED')
    conn.commit()
    return conn
...
def operator(cur, timezone):
    ...
    cur.execute("select set_config('app.change_reason','W real-session scrap race',true)")
...
def run_case(name, folder):
    ...
    with session('fixture') as conn, conn.cursor() as cur:
        if len(runtime.verified_successor(cur)) != 3:
            raise AssertionError('W_RACE_EXACT_RUNTIME_REQUIRED')
        if proof.AUTH_SOURCE.read_text().count('grant usage on schema public, erp to authenticated;') != 1:
            raise AssertionError('W_RACE_SCHEMA_ALIGNMENT_SOURCE_REQUIRED')
        if not proof.one(cur,"select has_schema_privilege('authenticated','erp','USAGE')"):
            cur.execute('grant usage on schema erp to authenticated')
        cur.execute("select set_config('request.jwt.claims',%s,true)", (json.dumps(dict(sub=base.OPERATOR_AUTH,role='authenticated')),))
        cur.execute("select set_config('app.change_reason','W race fixture',true)")
    ...
    query="select erp.reverse_scrap_sale(%s,'W serialized linked inverse')" if reverse else 'select erp.post_scrap_sale(%s)'
    try:
        with first.cursor() as cur:
            operator(cur,'UTC');cur.execute('select erp.post_scrap_sale(%s)',(transaction,))
        ...
        with session('observer') as observer:
            def observation():
                ...
                observer.commit()
                function='erp.reverse_scrap_sale' if reverse else 'erp.post_scrap_sale'
            ...
            if not blocked['real_business_query_observed'] or outcome:
                raise AssertionError('W_RACE_NOT_INSIDE_REAL_DOCUMENT_CALL')
        if abort:first.rollback()
        else:first.commit()
        thread.join(25)
        if thread.is_alive():raise AssertionError('W_RACE_CONTENDER_DID_NOT_FINISH')
        expected_outcome={'status':'REJECTED','sqlstate':'P0001','message':'Scrap sale must be DRAFT'} if name=='POST_THEN_POST' else {'status':'COMMITTED'}
        if outcome!=expected_outcome:raise AssertionError('W_RACE_WRITER_OR_ABORT_ORACLE:'+str(outcome))
        with session('reconcile') as conn,conn.cursor() as cur:
            ...
            state=proof.one(cur,'select status from erp.scrap_sales where id=%s',(transaction,))
            if state!=('REVERSED' if reverse else 'POSTED') or len(rows)!=(2 if reverse else 1):
                raise AssertionError('W_RACE_DOCUMENT_OR_JOURNAL_CARDINALITY')
            original=next(r for r in rows if r['source_type']=='SCRAP_SALE')
            if original['economic_date']!='2026-09-03' or original['transaction_date']!='2026-09-03':
                raise AssertionError('W_RACE_POST_DATE_WRONG')
            if any(r['debit']!='0.03' or r['credit']!='0.03' for r in rows):
                raise AssertionError('W_RACE_EXACT_CENTS_WRONG')
            if reverse:
                ...
                if inverse['reversal_of_id']!=original['id'] or inverse['economic_date']!=today:
                    raise AssertionError('W_RACE_LINKED_INVERSE_OR_DATE')
            deltas={}
            for day in before:
                ...
                if actual!=expected or after[day]['data_confidence']['status']!='READY':
                    raise AssertionError('W_RACE_CASH_DATE_OR_REPORT_CONFIDENCE')
...
def main():
    ...
    if tuple(os.environ.get(k) for k in ('PGURL','CP6_MAINTENANCE_PGURL','CP6_ROLLBACK_RACE_PGURL',
        ...
        raise SystemExit('W_RACE_DISPOSABLE_ENDPOINT_CONFIRMATION_REQUIRED')
    ROOT.mkdir(parents=True,exist_ok=True)
    report=dict(head=os.environ.get('GITHUB_SHA','LOCAL_UNBOUND'),production_go=False,
        classification='NATIVE_POSTGRESQL_REAL_SCRAP_SALE_DOCUMENT_LOCKS',source_generation=SOURCE_GENERATION,cases=[])
    for name in CASES:
