#!/usr/bin/env python3
"""Read-only Supabase security advisors, compared on exact AK and AL."""
...
def run(phase):
 ...
 result=dict(status='READ_ONLY_CAPTURED',phase=phase,writer_head=head,writer_tree=tree,
  runtime='AK' if phase=='before' else 'AL',cli_version='2.116.0',findings=observed,raw=parsed,boundary_unchanged=True,production_go=False)
 if phase=='after':
  ...
  for finding in added:
   ...
   if finding['name']=='rls_enabled_no_policy' and meta.get('schema')=='erp' and meta.get('name')=='cp6_v2620al_rollback_capsule':allowed.append(finding)
