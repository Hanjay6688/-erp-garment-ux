...
receipt day) or on the open day (closed history); FG and COGS in the report of day d are untouched by the invoice and the
report of d+1 moves by exactly 3x and 2x; the AW engine shows no negative daily inventory balance and is READY; the close
is accepted afterwards. Phase 'before' (without AY) must show the finding on the open lower-price case.
"""
...
def late_invoice(cur,today,price,closed,zone='Asia/Jakarta'):
    ...
    closed_before=awp.close(cur,d2,'AY close through the FG and sale day') if closed else None
    before=prod.observe(cur,fx,today)
    pre_errors=prod.differences(before,10,True,False)
    day_d_before=awp.snapshot_values(cur,d);day_d2_before=awp.snapshot_values(cur,d2)
    ...
    day_d_after=awp.snapshot_values(cur,d);day_d2_after=awp.snapshot_values(cur,d2)
    ...
    close_after=awp.close(cur,d2,'AY close after the late invoice') if not closed else ('ALREADY_CLOSED',None)
    x=dec(price)-10
    ...
    if closed:checks['closed_day_gl_unchanged']=day_d2_before==day_d2_after
    ok=all(checks.values())
    status='PASS' if ok else ('COUNTEREXAMPLE' if not ay_installed(cur) else 'FAIL')
    return dict(status=status,price=price,receipt_day=str(d),fg_and_sale_day=str(d2),closed_through_before_invoice=str(d2) if closed else None,
...
def cases(cur,today):
    return [('AY:LATE_INVOICE_LOWER_OPEN_RECEIPT_DAY',lambda:late_invoice(cur,today,'8.25',False)),
            ...
            ('AY:LATE_INVOICE_LOWER_CLOSED_HISTORY',lambda:late_invoice(cur,today,'8.25',True)),
            ('AY:LATE_INVOICE_HIGHER_CLOSED_HISTORY',lambda:late_invoice(cur,today,'10.70',True))]
