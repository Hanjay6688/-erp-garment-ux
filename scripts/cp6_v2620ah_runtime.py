#!/usr/bin/env python3
"""AH pins all 533 live functions and the 157 unchanged AC relation objects."""
from pathlib import Path
import hashlib,json,os,subprocess
import cp6_v2620ac_runtime as ac
import cp6_v2620ag_runtime as predecessor
...
PINS_SHA256='3ec72b36826569bd4793ceef600aeabdae87a82f5b623221ed0913c2638beb6f'
SOURCE_BASE='961c592e22b6582ba7c5b07d612e32a9dbbae28b'

def pins():
 raw=PINS.read_bytes();assert hashlib.sha256(raw).hexdigest()==PINS_SHA256,'AH_TRUSTED_PINS_CHANGED'
 p=json.loads(raw)
 assert (p['predecessor_head'],p['predecessor_tree'],p['boundary_count'],p['predecessor_table_count'])==(PREDECESSOR_HEAD,PREDECESSOR_TREE,220,222)
 return p
...
def verify_audit_source():
 from cp6_ah_gate_scope import verify
 ...
 assert os.environ.get('GITHUB_SHA')==git('rev-parse','HEAD'),'AH_NATIVE_HEAD_REQUIRED'
...
def catalog(cur,successor):
 ...
 assert len(observed)==533 and {r[0] for r in observed}==set(expected),'AH_COMPLETE_FUNCTION_SET_DRIFT'
 result={}
 for identity,digest,owner,acl in observed:
  e=expected[identity];assert (digest,owner,acl)==(e['sha256'],e['owner'],e['acl']),'AH_LIVE_FUNCTION_DRIFT:'+identity
...
def verify_predecessor(cur):
 ...
 catalog(cur,False)
 assert not cur.execute("select to_regclass(%s) is not null or exists(select 1 from erp.schema_migrations where version=%s) or exists(select 1 from supabase_migrations.schema_migrations where name=%s)",(CAPSULE,VERSION,NAME)).fetchone()[0],'AH_PREDECESSOR_RESIDUE'
 return True
...
def verified_successor(cur,*,pre_admission=False):
 ...
 for identity,before,actual,after,owner,acl,snapshot in rows:
  e=expected[identity]
  assert (before,actual,after,owner,acl)==(e['predecessor_sha256'],e['predecessor_sha256'],e['installed_sha256'],e['owner'],e['acl']) and snapshot==boundary,'AH_CAPSULE_DRIFT:'+identity
 # Historical capsules are immutable dependencies captured in the AH boundary.
 ...
 try:
  ...
  names=cur.execute("select relname from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='erp' and c.relkind='r' and relname like 'cp6%rollback_capsule' and relname<>'cp6_v2620ah_rollback_capsule' order by 1").fetchall()
  for (name,) in names:
   ...
   assert digest==boundary[name],'AH_INHERITED_CAPSULE_DRIFT:'+name
 ...
 assert len(objects)==690,'AH_RUNTIME_CARDINALITY'
 return objects
