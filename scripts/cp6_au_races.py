...
def run(today,first,commit,admin):
 ...
 def action(cur,kind):
  if kind=='VALIDATION':
   api.ordinary(cur)
   api.admin(cur)  # Private helper runs inside privileged legacy writers.
 ...
 with psycopg.connect(admin) as holder,holder.cursor() as cur,ThreadPoolExecutor(max_workers=1) as pool:
  ...
  with psycopg.connect(admin) as observer,observer.cursor() as check:
 ...
 return dict(status='PASS',first=first,first_committed=commit,blocking_observed=False,
     ...
     actions='private validator with owner claims versus ordinary master' if first=='VALIDATION' else 'ordinary authenticated master and WIP API',
     output_posted=output_committed,identity_history_preserved=True,
