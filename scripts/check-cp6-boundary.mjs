...
assert.match(packageJson, /"check:cp6": "node scripts\/check-cp6-boundary\.mjs && node scripts\/check-cp6-audit-closure\.mjs && node scripts\/check-cp6-reaudit-closure\.mjs && node scripts\/check-cp6-deep-business-repair\.mjs && node scripts\/check-cp6-reaudit-business-closure\.mjs && node scripts\/check-cp6-final-runtime-reliability\.mjs && node scripts\/check-cp6-independent-closure\.mjs && node scripts\/check-cp6-expanded-audit-closure\.mjs && node scripts\/check-cp6-h2-audit-closure\.mjs && node scripts\/check-cp6-payment-fact-closure\.mjs && node scripts\/check-cp6-payment-date-conservation\.mjs && node scripts\/check-cp6-exact-ledger-conservation\.mjs && node scripts\/check-cp6-subledger-closure\.mjs && node scripts\/check-cp6-supplier-cent-lifecycle\.mjs && node scripts\/check-cp6-supplier-return-allocation\.mjs && node scripts\/check-cp6-supplier-return-match-state\.mjs && node scripts\/check-cp6-supplier-invoice-exact-quantity\.mjs && node scripts\/check-cp6-receipt-invoice-dependency\.mjs && node scripts\/check-cp6-supplier-payment-business-date\.mjs && node scripts\/check-cp6-material-adjustment-revaluation\.mjs && node scripts\/check-cp6-canonical-business-date\.mjs && node scripts\/check-cp6-misc-finance-business-date\.mjs && node scripts\/check-cp6-scrap-business-date\.mjs && node scripts\/check-cp6-internal-role-fail-closed\.mjs && node scripts\/check-cp6-cash-business-dates\.mjs && node scripts\/check-cp6-accounting-close-business-date\.mjs"/)
...
for (const token of [
  ...
  'erp_schema_owners_grants_sequences_rows=IDENTICAL',
  'database_identity', 'public_schema_identity',
  "from aclexplode(d.datacl) a", 'database-identity.txt',
  "has_schema_privilege('postgres','public','CREATE')",
  "has_schema_privilege('authenticated','public','CREATE')",
  'database_owner_acl_connectivity=IDENTICAL',
  'public_schema_owner_acl_role_privileges=IDENTICAL', 'status=PASS',
...
for (const token of [
  ...
  'source_database_owner', "test \"$source_database_owner\" = 'postgres'",
  'source_database_acl_is_null', 'source_database_nonowner_grantors',
  'cross join lateral aclexplode(d.datacl)',
  ...
  'database_owner_preserved=PASS',
  'database_acl_replayed=PASS',
