#!/usr/bin/env python3
"""Twenty AH schedules using the unchanged established concurrency oracle."""
...
ROOT = Path("cp6-proof/AH_MAINTENANCE_ROLLBACK")
REPORT = ROOT / "manifest.json"


def prepare(target, operation, folder, *, source_generation):
    """Bind the old oracle's fixture setup to AH -> AG, with exact verification."""
    if target != "AH" or source_generation != "AH":
        raise AssertionError("AH_SCHEDULE_REQUIRES_EXACT_EDGE")
    matrix.command(
    ...
    with matrix.legacy.connect("ah-clone-source") as conn, conn.cursor() as cur:
        if len(runtime.verified_successor(cur)) != 690:
            raise AssertionError("AH_CLONE_RUNTIME_MISMATCH")
    matrix.maintenance_strip("AH", folder)
    with matrix.legacy.connect("ah-predecessor-fixture") as conn, conn.cursor() as cur:
    ...
    with matrix.legacy.connect("ah-exact-platform-ledger") as conn, conn.cursor() as cur:
        cur.execute(
...
def run():
    ...
    if actual != expected or os.environ.get("CP6_AH_MAINTENANCE_CONFIRM") != "cp6_rollback":
        raise AssertionError("AH_MAINTENANCE_EXACT_DISPOSABLE_ENVIRONMENT_REQUIRED")
    ...
    result = {
        "format": "CP6_AH_MAINTENANCE_SCHEDULES_V1", "status": "INCOMPLETE",
    ...
    try:
        for operation in matrix.OPERATIONS:
            for mode in matrix.MODES:
                name = f"AH-{operation}-{mode}"
                folder = ROOT / name
                folder.mkdir(parents=True, exist_ok=True)
                try:
                    evidence = matrix.run_case("AH", operation, mode, folder, source_generation="AH")
                except Exception as exc:
                    evidence = {
                        "status": "FAIL", "target": "AH", "operation": operation,
