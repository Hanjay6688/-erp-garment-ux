#!/usr/bin/env python3
"""AJ pins all 533 live functions and the 157 unchanged AC relation objects."""
from pathlib import Path
import hashlib,json,os,subprocess,sys
sys.path.insert(0,str(Path.cwd()/"scripts"))
import cp6_v2620ac_runtime as ac
import cp6_v2620ai_runtime as predecessor
from cp6_v2620aj_build_sql import STAMP,NAME,VERSION,MIGRATION,ROLLBACK,PINS,CATALOG,PREDECESSOR_HEAD,PREDECESSOR_TREE

ROOT=Path(__file__).resolve().parents[1]
MIGRATION,ROLLBACK,PINS,CATALOG=(ROOT/p for p in (MIGRATION,ROLLBACK,PINS,CATALOG))
CAPSULE='erp.cp6_v2620aj_rollback_capsule'
PINS_SHA256='18cff279a758a2be232a903183cbb8590a3aee1bf3e16b58b938a5feb5e1dd5c'

def pins():
 raw=PINS.read_bytes();assert hashlib.sha256(raw).hexdigest()==PINS_SHA256,'AJ_TRUSTED_PINS_CHANGED'
 p=json.loads(raw)
 assert (p['predecessor_head'],p['predecessor_tree'],p['boundary_count'],p['predecessor_table_count'])==(PREDECESSOR_HEAD,PREDECESSOR_TREE,222,224)
 return p

def verify_source_files():
 p=pins();predecessor.verify_source_files()
 for path,expected in p['source_pins'].items():
  b=(ROOT/path).read_bytes();assert len(b)==expected['bytes'] and hashlib.sha256(b).hexdigest()==expected['sha256'],'AJ_SOURCE_DRIFT:'+path
...
def verify_audit_source():
 ...
 assert (git(Path.cwd(),'rev-parse','HEAD'),git(Path.cwd(),'rev-parse','HEAD^{tree}'))==(PREDECESSOR_HEAD,PREDECESSOR_TREE)
 assert not git(Path.cwd(),'diff','--name-only','HEAD')
 head,tree=git(ROOT,'rev-parse','HEAD'),git(ROOT,'rev-parse','HEAD^{tree}')
 assert head==os.environ['CP6_AUDIT_HARNESS_HEAD']
...
def catalog(cur,successor):
 ...
 assert len(observed)==533 and {r[0] for r in observed}==set(expected),'AJ_COMPLETE_FUNCTION_SET_DRIFT'
 result={}
 for identity,digest,owner,acl in observed:
  e=expected[identity];assert (digest,owner,acl)==(e['sha256'],e['owner'],e['acl']),'AJ_LIVE_FUNCTION_DRIFT:'+identity
...
def verify_predecessor(cur):
 ...
 catalog(cur,False)
 assert not cur.execute("select to_regclass(%s) is not null or exists(select 1 from erp.schema_migrations where version=%s) or exists(select 1 from supabase_migrations.schema_migrations where name=%s)",(CAPSULE,VERSION,NAME)).fetchone()[0],'AJ_PREDECESSOR_RESIDUE'
 return True
...
def verified_successor(cur,*,pre_admission=False):
 ...
 assert platform==[(STAMP,p['source_pins'][str(MIGRATION.relative_to(ROOT))]['sha256'])],'AJ_PLATFORM_SOURCE_DRIFT'
 ...
 boundary=rows[0][-1];assert boundary is not None and len(boundary)==222
 for identity,before,actual,after,owner,acl,snapshot in rows:
  e=expected[identity]
  assert (before,actual,after,owner,acl)==(e['predecessor_sha256'],e['predecessor_sha256'],e['installed_sha256'],e['owner'],e['acl']) and snapshot==boundary,'AJ_CAPSULE_DRIFT:'+identity
 # Historical capsules are immutable dependencies captured in the AJ boundary.
 ...
 try:
  ...
  names=cur.execute("select relname from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='erp' and c.relkind='r' and relname like 'cp6%rollback_capsule' and relname<>'cp6_v2620aj_rollback_capsule' order by 1").fetchall()
  for (name,) in names:
   ...
   assert digest==boundary[name],'AJ_INHERITED_CAPSULE_DRIFT:'+name
 ...
 assert len(objects)==690,'AJ_RUNTIME_CARDINALITY'
 return objects
