#!/usr/bin/env python3
"""Native F/G/H/I/J/K rollback qualification under a closed-admission contract.

Six target generations x five real backend paths x four schedules = 120
...
def setup_rollback_plan(target: str, source_generation: str) -> tuple[str, ...]:
    ...
    if source_generation not in ('I', 'J', 'K') or target not in TARGETS:
...
def prepare(
    target: str, operation: str, folder: Path, *, source_generation: str = 'K',
...
def main() -> None:
    ...
    report['status'] = (
        ...
        and writer_first_body_entry == 30
