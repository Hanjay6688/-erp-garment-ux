#!/usr/bin/env python3
"""Native F/G/H/I/J/K/L/M/N/O/P/Q/R/S/T/U/V rollback qualification under closed admission.

Seventeen target generations x five real backend paths x four schedules = 340
...
def setup_rollback_plan(target: str, source_generation: str) -> tuple[str, ...]:
    # The full matrix clones V; each older guard names its restored source.
    # Missing or unexpected successors are checked before fixture writes.
    if source_generation not in ('I', 'J', 'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V') or target not in TARGETS:
...
def prepare(
    target: str, operation: str, folder: Path, *, source_generation: str = 'V',
