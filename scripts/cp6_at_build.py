"""Exact successor for the physical validity of WIP product identities."""
from pathlib import Path
import copy,json
import cp6_ao_ap_build as package
import cp6_ao_ap_inventory as inventory
import cp6_as_build as predecessor
from cp6_at_definitions import OLD,FUNCTIONS

ROOT=Path(__file__).resolve().parents[1]
STAMP='20260923005153'
NAME='erp_v2_6_20at_cp6_wip_temporal_identity'
VERSION='v2.6.20at'
CAP='erp.cp6_v2620at_rollback_capsule'
MIGRATION=Path(f'supabase/migrations/{STAMP}_{NAME}.sql')
ROLLBACK=Path(f'supabase/rollbacks/{STAMP}_{NAME}.rollback.sql')
PINS=Path('docs/evidence/cp6-at-pins.json')
PROVENANCE=Path('docs/evidence/cp6-at-cli-provenance.json')
BASE_PINS_SHA='8bfbb4faf563bd800fc79db65c87c6cdf62539869bfb39f060d65fe2e64be8b8'
q,j,sha=package.q,package.j,inventory.sha
INVENTORY_SQL=predecessor.INVENTORY_SQL.replace("'cp6_v2620as_rollback_capsule')","'cp6_v2620as_rollback_capsule','cp6_v2620at_rollback_capsule')")
...
def model():
    ...
    pin=dict(version=VERSION,stamp=STAMP,name=NAME,migration=str(MIGRATION),rollback=str(ROLLBACK),capsule=CAP,previous='AS',functions=functions)
...
def prior_guard(base,capture):
    ...
    result=predecessor.prior_guard(base,capture)+package.capsule_guard('AS',pin)
...
def build():
    ...
    s='-- CP6 AT: WIP output identity must cover its physical production day.\n'+package.START
...
"""+guard(before['objects'],'AT_PREDECESSOR')+prior_guard(base,capture)
...
"""+''.join(new.rstrip(';\n')+';\n' for new in FUNCTIONS.values())+guard(after['objects'],'AT_INSTALLED')
...
do $after_data$ declare v_table text;v_hash jsonb;v_after jsonb; begin
 ...
 if v_after is distinct from (select boundary_snapshot->'before' from {CAP} limit 1) then raise exception 'AT_INSTALL_CHANGED_DATA';end if;
 update {CAP} set boundary_snapshot=boundary_snapshot||jsonb_build_object('after',v_after);
end $after_data$;
insert into erp.schema_migrations(version,description) values({q(VERSION)},'Respect WIP output product validity without changing historical opening identity rules');
"""+package.capsule_guard('AT',pin)+'commit;\n'
    MIGRATION.write_text(s);pin['migration_sha256']=sha(s)
    s='-- CP6 AT: exact pre-use restore to AS under closed admission.\n'+package.START
...
"""+guard(after['objects'],'AT_ROLLBACK')+prior_guard(base,capture)+package.capsule_guard('AT',pin)
    s+=f"""create temporary table cp6_at_restore_boundary on commit drop as select boundary_snapshot from {CAP} limit 1;
...
"""+guard(before['objects'],'AT_RESTORED')
...
"""
    ...
    pin.update(builder_sha256=sha(Path(__file__).read_bytes()),definitions_sha256=sha(Path('scripts/cp6_at_definitions.py').read_bytes()),base_pins_sha256=BASE_PINS_SHA,production_go=False,independent_acceptance=False)
