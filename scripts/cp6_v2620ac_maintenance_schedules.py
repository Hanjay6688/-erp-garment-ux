#!/usr/bin/env python3
"""Twenty focused AC admission/drain/rollback schedules on fresh clones."""
...
ROOT = Path("cp6-proof/AC_MAINTENANCE_ROLLBACK")
...
def run() -> dict:
    ...
    if actual_environment != expected_environment:
        raise AssertionError("AC_MAINTENANCE_EXACT_DISPOSABLE_ENVIRONMENT_REQUIRED")
    if os.environ.get("CP6_AC_MAINTENANCE_CONFIRM") != "cp6_rollback":
        raise AssertionError("AC_MAINTENANCE_EXPLICIT_CONFIRMATION_REQUIRED")

    matrix.TARGETS["AC"] = (
        runtime.STAMP, runtime.NAME, runtime.VERSION, "v2.6.20ab", 115,
    )
    with psycopg.connect(matrix.SOURCE) as conn, conn.cursor() as cur:
        source_objects = runtime.verified_successor(cur)
    if len(source_objects) != 272:
        raise AssertionError("AC_MAINTENANCE_EXACT_AC_SOURCE_REQUIRED")

    result = {
        "format": "CP6_V2620AC_MAINTENANCE_SCHEDULES_V1",
        "status": "FAIL",
    ...
    for operation in matrix.OPERATIONS:
        for mode in matrix.MODES:
            name = f"AC-{operation}-{mode}"
            folder = ROOT / name
            folder.mkdir(parents=True, exist_ok=True)
            try:
                evidence = matrix.run_case(
                    "AC", operation, mode, folder, source_generation="AC"
                )
            except Exception as exc:
                evidence = {
                    "status": "FAIL", "target": "AC", "operation": operation,
