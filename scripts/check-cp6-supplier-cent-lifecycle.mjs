...
for (const token of ['Apply v2.6.20m exact supplier', 'Reproduce independent M supplier cent',
  ...
  'Qualify exact F G H I J K L M N O P Q R and S rollback', 'Prove trusted N capsule', 'Prove trusted M capsule']) {
...
for (const token of ["len(n_before['cases'])==5", "len(n['cases'])==15",
  ...
  "len(schedules['cases'])==280", "'expected':70,'observed':70",
  'CP6_V2620S_RUNTIME_MANIFEST.json', 'cp6-r1-v2620s-full-schema-auth-browser-proof']) {
...
assert.ok(install.includes(`test "$(jq '.completed_case_count' cp6-proof/CP6_ROLLBACK_SETUP_UNIT.json)" = '118'`))
assert.ok(workflow.includes('CP6_M_RACE_SOURCE_GENERATION: S'))
assert.ok(workflow.includes("mr['source_generation']=='S'"))
const auth = read('scripts/cp6_auth_permission_e2e.mjs')
assert.ok(auth.includes('PHYSICAL_DISPOSABLE_CP6_AUTH_CLONE_AFTER_V2620S'))
assert.ok(workflow.includes("report['target']=='PHYSICAL_DISPOSABLE_CP6_AUTH_CLONE_AFTER_V2620S'"))
