...
for (const token of ['Apply v2.6.20m exact supplier', 'Reproduce independent M supplier cent',
  ...
  'Qualify exact F G H I J K L M N O P Q and R rollback', 'Prove trusted N capsule', 'Prove trusted M capsule']) {
...
for (const token of ["len(n_before['cases'])==5", "len(n['cases'])==15",
  ...
  'CP6_V2620R_RUNTIME_MANIFEST.json', 'cp6-r1-v2620r-full-schema-auth-browser-proof']) {
...
assert.ok(install.includes(`test "$(jq '.completed_case_count' cp6-proof/CP6_ROLLBACK_SETUP_UNIT.json)" = '103'`))
assert.ok(workflow.includes('CP6_M_RACE_SOURCE_GENERATION: R'))
assert.ok(workflow.includes("mr['source_generation']=='R'"))
const auth = read('scripts/cp6_auth_permission_e2e.mjs')
assert.ok(auth.includes('PHYSICAL_DISPOSABLE_CP6_AUTH_CLONE_AFTER_V2620R'))
assert.ok(workflow.includes("report['target']=='PHYSICAL_DISPOSABLE_CP6_AUTH_CLONE_AFTER_V2620R'"))
