...
const order=['Apply v2.6.20j immutable','Apply v2.6.20k payment date','Run post-CP6 real Auth',
 'Prove native K per-date','Qualify exact F G H I J and K rollback','Prove trusted K capsule','Prove trusted J capsule']
let position=-1
for(const label of order){const next=workflow.indexOf(label);assert.ok(next>position,label);position=next}
for(const token of ['CP6_V2620K_RUNTIME_PROOF_V1','CP6_V2620K_PAYMENT_DATE_REGRESSION.json',
 'CP6_V2620K_ROLLBACK_GUARDS.json',"len(schedules['cases'])==120","'expected':30,'observed':30"])
