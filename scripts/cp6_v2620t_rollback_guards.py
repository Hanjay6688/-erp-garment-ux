#!/usr/bin/env python3
"""Trusted T capsule guards and maintenance-only exact T-to-S restore."""
...
ROLLBACK = Path(
    'supabase/rollbacks/20260912171034_erp_v2_6_20t_cp6_material_adjustment_revaluation.rollback.sql'
)
REPORT = Path('cp6-proof/CP6_V2620T_ROLLBACK_GUARDS.json')
MAINTENANCE_REPORT = Path('cp6-proof/V2620T_MAIN_MAINTENANCE_ROLLBACK.json')
CLONE_ROOT = Path('cp6-proof/T_TRUSTED_CAPSULE_GUARD')
DIRECT_GUARD_NAMES = (
...
def verify_capsule_fault() -> dict[str, Any]:
    ...
    with n_guards.disposable_clone_confirmation(matrix.CLONE):
        try:
            matrix.prepare('T', 'REPORT', CLONE_ROOT, source_generation='T')
            coherent_capsule_fault(matrix.CLONE)
            rejection = None
            try:
                maintenance.run_maintenance_rollback(
                    target_name='T', target_pgurl=matrix.CLONE,
...
def direct_guards(target_pgurl: str) -> list[dict[str, Any]]:
    ...
    with psycopg.connect(target_pgurl, autocommit=False) as conn:
        ...
        cases = (
            ('platform_bytes',
             "update supabase_migrations.schema_migrations set statements=array['tampered S'] where name='erp_v2_6_20t_cp6_material_adjustment_revaluation'",
             'T_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR', None),
            ('successor',
            ...
            ('definition_drift',
             ...
             'T_TRUSTED_PREDECESSOR_PIN_MISMATCH', None),
            ('acl_drift',
             'grant execute on function erp.sync_material_cost_revaluation(uuid) to anon',
             'T_TRUSTED_PREDECESSOR_PIN_MISMATCH', None),
            ('boundary_drift',
             "update erp.cp6_v2620t_rollback_capsule set boundary_snapshot=boundary_snapshot-'supplier_payments'",
             'T_BOUNDARY_SNAPSHOT_MISMATCH', None),
            ('post_install_boundary_history',
             "update erp.cp6_v2620s_rollback_capsule set boundary_snapshot=boundary_snapshot||'{\"r_post_use_probe\":true}'::jsonb",
             'T_POST_USE_ROLLBACK_REFUSED', None),
            ('coherent_capsule_and_checksum', None,
             'T_TRUSTED_PREDECESSOR_PIN_MISMATCH', coherent),
        )
...
def verify_extra_preflight_guards() -> list[dict[str, Any]]:
    ...
    with n_guards.disposable_clone_confirmation(matrix.CLONE):
        for name, mutation, expected in EXTRA_FAULTS:
            ...
            try:
                matrix.prepare('T', 'REPORT', folder, source_generation='T')
                with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                ...
                if rejection is None or expected not in rejection:
                    raise AssertionError(
                        'T_EXTRA_PREFLIGHT_WRONG_REJECTION: ' + name
                    )
                observed = matrix.read_json_if_present(
                ...
                if observed.get('admission_closed') or observed.get('rollback_started'):
                    raise AssertionError(
                        'T_EXTRA_PREFLIGHT_MUTATED_ADMISSION: ' + name
                    )
                ...
                with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                    marker = conn.execute("""select count(*)
                      from erp.schema_migrations where version='v2.6.20t'""").fetchone()
                if admission != (True,) or marker != (1,):
                    raise AssertionError(
                        'T_EXTRA_PREFLIGHT_STATE_CHANGED: ' + name
                    )
                results.append({
...
def verify_exact_s_restore(target_pgurl: str) -> dict[str, Any]:
    expected = maintenance.TRUSTED_FUNCTIONS['T']
    with psycopg.connect(target_pgurl, autocommit=False) as conn, conn.cursor() as cur:
        ...
        cur.execute("""select
          not exists(select 1 from erp.schema_migrations where version='v2.6.20t')
          and exists(select 1 from erp.schema_migrations where version='v2.6.20s')
          ...
          and to_regclass('erp.cp6_v2620t_rollback_capsule') is null
          and to_regclass('erp.material_adjustment_revaluation_facts') is null
          and to_regprocedure('erp._cp6_material_adjustment_revaluation_state(uuid)') is null
          and to_regprocedure('erp._cp6_sync_material_adjustment_revaluation(uuid,uuid)') is null
          and to_regprocedure('erp.guard_material_adjustment_revaluation_fact_v2620t()') is null
          and to_regclass('erp.cp6_v2620s_rollback_capsule') is not null
    ...
    return {
        'generation': 'S', 'restored_function_count': len(actual),
...
def run() -> dict[str, Any]:
    ...
    maintenance_result = maintenance.run_maintenance_rollback(
        target_name='T', target_pgurl=target_pgurl,
    ...
    result.update(
        status='PASS',
        exact_pre_use_restore=verify_exact_s_restore(target_pgurl),
        maintenance={
            'status': 'PASS', 'target': 'T',
...
def main() -> None:
    ...
    try:
        ...
        result = {
            'head': os.environ.get('GITHUB_SHA', 'LOCAL_UNBOUND'),
            'status': 'FAIL',
            'error_code': 'V2620T_ROLLBACK_GUARD_FAILED',
