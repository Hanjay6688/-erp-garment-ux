"""Read-only, source-pinned L successor verification for older business suites.
...
MIGRATION = Path('supabase/migrations/20260911070622_erp_v2_6_20l_cp6_exact_ledger_conservation.sql')

def verified_successor(cur):
    cur.execute("""select exists(select 1 from erp.schema_migrations where version='v2.6.20l'),
      to_regclass('erp.cp6_v2620l_rollback_capsule') is not null""")
    marker, capsule = cur.fetchone()
    if not marker and not capsule:
        return {}
    if not marker or not capsule:
        raise AssertionError('L_MARKER_CAPSULE_MISMATCH')
    ...
    rows = cur.fetchall()
    if len(rows) != 1 or rows[0][0] != '20260911070622' or rows[0][1] not in {
        hashlib.sha256(data).hexdigest(), hashlib.sha256(data[:-1]).hexdigest(),
    }:
        raise AssertionError('L_SOURCE_PLATFORM_MISMATCH')
    observations = maintenance._capsule_snapshot(cur.connection, 'L', maintenance.TARGETS['L'])
...
def effective_hash(successor, identity, predecessor):
    ...
    if predecessor != item['predecessor_sha256']:
        raise AssertionError('L_PREDECESSOR_CHAIN_MISMATCH')
