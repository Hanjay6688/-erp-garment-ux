...
AZ rev2 (round seven, writer; handoff §23.7 and the independent review of AY rev7), same rule for the rest of the family:
  ...
  - supplier invoice dated before its goods were received (erp.post_material_supplier_invoice; owner, option 1): booked on
    ...
    (erp.post_material_purchase_cost_correction), which carries an invoice date too, the same way.
...
REVAL_EVENT_NEW="values(p_material_id,q.mid,v_date,v_old,q.tgt,v_diff,v_counterpart,v_po,v_contractor)"
...
# AZ rev2.1 (writer, 24 Sep 2026; native AY:PRESEWING_REVERSAL_THEN_LATE_INVOICE_LOWER on 769abfe: MATERIAL_INVENTORY
# -10.50 between the cut and its reversal, GL_INVENTORY_NEGATIVE_ASOF CRITICAL). The cost engine replays a reversed
# movement and its reversal as if neither happened (erp._recalculate_material_cost_core), so the reversed movement keeps
# its posting cost and its revaluation was targeted to zero. Physically its units were out (in WIP for a cutting issue)
# from its day to the reversal's day, so a revaluation of the stock they came from must follow them there. A reversed
# movement is now revalued at the cost the replay gives the same movement (outbound: the replayed average before it; a
# cutting return: the cost of its issue) on its own day, and each reversal takes that back on its own day (state per
# movement; net zero once reversed).
REVAL_REVERSED_OLD="""    if exists(select 1 from erp.material_stock_movements rv where rv.reversal_of_id=r.id) then
      v_target:=0;
    else"""
AVG_BEFORE="""(select h.average_after from erp.material_cost_history h join erp.material_stock_movements hm on hm.id=h.movement_id
          where h.material_id=p_material_id and (hm.physical_at,hm.system_created_at,hm.id)<({m}.physical_at,{m}.system_created_at,{m}.id)
          order by hm.physical_at desc,hm.system_created_at desc,hm.id desc limit 1)"""
REVAL_REVERSED_NEW="""    if exists(select 1 from erp.material_stock_movements rv where rv.reversal_of_id=r.id) then
      -- AZ rev2.1: revalued like the replay would value it, on its own day; its reversal takes it back (legs below).
      v_would:=case when r.qty_signed<0 then """+AVG_BEFORE.format(m='r')+"""
        when r.source_type='CUTTING_GROUP_RETURN' then (select case when exists(select 1 from erp.material_stock_movements rv where rv.reversal_of_id=x.id)
            then """+AVG_BEFORE.format(m='x')+""" else x.unit_cost_snapshot end
...
"""
REVAL_LEGS_NEW="""    -- AZ rev2.1: the movement, then each of its reversals with the opposite target on the reversal's own day.
...
REVAL_STATE_NEW="""    insert into erp.material_cost_revaluation_state(movement_id,applied_inventory_delta,updated_at)
...
# ---------------------------------------------------------------- erp.post_material_supplier_invoice (AO): invoice before receipt
# Owner decision (24 Sep 2026, option 1): an invoice dated before its goods were received is booked on the receipt day (the
# latest receipt day of its lines); the invoice date stays the document date (due date, AP ageing). The recost context date
# (E) is that booking day too, so the recost legs follow the same rule. An invoice dated on or after the receipt: unchanged.
INV_HEAD="CREATE OR REPLACE FUNCTION erp.post_material_supplier_invoice(p_invoice_id uuid)"
INV_DECLARE_OLD="""  v_payable numeric;
begin"""
INV_DECLARE_NEW="""  v_payable numeric;
  v_book_date date;
begin"""
INV_CTX_OLD="""  insert into erp.invoice_recost_execution_context(transaction_id,invoice_date,source_id) values(txid_current(),h.invoice_date,h.id);"""
INV_CTX_NEW="""  -- AZ rev2 (owner, 24 Sep 2026, option 1): booked no earlier than the latest receipt day of its lines; the invoice date
...
INV_POST_OLD="""  perform erp._cp6_apply_supplier_cent_event('MATERIAL_SUPPLIER_INVOICE',h.id,h.invoice_date,'Supplier document cents '||h.id::text,v_n_before,false);"""
INV_POST_NEW="""  perform erp._cp6_apply_supplier_cent_event('MATERIAL_SUPPLIER_INVOICE',h.id,v_book_date,'Supplier document cents '||h.id::text,v_n_before,false);"""


# ---------------------------------------------------------------- erp.post_material_purchase_cost_correction (AO): same rule
# The purchase cost correction carries an invoice date too (context and supplier cent event); the owner's rule for a supplier
# invoice (option 1) applies the same way: booked no earlier than the receipt day of its purchase, the invoice date kept.
CC_HEAD="CREATE OR REPLACE FUNCTION erp.post_material_purchase_cost_correction(p_correction_id uuid)"
CC_DECLARE_OLD="""  v_post_seq bigint;
begin"""
CC_DECLARE_NEW="""  v_post_seq bigint;
  v_book_date date;
begin"""
CC_CTX_OLD="""  insert into erp.invoice_recost_execution_context(transaction_id,invoice_date,source_id) values(txid_current(),h.invoice_date,h.id);
  select * into p from erp.material_purchase_headers where id=h.purchase_id for update;"""
CC_CTX_NEW="""  select * into p from erp.material_purchase_headers where id=h.purchase_id for update;
  ...
  insert into erp.invoice_recost_execution_context(transaction_id,invoice_date,source_id) values(txid_current(),v_book_date,h.id);"""
CC_POST_OLD="""  perform erp._cp6_apply_supplier_cent_event('MATERIAL_PURCHASE_COST_CORRECTION',h.id,h.invoice_date,'Supplier document cents '||h.id::text,v_n_before,false);"""
CC_POST_NEW="""  perform erp._cp6_apply_supplier_cent_event('MATERIAL_PURCHASE_COST_CORRECTION',h.id,v_book_date,'Supplier document cents '||h.id::text,v_n_before,false);"""
...
def build():
    ...
    inv=function(AO,INV_HEAD,[(INV_DECLARE_OLD,INV_DECLARE_NEW),(INV_CTX_OLD,INV_CTX_NEW),(INV_POST_OLD,INV_POST_NEW)],once=False)
    cc=function(AO,CC_HEAD,[(CC_DECLARE_OLD,CC_DECLARE_NEW),(CC_CTX_OLD,CC_CTX_NEW),(CC_POST_OLD,CC_POST_NEW)],once=False)
    parts=['-- CP6 AZ material recost corrections dated from the physical movement: T1_FAMILY development install (NOT a release package).',
           ...
           'end $t1_guard$;',reval,adj,residual,pocket,bsv,nonpo,acc,rqc,rrw,wip,inv,cc,
