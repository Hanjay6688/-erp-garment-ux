...
requireTokens(matrix, '180 native schedules and body-entry proof', [
  ...
  "'J': ('20260910170556'", "'expected_case_count': 180",
  "'expected': 45", 'writer_first_body_entry == 45',
...
requireTokens(setupUnit, 'generation setup failure regressions', [
  ...
  "'expected_case_count': 54", 'unconditional_j_negative_control_rejected',
...
const order = [
  ...
  'Qualify exact F G H I J K L M and N rollback under closed admission across one hundred eighty native schedules',
...
requireTokens(workflow, 'exact-J artifact contract', [
  ...
  "setup_unit['completed_case_count']==setup_unit['expected_case_count']==54",
  ...
  "('F','G','H','I','J','K','L','M','N')", "len(schedules['cases'])==180",
  "'writer_first_backend_body_entries':45",
...
assert.ok(read('scripts/cp6_auth_permission_e2e.mjs').includes('AFTER_V2620N'))
...
console.log(JSON.stringify({
  ...
  maintenance_matrix_schedules: 180,
  writer_first_backend_body_entries: 45,
  production_go: false,
}))
