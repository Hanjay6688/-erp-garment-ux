"""Verify P and historical capsules without weakening rollback admission."""
import hashlib
...
def verified_successor(cur):
    cur.execute("""select exists(select 1 from erp.schema_migrations
      ...
      to_regclass('erp.cp6_v2620p_rollback_capsule') is not null""")
    marker, capsule = cur.fetchone()
    if not marker and not capsule:
        return {}
    if not marker or not capsule:
        raise AssertionError('P_MARKER_CAPSULE_MISMATCH')
    ...
    rows = cur.fetchall()
    if len(rows) != 1 or rows[0][0] != '20260912051922' or rows[0][1] not in {
        hashlib.sha256(data).hexdigest(), hashlib.sha256(data[:-1]).hexdigest(),
    }:
        raise AssertionError('P_SOURCE_PLATFORM_MISMATCH')
    import cp6_v2620q_runtime as q_runtime
    successor = q_runtime.verified_successor(cur)
    observations = q_runtime.predecessor_snapshot(cur, 'P', successor)
    q_runtime.extend_items(successor, observations)
...
def effective_hash(successor, identity, predecessor):
    ...
    if predecessor != item['predecessor_sha256']:
        raise AssertionError('P_PREDECESSOR_CHAIN_MISMATCH')
...
def predecessor_snapshot(cur, generation, successor):
    ...
    if len(rows) != len(wanted) or {r['identity'] for r in rows} != set(wanted):
        raise AssertionError('P_HISTORICAL_CAPSULE_CARDINALITY_MISMATCH')
    for row in rows:
        ...
        if (
            ...
            raise AssertionError('P_HISTORICAL_CAPSULE_SOURCE_PIN_MISMATCH')
    return rows
...
def verify_extra_objects(cur):
    ...
    if cur.fetchone() != (True, 'postgres', False, 2):
        raise AssertionError('P_CENT_FACT_SECURITY_MISMATCH')
