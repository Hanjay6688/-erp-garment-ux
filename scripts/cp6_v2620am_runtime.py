#!/usr/bin/env python3
"""AM pins all 533 live functions and the 157 unchanged AC relation objects."""
from pathlib import Path
import hashlib,json,os,subprocess,sys
sys.path.insert(0,str(Path.cwd()/"scripts"))
import cp6_v2620ac_runtime as ac
import cp6_v2620al_runtime as predecessor
from cp6_v2620am_build_sql import STAMP,NAME,VERSION,MIGRATION,ROLLBACK,PINS,CATALOG,PREDECESSOR_HEAD,PREDECESSOR_TREE

ROOT=Path(__file__).resolve().parents[1]
MIGRATION,ROLLBACK,PINS,CATALOG=(ROOT/p for p in (MIGRATION,ROLLBACK,PINS,CATALOG))
CAPSULE='erp.cp6_v2620am_rollback_capsule'
PINS_SHA256='bb745289df98c2adcba4ce27ee327f4cc98f7380b0b7e8857d456861db713799'

def pins():
 raw=PINS.read_bytes();assert hashlib.sha256(raw).hexdigest()==PINS_SHA256,'AM_TRUSTED_PINS_CHANGED'
 p=json.loads(raw)
 assert (p['predecessor_head'],p['predecessor_tree'],p['boundary_count'],p['predecessor_table_count'])==(PREDECESSOR_HEAD,PREDECESSOR_TREE,225,227)
 return p

def verify_source_files():
 p=pins();predecessor.verify_source_files()
 for path,expected in p['source_pins'].items():
  b=(ROOT/path).read_bytes();assert len(b)==expected['bytes'] and hashlib.sha256(b).hexdigest()==expected['sha256'],'AM_SOURCE_DRIFT:'+path
 assert len(p['functions'])==6
 return p

def verify_audit_source():
 verify_source_files()
 # Execution checkout stays pinned AI; the predecessor AL is installed and
...
def reviewed_local_rollback():
 ...
 # Materialize the exact already-pinned AM bytes there; retain that path gate
 # and the controller's independently pinned rollback checksum.
 verify_audit_source()
 target=Path.cwd()/'cp6-proof/writer-am/reviewed'/ROLLBACK.name
 assert target.resolve().is_relative_to(Path.cwd().resolve()),'AM_LOCAL_ROLLBACK_PATH'
...
def catalog(cur,successor):
 ...
 assert len(observed)==533 and {r[0] for r in observed}==set(expected),'AM_COMPLETE_FUNCTION_SET_DRIFT'
 result={}
 for identity,digest,owner,acl in observed:
  e=expected[identity];assert (digest,owner,acl)==(e['sha256'],e['owner'],e['acl']),'AM_LIVE_FUNCTION_DRIFT:'+identity
...
def verify_predecessor(cur):
 ...
 catalog(cur,False)
 assert not cur.execute("select to_regclass(%s) is not null or exists(select 1 from erp.schema_migrations where version=%s) or exists(select 1 from supabase_migrations.schema_migrations where name=%s)",(CAPSULE,VERSION,NAME)).fetchone()[0],'AM_PREDECESSOR_RESIDUE'
 return True
...
def verified_successor(cur,*,pre_admission=False):
 ...
 assert platform==[(STAMP,p['source_pins'][str(MIGRATION.relative_to(ROOT))]['sha256'])],'AM_PLATFORM_SOURCE_DRIFT'
 ...
 expected={f['identity']:f for f in p['functions']};assert len(rows)==6 and {r[0] for r in rows}==set(expected)
 boundary=rows[0][-1];assert boundary is not None and len(boundary)==225
 for identity,before,actual,after,owner,acl,snapshot in rows:
  e=expected[identity]
  assert (before,actual,after,owner,acl)==(e['predecessor_sha256'],e['predecessor_sha256'],e['installed_sha256'],e['owner'],e['acl']) and snapshot==boundary,'AM_CAPSULE_DRIFT:'+identity
 # Historical capsules are immutable dependencies captured in the AM boundary.
 ...
 try:
  ...
  names=cur.execute("select relname from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='erp' and c.relkind='r' and relname like 'cp6%rollback_capsule' and relname<>'cp6_v2620am_rollback_capsule' order by 1").fetchall()
  for (name,) in names:
   ...
   assert digest==boundary[name],'AM_INHERITED_CAPSULE_DRIFT:'+name
 ...
 assert len(objects)==690,'AM_RUNTIME_CARDINALITY'
 return objects
