...
assert.equal(authTarget,'PHYSICAL_DISPOSABLE_CP6_AUTH_CLONE_AFTER_V2620O')
...
for(const label of ['Apply v2.6.20k payment date','Reproduce independent K ledger',
 ...
 'Qualify exact F G H I J K L M N and O rollback','Prove trusted L capsule','Prove trusted K capsule']) {
 const position=workflow.indexOf(label);assert.ok(position>previous,label);previous=position
}
for(const token of ['CP6_V2620O_RUNTIME_PROOF_V1',"len(schedules['cases'])==200",
 "'expected':50,'observed':50",'CP6_V2620L_K_COUNTEREXAMPLES.json',
