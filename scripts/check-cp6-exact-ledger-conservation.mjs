...
assert.equal(authTarget,'PHYSICAL_DISPOSABLE_CP6_AUTH_CLONE_AFTER_V2620X')
...
for(const label of ['Apply v2.6.20k payment date','Reproduce independent K ledger',
 ...
 'Qualify exact F G H I J K L M N O P Q R S T U V W and X rollback','Prove trusted L capsule','Prove trusted K capsule']) {
 const position=workflow.indexOf(label);assert.ok(position>previous,label);previous=position
}
for(const token of ['CP6_V2620P_RUNTIME_PROOF_V1',"len(schedules['cases'])==380",
 "'expected':95,'observed':95",'CP6_V2620L_K_COUNTEREXAMPLES.json',
