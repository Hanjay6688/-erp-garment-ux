...
for(const label of ['Apply v2.6.20k payment date','Reproduce independent K ledger',
 ...
 'Qualify exact F G H I J K and L rollback','Prove trusted L capsule','Prove trusted K capsule']) {
 const position=workflow.indexOf(label);assert.ok(position>previous,label);previous=position
}
for(const token of ['CP6_V2620L_RUNTIME_PROOF_V1',"len(schedules['cases'])==140",
 "'expected':35,'observed':35",'CP6_V2620L_K_COUNTEREXAMPLES.json',
