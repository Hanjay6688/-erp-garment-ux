#!/usr/bin/env python3
"""Trusted AA capsule guards and maintenance-only exact AA-to-Z restore."""
...
REPORT = Path('cp6-proof/CP6_V2620AA_ROLLBACK_GUARDS.json')
DIRECT_REPORT = Path('cp6-proof/CP6_V2620AA_DIRECT_GUARD_DIAGNOSTICS.json')
MAINTENANCE_REPORT = Path('cp6-proof/V2620AA_MAIN_MAINTENANCE_ROLLBACK.json')
CLONE_ROOT = Path('cp6-proof/AA_TRUSTED_CAPSULE_GUARD')
DIRECT_GUARD_NAMES = (
...
def verify_capsule_fault() -> dict[str, Any]:
    ...
    with n_guards.disposable_clone_confirmation(matrix.CLONE):
        try:
            matrix.prepare('AA', 'REPORT', CLONE_ROOT, source_generation='AA')
            coherent_capsule_fault(matrix.CLONE)
            rejection = None
            try:
                maintenance.run_maintenance_rollback(
                    target_name='AA', target_pgurl=matrix.CLONE,
            ...
            if rejection is None or 'TRUSTED_PREDECESSOR_PIN_MISMATCH' not in rejection:
                raise AssertionError('AA coherent capsule fault was not rejected')
            ...
            if observed.get('admission_closed') or observed.get('rollback_started'):
                raise AssertionError('AA trust failure crossed mutation boundary')
            with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                marker = conn.execute("""select count(*) from erp.schema_migrations
                  where version='v2.6.20aa'""").fetchone()
            if marker != (1,):
                raise AssertionError('AA trust guard changed installed generation')
            return {
                'target': 'AA', 'status': 'PASS',
...
def table_boundary(cur: psycopg.Cursor) -> dict[str, Any]:
    cur.execute("""select c.relname from pg_class c
      ...
      where n.nspname='erp' and c.relkind in('r','p')
        and c.relname not in('schema_migrations','cp6_v2620aa_rollback_capsule')
      order by c.relname""")
    names = [row[0] for row in cur.fetchall()]
    if len(names) != 212:
        raise AssertionError(f'AA_DIRECT_BOUNDARY_CARDINALITY:{len(names)}')
    result = {}
    for name in names:
...
def direct_guards(target_pgurl: str) -> list[dict[str, Any]]:
    ...
    with psycopg.connect(target_pgurl, autocommit=False) as conn:
        ...
        cases = (
            ('platform_bytes',
             "update supabase_migrations.schema_migrations set statements=array['tampered W'] where name='erp_v2_6_20aa_cp6_material_cost_business_day'",
             'AA_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR', None),
            ('successor',
             ...
             'AA_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR', None),
            ('definition_drift',
             'alter function erp.refresh_material_cost_checkpoint(uuid,date) cost 999',
             'AA_TRUSTED_PREDECESSOR_PIN_MISMATCH', None),
            ('acl_drift',
             'grant execute on function erp.refresh_material_cost_checkpoint(uuid,date) to anon',
             'AA_TRUSTED_PREDECESSOR_PIN_MISMATCH', None),
            ('boundary_drift',
             "update erp.cp6_v2620aa_rollback_capsule set boundary_snapshot=boundary_snapshot-'supplier_payments'",
             'AA_BOUNDARY_SNAPSHOT_MISMATCH', None),
            ('post_install_boundary_history',
             ...
             'AA_POST_USE_ROLLBACK_REFUSED', None),
            ('post_install_other_module_history',
             "update erp.app_permissions set description=description||' [CP6 AA boundary probe]' where permission_key=(select permission_key from erp.app_permissions order by permission_key limit 1)",
             'AA_POST_USE_ROLLBACK_REFUSED: app_permissions', None),
            ('coherent_capsule_and_checksum', None,
             'AA_TRUSTED_PREDECESSOR_PIN_MISMATCH', coherent),
        )
        if tuple(case[0] for case in cases) != DIRECT_GUARD_NAMES:
            raise AssertionError('Direct AA guard case manifest drift')
                    ...
                    with conn.cursor() as cur:
                        if name == 'post_install_other_module_history':
                            ...
                            if detail['legacy_empty_update'] != {
                                ...
                                raise AssertionError('AA_LEGACY_EMPTY_UPDATE_CONTROL_DRIFT')
                            cur.execute("""select
                              (select bool_or(boundary_snapshot ? 'app_permissions') from erp.cp6_v2620w_rollback_capsule),
                              (select bool_and(boundary_snapshot ? 'app_permissions') from erp.cp6_v2620aa_rollback_capsule)""")
                            in_w, in_y = cur.fetchone()
                            detail['boundary_membership'] = {'table': 'app_permissions', 'in_w': in_w, 'in_y': in_y}
                            if (in_w, in_y) != (False, True):
                                raise AssertionError('AA_OTHER_MODULE_BOUNDARY_MEMBERSHIP')
                        ...
                        if coherent_definition is None:
                            ...
                            if mutation.lstrip().lower().startswith(('update ', 'insert ', 'delete ')):
                                ...
                                if cur.rowcount <= 0:
                                    raise AssertionError(f'AA_EMPTY_GUARD_MUTATION:{name}:{cur.rowcount}')
                        else:
                            cur.execute("""update erp.cp6_v2620aa_rollback_capsule
                              set object_definition=%s,
                                definition_sha256=encode(extensions.digest(
                            ...
                            detail['affected_rows'] = cur.rowcount
                            if cur.rowcount != 1:
                                raise AssertionError(f'AA_COHERENT_GUARD_CARDINALITY:{cur.rowcount}')
                        if name == 'post_install_other_module_history':
                            ...
                            if detail['affected_rows'] != 1 or detail['changed_tables'] != ['app_permissions']:
                                raise AssertionError('AA_OTHER_MODULE_MUTATION_NOT_ISOLATED')
                            if mutated['app_permissions']['rows'] != original_boundary['app_permissions']['rows']:
                                raise AssertionError('AA_PERMISSION_DESCRIPTION_CHANGED_ROW_COUNT')
                ...
                with conn.cursor() as cur:
                    ...
                    if name == 'post_install_other_module_history':
                        ...
                        if not detail['full_boundary_restored']:
                            raise AssertionError('AA_OTHER_MODULE_GUARD_LEFT_TABLE_RESIDUE')
                conn.commit()
...
def verify_extra_preflight_guards() -> list[dict[str, Any]]:
    ...
    with n_guards.disposable_clone_confirmation(matrix.CLONE):
        for name, mutation, expected in EXTRA_FAULTS:
            ...
            try:
                matrix.prepare('AA', 'REPORT', folder, source_generation='AA')
                with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                ...
                if rejection is None or expected not in rejection:
                    raise AssertionError(
                        'AA_EXTRA_PREFLIGHT_WRONG_REJECTION: ' + name
                    )
                observed = matrix.read_json_if_present(
                ...
                if observed.get('admission_closed') or observed.get('rollback_started'):
                    raise AssertionError(
                        'AA_EXTRA_PREFLIGHT_MUTATED_ADMISSION: ' + name
                    )
                ...
                with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                    marker = conn.execute("""select count(*)
                      from erp.schema_migrations where version='v2.6.20aa'""").fetchone()
                if admission != (True,) or marker != (1,):
                    raise AssertionError(
                        'AA_EXTRA_PREFLIGHT_STATE_CHANGED: ' + name
                    )
                results.append({
...
def verify_exact_z_restore(target_pgurl: str) -> dict[str, Any]:
    expected = maintenance.TRUSTED_FUNCTIONS['AA']
    with psycopg.connect(target_pgurl, autocommit=False) as conn, conn.cursor() as cur:
        ...
        cur.execute("""select
          not exists(select 1 from erp.schema_migrations where version='v2.6.20aa')
          and exists(select 1 from erp.schema_migrations where version='v2.6.20z')
          ...
          and to_regclass('erp.cp6_v2620aa_rollback_capsule') is null
        ...
        if cur.fetchone()[0] is not True:
            raise AssertionError('AA rollback metadata/schema residue')
    return {
        'generation': 'Z', 'restored_function_count': len(actual),
...
def run() -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        'head': os.environ.get('GITHUB_SHA', 'LOCAL_UNBOUND'),
        'classification': (
            'DISPOSABLE_NATIVE_AA_TRUSTED_GUARDS_AND_CLOSED_ADMISSION_RESTORE'
        ),
    ...
    maintenance_result = maintenance.run_maintenance_rollback(
        target_name='AA', target_pgurl=target_pgurl,
    ...
    if maintenance_result['status'] != 'PASS':
        raise AssertionError('AA maintenance rollback failed')
    required_phases = {
    ...
    if not required_phases.issubset(observed_phases):
        raise AssertionError('AA maintenance phase evidence incomplete')
    result.update(
        status='PASS',
        exact_pre_use_restore=verify_exact_z_restore(target_pgurl),
        maintenance={
            'status': 'PASS', 'target': 'AA',
...
def main() -> None:
    ...
    try:
        ...
        result = {
            'head': os.environ.get('GITHUB_SHA', 'LOCAL_UNBOUND'),
            'status': 'FAIL',
            'error_code': 'V2620AA_ROLLBACK_GUARD_FAILED',
