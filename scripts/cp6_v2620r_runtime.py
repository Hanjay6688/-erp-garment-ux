"""Verify R and historical capsules without weakening rollback admission."""
import hashlib
...
MIGRATION = Path(
    'supabase/migrations/20260912114635_erp_v2_6_20r_cp6_receipt_invoice_dependency.sql'
)
...
def verified_successor(cur):
    cur.execute("""select exists(select 1 from erp.schema_migrations
      ...
      to_regclass('erp.cp6_v2620r_rollback_capsule') is not null""")
    marker, capsule = cur.fetchone()
    if not marker and not capsule:
        return {}
    if not marker or not capsule:
        raise AssertionError('R_MARKER_CAPSULE_MISMATCH')
    ...
    rows = cur.fetchall()
    if len(rows) != 1 or rows[0][0] != '20260912114635' or rows[0][1] not in {
        hashlib.sha256(data).hexdigest(), hashlib.sha256(data[:-1]).hexdigest(),
    }:
        raise AssertionError('R_SOURCE_PLATFORM_MISMATCH')
    import cp6_v2620s_runtime as s_runtime
    successor = s_runtime.verified_successor(cur)
    observations = s_runtime.predecessor_snapshot(cur, 'R', successor)
    s_runtime.extend_items(successor, observations)
...
def effective_hash(successor, identity, predecessor):
    ...
    if predecessor != item['predecessor_sha256']:
        raise AssertionError('R_PREDECESSOR_CHAIN_MISMATCH')
...
def predecessor_snapshot(cur, generation, successor):
    ...
    if len(rows) != len(wanted) or {r['identity'] for r in rows} != set(wanted):
        raise AssertionError('R_HISTORICAL_CAPSULE_CARDINALITY_MISMATCH')
    for row in rows:
        ...
        if (
            ...
            raise AssertionError('R_HISTORICAL_CAPSULE_SOURCE_PIN_MISMATCH')
    return rows
...
def extend_items(successor, items):
    for item in items:
        if item['identity'] in successor:
            ...
            item['pre_r_installed_sha256'] = old
...
def verify_extra_objects(cur):
    ...
    if cur.fetchone() != (True, 'postgres', False, 2):
        raise AssertionError('R_CENT_FACT_SECURITY_MISMATCH')
