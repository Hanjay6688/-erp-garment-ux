#!/usr/bin/env python3
"""Read-only Supabase security advisors, compared on exact AM and AN."""
...
def run(phase):
 ...
 with psycopg.connect(URL.replace('postgres:postgres@','supabase_admin:postgres@')) as conn,conn.cursor() as cur:
  cur.execute("set local timezone='UTC'");assert len(version.verified_successor(cur))==(690 if phase=='before' else 692)
 ...
 result=dict(status='READ_ONLY_CAPTURED',phase=phase,writer_head=head,writer_tree=tree,
  runtime='AM' if phase=='before' else 'AN',cli_version='2.116.0',findings=observed,raw=parsed,boundary_unchanged=True,production_go=False)
 if phase=='after':
  ...
  for finding in added:
   ...
   if finding['name']=='rls_enabled_no_policy' and meta.get('schema')=='erp' and meta.get('name')=='cp6_v2620an_rollback_capsule':allowed.append(finding)
