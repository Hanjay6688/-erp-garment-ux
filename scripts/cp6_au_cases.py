...
def extra(cur,today,variant):
 f,s,p=fixture(cur,today,variant.startswith('USED') or variant in ('REPLAY','FORGED_UPDATE','PHYSICAL_HISTORY','NAME_ONLY','CANCEL_PRICE'))
 ...
 if variant in ('PRICE_REFERENCE','CANCEL_PRICE'):
  if variant=='CANCEL_PRICE':
   pnew=edit(cur,p,effective);api.admin(cur)
   p=(pnew,p[1],p[2],p[3],p[4])
 ...
 def operation():
  api.ordinary(cur)
  if variant=='CANCEL_PRICE':return cancel(cur,p[0])
...
def cases(cur,today):
 ...
 result += [('AU:'+v,lambda v=v:extra(cur,today,v)) for v in ('NAME_ONLY','PRICE_REFERENCE','CANCEL_PRICE','PHYSICAL_HISTORY','REPLAY','FORGED_CONTEXT','FORGED_INSERT','FORGED_UPDATE','FORGED_UNUSED','INVALID_CHILD')]
 return result
