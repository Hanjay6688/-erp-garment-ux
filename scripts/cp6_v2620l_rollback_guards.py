#!/usr/bin/env python3
"""Trusted L capsule guards plus maintenance-only exact L-to-K restore."""
...
REPORT = Path('cp6-proof/CP6_V2620L_ROLLBACK_GUARDS.json')
MAINTENANCE_REPORT = Path('cp6-proof/V2620L_MAIN_MAINTENANCE_ROLLBACK.json')
CLONE_ROOT = Path('cp6-proof/L_TRUSTED_CAPSULE_GUARD')
DIRECT_L_GUARD_NAMES = (
    ...
    with psycopg.connect(pgurl, autocommit=False) as conn, conn.cursor() as cur:
        cur.execute(
            """select object_regidentity,object_definition,definition_sha256
               from erp.cp6_v2620l_rollback_capsule
        ...
        cur.execute(
            """update erp.cp6_v2620l_rollback_capsule
...
def verify_l_capsule_fault() -> dict[str, Any]:
    """Return public verification outcomes, never capsule contents or credentials."""
    CLONE_ROOT.mkdir(parents=True, exist_ok=True)
    marker = maintenance.TARGETS['L']['marker']
    with disposable_clone_confirmation(matrix.CLONE):
        try:
            matrix.prepare('L', 'REPORT', CLONE_ROOT, source_generation='L')
            ...
            if rejection is None or 'TRUSTED_PREDECESSOR_PIN_MISMATCH' not in rejection:
                raise AssertionError(f'L coherent capsule fault was not rejected: {rejection}')
            ...
            if observed.get('admission_closed') or observed.get('rollback_started'):
                raise AssertionError(f'L trust failure crossed mutation boundary: {observed}')
            ...
            if marker_count != 1:
                raise AssertionError('L trusted guard changed installed generation')
            return {
                'target': 'L',
                'status': 'PASS',
...
def direct_l_guards(target_pgurl: str) -> list[dict[str, Any]]:
    ...
    with psycopg.connect(target_pgurl, autocommit=False) as conn:
        ...
        cases = (
            (
                'platform_bytes',
                "update supabase_migrations.schema_migrations set statements=array['tampered K'] where name='erp_v2_6_20l_cp6_exact_ledger_conservation'",
            ...
            (
                'boundary_drift',
                "update erp.cp6_v2620l_rollback_capsule set boundary_snapshot=boundary_snapshot-'sales_payments'",
        ...
        if tuple(case[0] for case in cases) != DIRECT_L_GUARD_NAMES:
            raise AssertionError('Direct L guard case manifest drift')
    ...
    return [
        ...
        for name in DIRECT_L_GUARD_NAMES
    ]


def verify_exact_k_restore(target_pgurl: str) -> dict[str, Any]:
    ...
    with psycopg.connect(target_pgurl, autocommit=False) as conn, conn.cursor() as cur:
        ...
        cur.execute(
            """select not exists(select 1 from erp.schema_migrations where version='v2.6.20l')
              and exists(select 1 from erp.schema_migrations where version='v2.6.20k')
              and not exists(select 1 from supabase_migrations.schema_migrations
                where name='erp_v2_6_20l_cp6_exact_ledger_conservation')
              and to_regclass('erp.cp6_v2620l_rollback_capsule') is null
              ...
              and to_regclass('erp.cp6_v2620k_rollback_capsule') is not null"""
        )
        if cur.fetchone()[0] is not True:
            raise AssertionError('L rollback metadata/schema residue')
    return {
        'generation': 'K',
...
def run() -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        'classification': 'DISPOSABLE_NATIVE_L_TRUSTED_GUARDS_AND_CLOSED_ADMISSION_RESTORE',
        'rollback_sha256': hashlib.sha256(ROLLBACK.read_bytes()).hexdigest(),
        'production_go': False,
    }
    result['trusted_capsule_guard'] = verify_l_capsule_fault()
    result['guards'] = direct_l_guards(target_pgurl)
    ...
    if maintenance_result['status'] != 'PASS':
        raise AssertionError(f'L maintenance rollback failed: {maintenance_result}')
    required_phases = {
    ...
    if not required_phases.issubset(observed_phases):
        raise AssertionError('L maintenance phase evidence incomplete')
    result.update(
        status='PASS',
        exact_pre_use_restore=verify_exact_k_restore(target_pgurl),
        maintenance={
            'status': 'PASS',
            'target': 'L',
...
def main() -> None:
    ...
    try:
        ...
        result = {
            'head': os.environ.get('GITHUB_SHA', 'LOCAL_UNBOUND'),
            'status': 'FAIL',
            'error_code': 'V2620L_ROLLBACK_GUARD_FAILED',
