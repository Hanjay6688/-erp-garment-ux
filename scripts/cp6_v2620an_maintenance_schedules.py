#!/usr/bin/env python3
"""Twenty AN schedules using the unchanged established concurrency oracle."""
...
# Bind only the controller dependency to the AN target addition. The matrix
# schedule, business sentinel, drain and refusal assertions stay byte-identical.
spec = importlib.util.spec_from_file_location('an_matrix_controller', runtime.ROOT / 'scripts/cp6_preuse_rollback_maintenance.py')
controller = importlib.util.module_from_spec(spec)
spec.loader.exec_module(controller)
matrix.maintenance = controller


ROOT = Path("cp6-proof/AN_MAINTENANCE_ROLLBACK")
REPORT = ROOT / "manifest.json"


def prepare(target, operation, folder, *, source_generation):
    """Bind the old oracle's fixture setup to AN -> AM, with exact verification."""
    if target != "AN" or source_generation != "AN":
        raise AssertionError("AN_SCHEDULE_REQUIRES_EXACT_EDGE")
    matrix.command(
    ...
    with matrix.legacy.connect("an-clone-source") as conn, conn.cursor() as cur:
        if len(runtime.verified_successor(cur)) != 692:
            raise AssertionError("AN_CLONE_RUNTIME_MISMATCH")
    matrix.maintenance_strip("AN", folder)
    with matrix.legacy.connect("an-predecessor-fixture") as conn, conn.cursor() as cur:
    ...
    with matrix.legacy.connect("an-exact-platform-ledger") as conn, conn.cursor() as cur:
        ...
        conn.commit()
    capsule = matrix.legacy.capture_capsule("AN")
    if len(capsule) != 1:
        raise AssertionError("AN_SCHEDULE_CAPSULE_CARDINALITY")
...
def run():
    ...
    if actual != expected or os.environ.get("CP6_AN_MAINTENANCE_CONFIRM") != "cp6_rollback":
        raise AssertionError("AN_MAINTENANCE_EXACT_DISPOSABLE_ENVIRONMENT_REQUIRED")
    ...
    result = {
        "format": "CP6_AN_MAINTENANCE_SCHEDULES_V1", "status": "INCOMPLETE",
    ...
    try:
        for operation in matrix.OPERATIONS:
            for mode in matrix.MODES:
                name = f"AN-{operation}-{mode}"
                folder = ROOT / name
                folder.mkdir(parents=True, exist_ok=True)
                try:
                    evidence = matrix.run_case("AN", operation, mode, folder, source_generation="AN")
                except Exception as exc:
                    evidence = {
                        "status": "FAIL", "target": "AN", "operation": operation,
