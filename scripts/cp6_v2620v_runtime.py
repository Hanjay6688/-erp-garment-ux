"""Verify V and historical capsules without weakening rollback admission."""
import hashlib
...
MIGRATION = Path(
    'supabase/migrations/20260913135850_erp_v2_6_20v_cp6_misc_finance_business_date.sql'
)
...
def verified_successor(cur):
    cur.execute("""select exists(select 1 from erp.schema_migrations
      ...
      to_regclass('erp.cp6_v2620v_rollback_capsule') is not null""")
    marker, capsule = cur.fetchone()
    if not marker and not capsule:
        return {}
    if not marker or not capsule:
        raise AssertionError('V_MARKER_CAPSULE_MISMATCH')
    ...
    rows = cur.fetchall()
    if len(rows) != 1 or rows[0][0] != '20260913135850' or rows[0][1] not in {
        hashlib.sha256(data).hexdigest(), hashlib.sha256(data[:-1]).hexdigest(),
    }:
        raise AssertionError('V_SOURCE_PLATFORM_MISMATCH')
    import cp6_v2620w_runtime as w_runtime
    successor = w_runtime.verified_successor(cur)
    observations = w_runtime.predecessor_snapshot(cur, 'V', successor)
    w_runtime.extend_items(successor, observations)
...
def effective_hash(successor, identity, predecessor):
    ...
    if predecessor != item['predecessor_sha256']:
        raise AssertionError('V_PREDECESSOR_CHAIN_MISMATCH')
...
def predecessor_snapshot(cur, generation, successor):
    ...
    if len(rows) != len(wanted) or {r['identity'] for r in rows} != set(wanted):
        raise AssertionError('V_HISTORICAL_CAPSULE_CARDINALITY_MISMATCH')
    for row in rows:
        ...
        if (
            ...
            raise AssertionError('V_HISTORICAL_CAPSULE_SOURCE_PIN_MISMATCH')
    return rows
...
def extend_items(successor, items):
    for item in items:
        if item['identity'] in successor:
            ...
            item['pre_v_installed_sha256'] = old
...
def verify_extra_objects(cur):
    """V creates no compatibility object; verify the complete inherited T set."""
