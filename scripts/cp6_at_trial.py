"""AT qualification preserves every AS oracle and adds WIP temporal controls."""
...
import cp6_as_runtime as predecessor_runtime
...
OUT=Path('cp6-proof/at')
BASE='5eb9de3ca59e3b86621ce1c158277e462d57efb4'
...
def source():
    ...
    allowed={'.github/workflows/cp6-at-qualification.yml','docs/cp6-at-qualification.md',
             str(runtime.build.MIGRATION),str(runtime.build.ROLLBACK),str(runtime.build.PINS),str(runtime.build.PROVENANCE)}
    allowed.update('scripts/cp6_at_'+name+'.py' for name in ('definitions','build','runtime','cases','races','trial','browser_fixture','browser_runtime'))
    allowed.add('scripts/cp6_at_browser_ui.mjs')
    assert set(changed)==allowed,('AT_SCOPE',changed)
    assert not subprocess.check_output(['git','diff','HEAD','--name-only'],text=True).strip(),'AT_TRACKED_WORKTREE_DIRTY'
...
def group(name,factory,stage='AT'):
    verify=runtime.verified if stage=='AT' else predecessor_runtime.verified
    ...
    with psycopg.connect(ADMIN) as conn,conn.cursor() as cur:
        ...
        for key,operation in cases:
            ...
            detail=row if name in ('NEW_CASES','AT_CASES','AS_EXTRA_PROBES') else {k:row.get(k) for k in ('status','error','traceback')}
    ...
    if report['complete_boundary_restored'] and not any(report['counts'][s] for s in ('INCOMPLETE','BUG_PROVEN','GAP_PROVEN','FAIL')):
        report['status']='PROBE_COMPLETE' if stage=='AS' else 'WRITER_PASS_WITH_HISTORICAL_HOLD' if report['counts']['DATE_POLICY_REVIEW_REQUIRED'] else 'WRITER_PASS' if not report['counts']['COUNTEREXAMPLE'] else 'COUNTEREXAMPLE'
    save(name,report)
    return report
...
def qualify(report):
    before=group('AS_EXTRA_PROBES',lambda cur,today:[r for r in temporal.cases(cur,today) if r[0].startswith('WIP_VERSION:')],stage='AS')
    report['predecessor_extra_probes']={k:before[k] for k in ('status','counts')}
    # Qualification uses a genuinely populated predecessor, like the AS gate.
