#!/usr/bin/env python3
"""AI pins all 533 live functions and the 157 unchanged AC relation objects."""
from pathlib import Path
import hashlib,json,os,subprocess
import cp6_v2620ac_runtime as ac
import cp6_v2620ah_runtime as predecessor
from cp6_v2620ai_build_sql import STAMP,NAME,VERSION,MIGRATION,ROLLBACK,PINS,CATALOG,PREDECESSOR_HEAD,PREDECESSOR_TREE

CAPSULE='erp.cp6_v2620ai_rollback_capsule'
PINS_SHA256='2afa10f94618b42ff297f0f894672db341b3323ee1f75af24a8c4e2570767300'
SOURCE_BASE='4337b2f4ede0a24a54a925eb8a789eb37f8362a4'

def pins():
 raw=PINS.read_bytes();assert hashlib.sha256(raw).hexdigest()==PINS_SHA256,'AI_TRUSTED_PINS_CHANGED'
 p=json.loads(raw)
 assert (p['predecessor_head'],p['predecessor_tree'],p['boundary_count'],p['predecessor_table_count'])==(PREDECESSOR_HEAD,PREDECESSOR_TREE,221,223)
 return p

def verify_source_files():
 p=pins();predecessor.verify_source_files()
 for path,expected in p['source_pins'].items():
  b=Path(path).read_bytes();assert len(b)==expected['bytes'] and hashlib.sha256(b).hexdigest()==expected['sha256'],'AI_SOURCE_DRIFT:'+path
...
def verify_audit_source():
 ...
 assert os.environ.get('GITHUB_SHA')==git('rev-parse','HEAD'),'AI_NATIVE_HEAD_REQUIRED'
...
def catalog(cur,successor):
 ...
 assert len(observed)==533 and {r[0] for r in observed}==set(expected),'AI_COMPLETE_FUNCTION_SET_DRIFT'
 result={}
 for identity,digest,owner,acl in observed:
  e=expected[identity];assert (digest,owner,acl)==(e['sha256'],e['owner'],e['acl']),'AI_LIVE_FUNCTION_DRIFT:'+identity
...
def verify_predecessor(cur):
 ...
 catalog(cur,False)
 assert not cur.execute("select to_regclass(%s) is not null or exists(select 1 from erp.schema_migrations where version=%s) or exists(select 1 from supabase_migrations.schema_migrations where name=%s)",(CAPSULE,VERSION,NAME)).fetchone()[0],'AI_PREDECESSOR_RESIDUE'
 return True
...
def verified_successor(cur,*,pre_admission=False):
 ...
 assert platform==[(STAMP,p['source_pins'][str(MIGRATION)]['sha256'])],'AI_PLATFORM_SOURCE_DRIFT'
 ...
 boundary=rows[0][-1];assert boundary is not None and len(boundary)==221
 for identity,before,actual,after,owner,acl,snapshot in rows:
  e=expected[identity]
  assert (before,actual,after,owner,acl)==(e['predecessor_sha256'],e['predecessor_sha256'],e['installed_sha256'],e['owner'],e['acl']) and snapshot==boundary,'AI_CAPSULE_DRIFT:'+identity
 # Historical capsules are immutable dependencies captured in the AI boundary.
 ...
 try:
  ...
  names=cur.execute("select relname from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='erp' and c.relkind='r' and relname like 'cp6%rollback_capsule' and relname<>'cp6_v2620ai_rollback_capsule' order by 1").fetchall()
  for (name,) in names:
   ...
   assert digest==boundary[name],'AI_INHERITED_CAPSULE_DRIFT:'+name
 ...
 assert len(objects)==690,'AI_RUNTIME_CARDINALITY'
 return objects
