"""AV rev2: one identity-family successor on exact AU (AU-R1, owner decisions 1C/2A, AV-GUARD-01).

Built like AU from AU's own pinned model: the predecessor catalog is the
...
from cp6_av_definitions import (OLD,FUNCTIONS,NEW_FUNCTIONS,PRIVATE_ACL,SCHEMA,TRIGGERS,ORIGINS,
                                HELPER,COVERAGE,ORIGIN_GUARD)

ROOT=Path(__file__).resolve().parents[1]
STAMP='20260923110000'
NAME='erp_v2_6_20av_cp6_identity_new_stock_cutoff'
VERSION='v2.6.20av'
...
PINS=ROOT/'docs/evidence/cp6-av-r2-pins.json'
DEFINITIONS=ROOT/'scripts/cp6_av_definitions.py'
...
def model():
    ...
    assert sha(CATALOG.read_bytes())==CATALOG_SHA,'AV_SCHEMA_CAPTURE_DRIFT'
    native=json.loads(CATALOG.read_text())
    assert native['status']=='CAPTURE_PASS' and native['complete_boundary_restored'] and native['primary_unchanged'] and not native['clone_remaining']
    assert native['definitions_sha256']==sha(DEFINITIONS.read_bytes()),'AV_CAPTURE_FOR_OTHER_DEFINITIONS'
    assert native['schema_sha256']==sha(SCHEMA) and native['triggers_sha256']==sha(TRIGGERS)
    assert native['before_sha256']=={k:sha(json.dumps(before[k],sort_keys=True,ensure_ascii=False)) for k in ('objects','functions')}
    assert not native['removed_objects'] and all(native['canonical_matches'].values())
    ...
    function_keys={'FUNCTION:'+k for k in NEW_FUNCTIONS}
    assert function_keys<=set(additions) and all(ORIGINS in k for k in set(additions)-function_keys)
    for identity,new in NEW_FUNCTIONS.items():
        assert native['new_function_pins'][identity]==dict(sha256=sha(new),owner='postgres',acl=PRIVATE_ACL)
        assert additions['FUNCTION:'+identity]==sha(json.dumps([new,'postgres',PRIVATE_ACL],ensure_ascii=False))
    ...
    for identity,new in FUNCTIONS.items():
        ...
        assert native['changed_function_pins'][identity]==dict(original,sha256=sha(new))
    ...
    assert len(after['objects'])==native['after_counts']['objects'] and len(after['functions'])==native['after_counts']['functions']
    pin=dict(version=VERSION,stamp=STAMP,name=NAME,migration=MIGRATION_REL,rollback=ROLLBACK_REL,capsule=CAP,previous='AU',
             revision=2,supersedes_candidate='bb4009ccd9d4bb40472de2e7886f9b9077a3c0e0',functions=functions,
             new_functions=[dict(identity=i,installed_sha256=sha(d),owner='postgres',acl=PRIVATE_ACL) for i,d in NEW_FUNCTIONS.items()],
             new_tables=[ORIGINS],schema_capture_sha256=CATALOG_SHA,schema_sha256=sha(SCHEMA),triggers_sha256=sha(TRIGGERS))
...
def build():
    ...
    s='-- CP6 AV rev2: NEW_STOCK physical cutoff, manual BS origin (1C), rework GOOD as existing stock (2A), fail-closed coverage registry.\n'+package.START
 ...
 if exists(select 1 from erp.schema_migrations where version={q(VERSION)}) or to_regclass({q(CAP)}) is not null
  or to_regclass({q('erp.'+ORIGINS)}) is not null or to_regprocedure({q(HELPER)}) is not null
  or to_regprocedure({q(COVERAGE)}) is not null or to_regprocedure({q(ORIGIN_GUARD)}) is not null
...
insert into erp.schema_migrations(version,description) values({q(VERSION)},'Identity family: NEW_STOCK physical cutoff, manual BS origin and rework GOOD as existing stock (AV rev2)');
"""+package.capsule_guard('AV',pin)+'commit;\n'
    MIGRATION.write_text(s);pin['migration_sha256']=sha(s)
    s='-- CP6 AV rev2: exact pre-use restore to AU under closed admission.\n'+package.START
