...
def build(old_definition):
    ...
    rebuild=substitute(rebuild,[("  select coalesce(sum(adjustment_amount),0),", "  v_pending:=v_pending or exists(select 1 from erp.initial_import_production_sources ps where ps.po_id=p_po_id and erp.be_pocket_item_pending_v1(ps.opening_item_id));\n  select coalesce(sum(adjustment_amount),0),")],'BE pending pocket receipt on opening WIP/BS')
