...
drop_clone() {
  docker exec "$CP6_DATABASE_CONTAINER" \
    dropdb -U supabase_admin --if-exists --force --maintenance-db=template1 "$database_name"
  remaining="$(docker exec "$CP6_DATABASE_CONTAINER" \
...
cleanup() {
  ...
  cleanup_status=$?
  set -e
  if test "$cleanup_status" -ne 0; then prior_status=1; fi
  printf 'clone_database=%s\nremaining_databases=0\nstatus=%s\n' \
    "$database_name" "$(test "$prior_status" -eq 0 && echo PASS || echo FAIL)" \
