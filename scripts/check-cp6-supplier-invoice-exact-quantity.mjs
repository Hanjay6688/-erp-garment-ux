...
let previous=-1
for(const token of ["Apply v2.6.20p supplier return match", "Reproduce independent P exact supplier invoice quantity", "Apply v2.6.20q exact supplier invoice quantity", "Run post-CP6 real Auth", "Prove native Q exact supplier invoice quantity", "Qualify exact F G H I J K L M N O P Q R S T U V W X and Y rollback", "Prove trusted Q capsule", "Prove trusted P capsule"]) {const at=workflow.indexOf(token);assert.ok(at>previous,token);previous=at}
for(const token of ["len(schedules['cases'])==400", "'expected':100,'observed':100", "setup_unit['completed_case_count']==setup_unit['expected_case_count']==229", "sum(c['upgrade_guard']['invalid_history_refused'] for c in q_before['cases'].values())==5", "CP6_V2620Y_RUNTIME_MANIFEST.json", "CP6_V2620Q_SUPPLIER_INVOICE_QUANTITY_REGRESSION.json", "PHYSICAL_DISPOSABLE_CP6_AUTH_CLONE_AFTER_V2620Y", "CP6_M_RACE_SOURCE_GENERATION: Y"]) assert.ok(workflow.includes(token),token)
...
console.log(JSON.stringify({status:'PASS',classification:'STATIC_SOURCE_CONTRACT_NOT_NATIVE_PROOF',replaced_functions:4,boundary_tables:67,native_cases:11,known_p_paths:7,controls:4,maintenance_schedules:400,writer_body_entries:100,production_go:false}))
