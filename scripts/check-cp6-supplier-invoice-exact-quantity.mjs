...
let previous=-1
for(const token of ["Apply v2.6.20p supplier return match", "Reproduce independent P exact supplier invoice quantity", "Apply v2.6.20q exact supplier invoice quantity", "Run post-CP6 real Auth", "Prove native Q exact supplier invoice quantity", "Qualify exact F G H I J K L M N O P Q R S and T rollback", "Prove trusted Q capsule", "Prove trusted P capsule"]) {const at=workflow.indexOf(token);assert.ok(at>previous,token);previous=at}
for(const token of ["len(schedules['cases'])==300", "'expected':75,'observed':75", "setup_unit['completed_case_count']==setup_unit['expected_case_count']==134", "sum(c['upgrade_guard']['invalid_history_refused'] for c in q_before['cases'].values())==5", "CP6_V2620T_RUNTIME_MANIFEST.json", "CP6_V2620Q_SUPPLIER_INVOICE_QUANTITY_REGRESSION.json", "PHYSICAL_DISPOSABLE_CP6_AUTH_CLONE_AFTER_V2620T", "CP6_M_RACE_SOURCE_GENERATION: T"]) assert.ok(workflow.includes(token),token)
...
console.log(JSON.stringify({status:'PASS',classification:'STATIC_SOURCE_CONTRACT_NOT_NATIVE_PROOF',replaced_functions:4,boundary_tables:67,native_cases:11,known_p_paths:7,controls:4,maintenance_schedules:300,writer_body_entries:75,production_go:false}))
