...
def final_receipt(cur,day,qty=10,price=10):
    ...
    material=prior.clone_material(cur,'azcc')
    location=uuid.uuid4()
...
def presewing_reversal(cur,today,price=None,installed=None):
    ...
    over the batch pieces, x 8 pcs), the PO's WIP never negative, no negative daily balance. On the invoice path (AY rev7.4 +
    AZ rev2.1) group 2's 6 units were in WIP from d+1 to today, so the invoice revalues them there (AZ) and the lot carries
    them until the reversal (AY): FG changes 10x x 8/pieces on d+1 and gives group 2's share back today; material at the
    invoice price for the units on hand each day (10 on d, 0 on d+1 and d+2, 6 today). `installed`: the phase's own marker
    (AY probe: AY; AZ probe: AZ)."""
    import sys
    azp=sys.modules[__name__];installed=installed or az_installed
    prod=chain.production
    d=today-timedelta(days=3);d2=d+timedelta(days=1)
    days=[d+timedelta(days=i) for i in range(4)]
    boundary.historical.prior.set_open_period(cur,d-timedelta(days=1))
    ...
    # The daily moves below are those of the queue or the invoice only (the reversal's own journals are before this).
    ...
    if price is not None:
        x=dec(price)-10
        stock={str(d):10,str(d2):0,str(d2+timedelta(days=1)):0,str(today):6}
        checks.update(fg_both_groups_at_invoice_price_from_lot_day=abs(move[str(d2)]['FG_INVENTORY']-10*x*8/eff)<=Decimal('0.01'),
                      fg_reversed_material_leaves_on_reversal_day=abs(move[str(today)]['FG_INVENTORY']-(4*x*8/eff-Decimal(60)*8/eff))<=Decimal('0.01'),
    ...
    status='PASS' if all(checks.values()) else ('COUNTEREXAMPLE' if not installed(cur) else 'FAIL')
    return dict(status=status,finding='writer 24 Sep 2026: a cutting issue reversed before sewing stayed in the HPP (AY rev7.4) and kept its old cost in WIP while the stock it came from was revalued (AZ rev2.1)',
