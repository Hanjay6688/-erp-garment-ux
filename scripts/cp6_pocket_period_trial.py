...
def production(a,cur,today,special=False):
    ...
    try:
        if special:
            ...
            receipts.rpc(a,cur,'set_contractor_hpp_policy_v1',json.dumps(dict(contractor_id=str(contractor),effective_from=str(today-timedelta(days=8)),is_special=True,attendance_required=False,reason='Special contractor fixture')),uuid.uuid4(),current[0] if current else None)
...
def lifecycle(a,cur,today,special=False,closed=False):
    ...
    before=ledger(cur);base=pocket.effects(cur);stock=pocket.read(a,cur,f['roll']['code'])['rolls'][0]['qty']
