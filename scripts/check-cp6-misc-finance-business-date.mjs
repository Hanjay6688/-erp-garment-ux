...
for(const token of ['Apply v2.6.20u canonical','Reproduce native U misc cash counterexamples before V',
 ...
 'Run post-CP6 real Auth','Prove native V misc cash document races','Qualify exact F G H I J K L M N O P Q R S T U and V rollback',
...
for(const token of ["len(schedules['cases'])==340","'expected':85,'observed':85",'CP6_V2620V_RUNTIME_MANIFEST.json',
 "len(vp['expanded_cases'])==21","len(vr['cases'])==3","len(vg['guards'])==7","len(vg['extra_object_preflight_guards'])==9",
 'PHYSICAL_DISPOSABLE_CP6_AUTH_CLONE_AFTER_V2620V','V_COMPLETE_U_CATALOG_RESTORE_MISMATCH',
 'setup_unit[\'completed_case_count\']==setup_unit[\'expected_case_count\']==169'])assert.ok(workflow.includes(token),token)
