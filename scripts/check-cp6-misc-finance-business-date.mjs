...
assert.ok(read('scripts/cp6_v2620m_subledger_races.py').includes("SOURCE_GENERATION not in ('M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W')"), 'M race source admission must include exact verified V')
...
for(const token of ['POST_THEN_REVERSE','POST_THEN_POST','POST_ABORT_THEN_POST','pg_blocking_pids(pid)',
 "SOURCE_GENERATION = 'W'",'matrix.verify_setup_source(SOURCE_GENERATION)','first.rollback()',"manual_prelock_count=0,retry_count=0",'REAL_AUTHENTICATED_SESSION_OWNER'])assert.ok(races.includes(token),token)
...
for(const token of ['Apply v2.6.20u canonical','Reproduce native U misc cash counterexamples before V',
 ...
 'Run post-CP6 real Auth','Prove native V misc cash document races','Qualify exact F G H I J K L M N O P Q R S T U V and W rollback',
...
for(const token of ["len(schedules['cases'])==360","'expected':90,'observed':90",'CP6_V2620W_RUNTIME_MANIFEST.json',
 "len(vp['expanded_cases'])==21","len(vr['cases'])==3","len(vg['guards'])==7","len(vg['extra_object_preflight_guards'])==9",
 'PHYSICAL_DISPOSABLE_CP6_AUTH_CLONE_AFTER_V2620W','V_COMPLETE_U_CATALOG_RESTORE_MISMATCH',
 'setup_unit[\'completed_case_count\']==setup_unit[\'expected_case_count\']==188'])assert.ok(workflow.includes(token),token)
...
console.log(JSON.stringify({status:'PASS',classification:'STATIC_SOURCE_CONTRACT_NOT_NATIVE_PROOF',replaced_functions:3,
 ...
 maintenance_schedules:360,writer_body_entries:90,rollback_setup_units:188,production_go:false}))
