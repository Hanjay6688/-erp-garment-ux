...
def pins():
    ...
    assert sha(Path(build.__file__).read_bytes())==p['builder_sha256'],'AR_BUILDER_DRIFT'
    assert sha(Path('scripts/cp6_ar_definitions.py').read_bytes())==p['definitions_sha256'],'AR_DEFINITIONS_DRIFT'
    for key in ('migration','rollback'):assert sha(Path(p[key]).read_bytes())==p[key+'_sha256'],'AR_SQL_DRIFT'
    return p

def verified(cur):
    p=pins();base,capture,before,after,_=build.model()
    cur.execute("set local search_path='';set local timezone='UTC'")
    assert function_pins(cur)==after['functions'],'AR_FUNCTION_OWNER_ACL_DRIFT'
    assert cur.execute(build.INVENTORY_SQL).fetchone()[0]==after['objects'],'AR_FULL_CATALOG_DRIFT'
    cur.execute(build.prior_guard(base,capture),prepare=False)
    cur.execute(build.package.capsule_guard('AR',p),prepare=False)
    ...
    return dict(stage='AR',function_count=len(after['functions']),object_count=len(after['objects']),full_catalog_verified=True,
...
def change(kind,pg,control_url):
    ...
    with psycopg.connect(pg,autocommit=True,application_name='cp6-ar-'+kind) as conn,psycopg.connect(control_url,autocommit=True) as control:
...
def advisors(pg,stage,path,baseline=None):
    def snapshot():
        with psycopg.connect(pg) as conn,conn.cursor() as cur:
            (aq.verified(cur) if stage=='AQ' else verified(cur))
    ...
    assert r.returncode==0,'AR_ADVISORS_FAILED'
    if r.stdout.strip():findings=json.loads(r.stdout)
    else:
        assert 'No issues found' in r.stderr.splitlines(),'AR_ADVISORS_MISSING_RESULT'
        findings=[]
    assert isinstance(findings,list) and all(isinstance(x,dict) and x.get('name') and x.get('level') for x in findings)
    assert snapshot()==before,'AR_ADVISORS_CHANGED_BOUNDARY'
...
def qualify(pg,admin,out):
    """The two pre-use cycles operate on the same seeded, nonempty AQ clone."""
    ...
    try:
        baseline=advisors(pg,'AQ',out/'AR_ADVISORS_BEFORE.json')
        for _ in range(2):
            ...
            with psycopg.connect(admin) as conn,conn.cursor() as cur:
                ...
                else:raise AssertionError('AR_OPEN_ROLLBACK_ADMITTED')
            assert snapshot()==boundary
            restored=change('rollback',pg,control);assert snapshot()==before
            report['cycles'].append(dict(install=installed,open_rollback_refused_atomically=True,restore=restored,exact_aq_data_and_history_restored=True))
        report['final_install']=change('install',pg,control)
        security=advisors(pg,'AR',out/'AR_ADVISORS_AFTER.json',baseline)
        ...
        report['status']='PASS'
    finally:(out/'AR_PACKAGE.json').write_text(json.dumps(report,indent=2,default=str)+'\n')

def refuse_post_use(pg,admin,out):
    """A used AR clone cannot be restored even after linked business reversals."""
    ...
    with psycopg.connect(pg,autocommit=True) as conn,psycopg.connect(control_url,autocommit=True) as control:
        ...
        try:
            ...
            with conn.transaction(),conn.cursor() as cur:
                ...
                try:
                    with conn.transaction():cur.execute(prior.sql_body(build.ROLLBACK.read_text()),prepare=False)
                except psycopg.Error as exc:assert 'AR_POST_USE_ROLLBACK_REFUSED' in str(exc)
                else:raise AssertionError('AR_USED_ROLLBACK_ADMITTED')
    ...
    (out/'AR_POST_USE_REFUSAL.json').write_text(json.dumps(dict(status='PASS',all_erp_data_unchanged=True,platform_unchanged=True,production_go=False,independent_acceptance=False),indent=2)+'\n')
