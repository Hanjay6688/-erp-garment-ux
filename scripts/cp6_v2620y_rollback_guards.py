#!/usr/bin/env python3
"""Trusted Y capsule guards and maintenance-only exact Y-to-X restore."""
...
REPORT = Path('cp6-proof/CP6_V2620Y_ROLLBACK_GUARDS.json')
DIRECT_REPORT = Path('cp6-proof/CP6_V2620Y_DIRECT_GUARD_DIAGNOSTICS.json')
MAINTENANCE_REPORT = Path('cp6-proof/V2620Y_MAIN_MAINTENANCE_ROLLBACK.json')
CLONE_ROOT = Path('cp6-proof/Y_TRUSTED_CAPSULE_GUARD')
DIRECT_GUARD_NAMES = (
...
def verify_capsule_fault() -> dict[str, Any]:
    ...
    with n_guards.disposable_clone_confirmation(matrix.CLONE):
        try:
            matrix.prepare('Y', 'REPORT', CLONE_ROOT, source_generation='Y')
            coherent_capsule_fault(matrix.CLONE)
            rejection = None
            try:
                maintenance.run_maintenance_rollback(
                    target_name='Y', target_pgurl=matrix.CLONE,
            ...
            if rejection is None or 'TRUSTED_PREDECESSOR_PIN_MISMATCH' not in rejection:
                raise AssertionError('X coherent capsule fault was not rejected')
            ...
            if observed.get('admission_closed') or observed.get('rollback_started'):
                raise AssertionError('X trust failure crossed mutation boundary')
            with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                marker = conn.execute("""select count(*) from erp.schema_migrations
                  where version='v2.6.20y'""").fetchone()
            if marker != (1,):
                raise AssertionError('X trust guard changed installed generation')
            return {
                'target': 'Y', 'status': 'PASS',
...
def table_boundary(cur: psycopg.Cursor) -> dict[str, Any]:
    cur.execute("""select c.relname from pg_class c
      ...
      where n.nspname='erp' and c.relkind in('r','p')
        and c.relname not in('schema_migrations','cp6_v2620y_rollback_capsule')
      order by c.relname""")
    names = [row[0] for row in cur.fetchall()]
    if len(names) != 210:
        raise AssertionError(f'Y_DIRECT_BOUNDARY_CARDINALITY:{len(names)}')
    result = {}
    for name in names:
...
def direct_guards(target_pgurl: str) -> list[dict[str, Any]]:
    ...
    with psycopg.connect(target_pgurl, autocommit=False) as conn:
        ...
        cases = (
            ('platform_bytes',
             "update supabase_migrations.schema_migrations set statements=array['tampered W'] where name='erp_v2_6_20y_cp6_cash_business_dates'",
             'Y_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR', None),
            ('successor',
            ...
            ('definition_drift',
             ...
             'Y_TRUSTED_PREDECESSOR_PIN_MISMATCH', None),
            ('acl_drift',
             'grant execute on function erp.post_sales_payment(uuid) to anon',
             'Y_TRUSTED_PREDECESSOR_PIN_MISMATCH', None),
            ('boundary_drift',
             "update erp.cp6_v2620y_rollback_capsule set boundary_snapshot=boundary_snapshot-'supplier_payments'",
             'Y_BOUNDARY_SNAPSHOT_MISMATCH', None),
            ('post_install_boundary_history',
             ...
             'Y_POST_USE_ROLLBACK_REFUSED', None),
            ('post_install_other_module_history',
             "update erp.app_permissions set description=description||' [CP6 Y boundary probe]' where permission_key=(select permission_key from erp.app_permissions order by permission_key limit 1)",
             'Y_POST_USE_ROLLBACK_REFUSED: app_permissions', None),
            ('coherent_capsule_and_checksum', None,
             'Y_TRUSTED_PREDECESSOR_PIN_MISMATCH', coherent),
        )
        if tuple(case[0] for case in cases) != DIRECT_GUARD_NAMES:
            raise AssertionError('Direct Y guard case manifest drift')
                    ...
                    with conn.cursor() as cur:
                        ...
                        if coherent_definition is None:
                            ...
                            if mutation.lstrip().lower().startswith(('update ', 'insert ', 'delete ')):
                                ...
                                if cur.rowcount <= 0:
                                    raise AssertionError(f'Y_EMPTY_GUARD_MUTATION:{name}:{cur.rowcount}')
                        else:
                            cur.execute("""update erp.cp6_v2620y_rollback_capsule
                              set object_definition=%s,
                                definition_sha256=encode(extensions.digest(
                            ...
                            detail['affected_rows'] = cur.rowcount
                            if cur.rowcount != 1:
                                raise AssertionError(f'Y_COHERENT_GUARD_CARDINALITY:{cur.rowcount}')
                        if name == 'post_install_other_module_history':
                            ...
                            if detail['affected_rows'] != 1 or detail['changed_tables'] != ['app_permissions']:
                                raise AssertionError('Y_OTHER_MODULE_MUTATION_NOT_ISOLATED')
                            if mutated['app_permissions']['rows'] != original_boundary['app_permissions']['rows']:
                                raise AssertionError('Y_PERMISSION_DESCRIPTION_CHANGED_ROW_COUNT')
                ...
                with conn.cursor() as cur:
                    ...
                    if name == 'post_install_other_module_history':
                        ...
                        if not detail['full_boundary_restored']:
                            raise AssertionError('Y_OTHER_MODULE_GUARD_LEFT_TABLE_RESIDUE')
                conn.commit()
...
def verify_extra_preflight_guards() -> list[dict[str, Any]]:
    ...
    with n_guards.disposable_clone_confirmation(matrix.CLONE):
        for name, mutation, expected in EXTRA_FAULTS:
            ...
            try:
                matrix.prepare('Y', 'REPORT', folder, source_generation='Y')
                with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                ...
                if rejection is None or expected not in rejection:
                    raise AssertionError(
                        'Y_EXTRA_PREFLIGHT_WRONG_REJECTION: ' + name
                    )
                observed = matrix.read_json_if_present(
                ...
                if observed.get('admission_closed') or observed.get('rollback_started'):
                    raise AssertionError(
                        'Y_EXTRA_PREFLIGHT_MUTATED_ADMISSION: ' + name
                    )
                ...
                with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                    marker = conn.execute("""select count(*)
                      from erp.schema_migrations where version='v2.6.20y'""").fetchone()
                if admission != (True,) or marker != (1,):
                    raise AssertionError(
                        'Y_EXTRA_PREFLIGHT_STATE_CHANGED: ' + name
                    )
                results.append({
...
def verify_exact_x_restore(target_pgurl: str) -> dict[str, Any]:
    expected = maintenance.TRUSTED_FUNCTIONS['Y']
    with psycopg.connect(target_pgurl, autocommit=False) as conn, conn.cursor() as cur:
        ...
        cur.execute("""select
          not exists(select 1 from erp.schema_migrations where version='v2.6.20y')
          and exists(select 1 from erp.schema_migrations where version='v2.6.20x')
          and not exists(select 1 from supabase_migrations.schema_migrations
            where name='erp_v2_6_20y_cp6_cash_business_dates')
          and to_regclass('erp.cp6_v2620y_rollback_capsule') is null
          and to_regclass('erp.cp6_v2620v_rollback_capsule') is not null
          and to_regclass('erp.cp6_v2620w_rollback_capsule') is not null
          and to_regclass('erp.cp6_v2620x_rollback_capsule') is not null
        ...
        if cur.fetchone()[0] is not True:
            raise AssertionError('X rollback metadata/schema residue')
    return {
        'generation': 'X', 'restored_function_count': len(actual),
...
def run() -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        'head': os.environ.get('GITHUB_SHA', 'LOCAL_UNBOUND'),
        'classification': (
            'DISPOSABLE_NATIVE_Y_TRUSTED_GUARDS_AND_CLOSED_ADMISSION_RESTORE'
        ),
    ...
    maintenance_result = maintenance.run_maintenance_rollback(
        target_name='Y', target_pgurl=target_pgurl,
    ...
    if maintenance_result['status'] != 'PASS':
        raise AssertionError('X maintenance rollback failed')
    required_phases = {
    ...
    if not required_phases.issubset(observed_phases):
        raise AssertionError('X maintenance phase evidence incomplete')
    result.update(
        status='PASS',
        exact_pre_use_restore=verify_exact_x_restore(target_pgurl),
        maintenance={
            'status': 'PASS', 'target': 'Y',
...
def main() -> None:
    ...
    try:
        ...
        result = {
            'head': os.environ.get('GITHUB_SHA', 'LOCAL_UNBOUND'),
            'status': 'FAIL',
            'error_code': 'V2620Y_ROLLBACK_GUARD_FAILED',
