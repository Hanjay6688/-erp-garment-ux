#!/usr/bin/env python3
"""Trusted U capsule guards and maintenance-only exact U-to-T restore."""
...
REPORT = Path('cp6-proof/CP6_V2620U_ROLLBACK_GUARDS.json')
MAINTENANCE_REPORT = Path('cp6-proof/V2620U_MAIN_MAINTENANCE_ROLLBACK.json')
CLONE_ROOT = Path('cp6-proof/U_TRUSTED_CAPSULE_GUARD')
DIRECT_GUARD_NAMES = (
...
def verify_capsule_fault() -> dict[str, Any]:
    ...
    with n_guards.disposable_clone_confirmation(matrix.CLONE):
        try:
            matrix.prepare('U', 'REPORT', CLONE_ROOT, source_generation='U')
            coherent_capsule_fault(matrix.CLONE)
            rejection = None
            try:
                maintenance.run_maintenance_rollback(
                    target_name='U', target_pgurl=matrix.CLONE,
...
def direct_guards(target_pgurl: str) -> list[dict[str, Any]]:
    ...
    with psycopg.connect(target_pgurl, autocommit=False) as conn:
        ...
        cases = (
            ('platform_bytes',
             "update supabase_migrations.schema_migrations set statements=array['tampered U'] where name='erp_v2_6_20u_cp6_canonical_business_date'",
             'U_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR', None),
            ('successor',
             ...
             'U_ROLLBACK_PLATFORM_IDENTITY_OR_SUCCESSOR', None),
            ('definition_drift',
             'alter function erp.post_material_adjustment(uuid) cost 999',
             'U_TRUSTED_PREDECESSOR_PIN_MISMATCH', None),
            ('acl_drift',
             'grant execute on function erp.post_material_adjustment(uuid) to anon',
             'U_TRUSTED_PREDECESSOR_PIN_MISMATCH', None),
            ('boundary_drift',
             "update erp.cp6_v2620u_rollback_capsule set boundary_snapshot=boundary_snapshot-'supplier_payments'",
             'U_BOUNDARY_SNAPSHOT_MISMATCH', None),
            ('post_install_boundary_history',
             "update erp.cp6_v2620t_rollback_capsule set boundary_snapshot=boundary_snapshot||'{\"u_post_use_probe\":true}'::jsonb",
             'U_POST_USE_ROLLBACK_REFUSED', None),
            ('coherent_capsule_and_checksum', None,
             'U_TRUSTED_PREDECESSOR_PIN_MISMATCH', coherent),
        )
        if tuple(case[0] for case in cases) != DIRECT_GUARD_NAMES:
            raise AssertionError('Direct U guard case manifest drift')
...
def verify_extra_preflight_guards() -> list[dict[str, Any]]:
    ...
    with n_guards.disposable_clone_confirmation(matrix.CLONE):
        for name, mutation, expected in EXTRA_FAULTS:
            ...
            try:
                matrix.prepare('U', 'REPORT', folder, source_generation='U')
                with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                ...
                if rejection is None or expected not in rejection:
                    raise AssertionError(
                        'U_EXTRA_PREFLIGHT_WRONG_REJECTION: ' + name
                    )
                observed = matrix.read_json_if_present(
                ...
                if observed.get('admission_closed') or observed.get('rollback_started'):
                    raise AssertionError(
                        'U_EXTRA_PREFLIGHT_MUTATED_ADMISSION: ' + name
                    )
                ...
                with psycopg.connect(matrix.CLONE, autocommit=True) as conn:
                    marker = conn.execute("""select count(*)
                      from erp.schema_migrations where version='v2.6.20u'""").fetchone()
                if admission != (True,) or marker != (1,):
                    raise AssertionError(
                        'U_EXTRA_PREFLIGHT_STATE_CHANGED: ' + name
                    )
                results.append({
...
def verify_exact_t_restore(target_pgurl: str) -> dict[str, Any]:
    expected = maintenance.TRUSTED_FUNCTIONS['U']
    with psycopg.connect(target_pgurl, autocommit=False) as conn, conn.cursor() as cur:
        ...
        cur.execute("""select
          not exists(select 1 from erp.schema_migrations where version='v2.6.20u')
          and exists(select 1 from erp.schema_migrations where version='v2.6.20t')
          and not exists(select 1 from supabase_migrations.schema_migrations
            where name='erp_v2_6_20u_cp6_canonical_business_date')
          and to_regclass('erp.cp6_v2620u_rollback_capsule') is null
          and to_regclass('erp.cp6_v2620t_rollback_capsule') is not null
        ...
        if cur.fetchone()[0] is not True:
            raise AssertionError('U rollback metadata/schema residue')
    return {
        'generation': 'T', 'restored_function_count': len(actual),
...
def run() -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        'classification': (
            'DISPOSABLE_NATIVE_U_TRUSTED_GUARDS_AND_CLOSED_ADMISSION_RESTORE'
        ),
    ...
    maintenance_result = maintenance.run_maintenance_rollback(
        target_name='U', target_pgurl=target_pgurl,
    ...
    result.update(
        status='PASS',
        exact_pre_use_restore=verify_exact_t_restore(target_pgurl),
        maintenance={
            'status': 'PASS', 'target': 'U',
...
def main() -> None:
    ...
    try:
        ...
        result = {
            'head': os.environ.get('GITHUB_SHA', 'LOCAL_UNBOUND'),
            'status': 'FAIL',
            'error_code': 'V2620U_ROLLBACK_GUARD_FAILED',
