#!/usr/bin/env python3
"""Twenty AK schedules using the unchanged established concurrency oracle."""
...
# Bind only the controller dependency to the AK target addition. The matrix
# schedule, business sentinel, drain and refusal assertions stay byte-identical.
spec = importlib.util.spec_from_file_location('ak_matrix_controller', runtime.ROOT / 'scripts/cp6_preuse_rollback_maintenance.py')
controller = importlib.util.module_from_spec(spec)
spec.loader.exec_module(controller)
matrix.maintenance = controller


ROOT = Path("cp6-proof/AK_MAINTENANCE_ROLLBACK")
REPORT = ROOT / "manifest.json"


def prepare(target, operation, folder, *, source_generation):
    """Bind the old oracle's fixture setup to AK -> AJ, with exact verification."""
    if target != "AK" or source_generation != "AK":
        raise AssertionError("AK_SCHEDULE_REQUIRES_EXACT_EDGE")
    matrix.command(
    ...
    with matrix.legacy.connect("ak-clone-source") as conn, conn.cursor() as cur:
        if len(runtime.verified_successor(cur)) != 690:
            raise AssertionError("AK_CLONE_RUNTIME_MISMATCH")
    matrix.maintenance_strip("AK", folder)
    with matrix.legacy.connect("ak-predecessor-fixture") as conn, conn.cursor() as cur:
    ...
    with matrix.legacy.connect("ak-exact-platform-ledger") as conn, conn.cursor() as cur:
        cur.execute(
        ...
        if len(runtime.verified_successor(cur)) != 690:
            raise AssertionError("AK_SEEDED_RUNTIME_MISMATCH")
        conn.commit()
    capsule = matrix.legacy.capture_capsule("AK")
    if len(capsule) != 8:
        raise AssertionError("AK_SCHEDULE_CAPSULE_CARDINALITY")
...
def run():
    ...
    controller.TARGETS['AK']['rollback'] = runtime.reviewed_local_rollback()
    ...
    if actual != expected or os.environ.get("CP6_AK_MAINTENANCE_CONFIRM") != "cp6_rollback":
        raise AssertionError("AK_MAINTENANCE_EXACT_DISPOSABLE_ENVIRONMENT_REQUIRED")
    with psycopg.connect(matrix.SOURCE) as conn, conn.cursor() as cur:
        if len(runtime.verified_successor(cur)) != 690:
            raise AssertionError("AK_MAINTENANCE_SOURCE_MISMATCH")
    matrix.TARGETS["AK"] = (runtime.STAMP, runtime.NAME, runtime.VERSION, "v2.6.20aj", 8)
    original_prepare = matrix.prepare
    original_path = matrix.path
    matrix.path = lambda target,rollback=False: (runtime.ROLLBACK if rollback else runtime.MIGRATION) if target=="AK" else original_path(target,rollback)
    matrix.prepare = prepare
    result = {
        "format": "CP6_AK_MAINTENANCE_SCHEDULES_V1", "status": "INCOMPLETE",
    ...
    try:
        for operation in matrix.OPERATIONS:
            for mode in matrix.MODES:
                name = f"AK-{operation}-{mode}"
                folder = ROOT / name
                folder.mkdir(parents=True, exist_ok=True)
                try:
                    evidence = matrix.run_case("AK", operation, mode, folder, source_generation="AK")
                except Exception as exc:
                    evidence = {
                        "status": "FAIL", "target": "AK", "operation": operation,
