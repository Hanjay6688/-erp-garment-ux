"""Reconstructed audit scenario for initial-import DRAFT discovery — Fable fix rev1: the _actor permission join used a
non-existent column (erp.app_permissions.id); joined on permission_key instead (GPT batch run 36065350201: INCOMPLETE on this fixture).
...
def _actor(cur):
    ...
    permissions = [r[0] for r in cur.execute(
        """select p.permission_key from erp.app_role_permissions rp
           join erp.app_permissions p on p.permission_key=rp.permission_key
