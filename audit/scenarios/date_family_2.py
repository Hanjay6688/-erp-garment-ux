"""AUDITOR SCENARIO — DATE FAMILY 2 (fixture fixed: cuts after the 23:30 receipt; S04 probe added) (independent auditor, blind phase 1).
...
def material_shape(cur,today,price,cuts,invoice_offset,closed=None):
    ...
    last=mv[str(days[-1])];checks['correction_conserved_in_four_accounts_by_today']=sum(D(last[k]) for k in KEYS)==10*x
...
def same_day_cut_before_receipt(cur,today,price):
    """Receipt lands on d at 23:30 (writer fixture). A cut of 4 units on d at 08:00 precedes the physical arrival.
    Contract AUD-S04 (M:6002-6026): backdated/historical postings need per-location historical balance validation, not
    only current stock. Expected: the cutting RPC refuses (no units at 08:00). Observed both outcomes and the later
    invoice behaviour are recorded verbatim."""
    prod=chain.production
    d=today-timedelta(days=3);E=d+timedelta(days=2);days=[d+timedelta(days=i) for i in range(4)]
    boundary.historical.prior.set_open_period(cur,d-timedelta(days=1))
    awp.quiet_seed(cur,d,today-timedelta(days=1),exclude=[prod.CONTRACTOR])
    fx=prod.estimated_receipt(cur,today)
    api.admin(cur)
    received_at=str(cur.execute('select physical_at from erp.material_purchase_headers where id=%s',(fx['purchase'],)).fetchone()[0])
    cut_result,cut_error=awp.r1.peer.attempt(cur,lambda:azp.cut(cur,fx,d,4,hour=8))
    api.admin(cur)
    obs=dict(receipt_physical_at=received_at,cut_at=str(prod.at(d,8)),cut_accepted=cut_error is None,cut_error=(cut_error or {}).get('message') if cut_error else None)
    if cut_error is None:
        stock_at_0900=cur.execute("""select coalesce(sum(case when movement_type in('IN','RECEIPT') then qty_signed else qty_signed end),0) from erp.material_stock_movements
            where roll_id=%s and physical_at<=%s""",(fx['roll'],prod.at(d,9))).fetchone()[0] if cur.execute("select to_regclass('erp.material_stock_movements') is not null").fetchone()[0] else None
        obs['roll_movement_sum_at_0900']=str(stock_at_0900)
        inv_result,inv_error=awp.r1.peer.attempt(cur,lambda:azp.invoice(cur,fx,today,price,E))
        api.admin(cur)
        obs['late_invoice_accepted']=inv_error is None
        obs['late_invoice_error']=(inv_error or {}).get('message') if inv_error else None
    status='COUNTEREXAMPLE' if cut_error is None else 'PASS'
    return dict(status=status,checks=dict(cut_before_receipt_refused=cut_error is not None),observed=obs,
                expected='AUD-S04 (M:6002-6026): a cut at 08:00 from a roll received 23:30 the same day must be refused by historical per-location validation; if accepted, the later invoice must still be bookable')
...
def cases(cur,today):
    ...
    def wrap(fn,*a):
        ...
        return run
    return [('DATE2:OPEN:INVOICE_AFTER_CUT:HIGHER_12',wrap(material_shape,'12',[(1,4)],2)),
            ('DATE2:OPEN:INVOICE_AFTER_CUT:LOWER_8',wrap(material_shape,'8',[(1,4)],2)),
            ('DATE2:OPEN:TWO_CUTS_INVOICE_LAST_CUT_DAY:LOWER_8',wrap(material_shape,'8',[(1,3),(2,4)],2)),
            ('DATE2:OPEN:INVOICE_BETWEEN_TWO_CUTS_AMBIGUOUS:LOWER_8',wrap(material_shape,'8',[(1,3),(2,4)],1)),
            ('DATE2:CLOSED:CUT_CLOSED_INVOICE_DATED_IN_CLOSED:HIGHER_12',wrap(material_shape,'12',[(1,4)],0,1)),
            ('DATE2:CLOSED:CUT_CLOSED_INVOICE_DATED_IN_CLOSED:LOWER_8',wrap(material_shape,'8',[(1,4)],1,1)),
            ('DATE2:S04:SAME_DAY_CUT_BEFORE_RECEIPT_TIME:LOWER_8',wrap(same_day_cut_before_receipt,'8')),
            ('DATE2:GOODS:INVOICE_BEFORE_GOODS_AMBIGUOUS:HIGHER_12',wrap(goods_shape,'12',False))]
