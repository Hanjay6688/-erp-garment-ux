      ...
      rows.push(['G8UI:WIB:'+kind+':'+zone,async()=>{
        ...
        try{
          if(kind==='cutting'){
            ...
            await page.locator('.cpick-setup select').selectOption(f.contractor)
          ...
          const visible=await page.locator('.connected-pickup-page').innerText().catch(()=>'<pickup page not available>')
          return {status:'INCOMPLETE',zone,kind,error:String(error),visible_form:visible.slice(0,4500),fixture:f}
