...
from cp6_aw_probe import boundary
...
def case(cur, today, kind, backdated):
    ...
    key = uuid.uuid4()
    api.admin(cur)
    refusal_boundary = boundary.snapshot(cur)
    ...
    try:
        ...
        atomic = boundary.snapshot(cur) == refusal_boundary
        exact = (backdated and error['sqlstate'] == 'P0001' and
                 error['message'].split(':', 1)[0] == 'BA_ADVANCE_DATED_CAPACITY')
        return plain(dict(status="PASS" if exact and atomic and after_refusal == before_refund and source(cur, fx) == original else "INCOMPLETE", kind=kind, backdated=backdated,
            stage="REFUND_REFUSED_UNCLASSIFIED", refusal=error,
            full_refusal_boundary_unchanged=atomic,
            atomic_event_observation=after_refusal == before_refund,
            source_unchanged=source(cur, fx) == original,
            expected="C0 section4 (D02), hash e83d56e6: dated refund requires BA_ADVANCE_DATED_CAPACITY; all domain effects unchanged."))
    cur.execute("release savepoint advance_refund_attempt")
    api.admin(cur)
