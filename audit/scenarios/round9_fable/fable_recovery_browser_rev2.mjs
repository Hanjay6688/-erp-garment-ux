// Fable rev2 (round 9, tool head d1bc8ad): GPT's frozen recovery cases with the error locator narrowed to role="alert",
// because the W10 fix adds a role="status" banner with the same class (strict-mode violation in run 36123210828).
// Oracle unchanged (M1679/M3819). Unknown-read cases dropped here (covered by unknown_round8 rev5/rev6).
...
async function patternRetry(ui) {
  ...
  try {
    ...
    await ui.expect(page.locator('.pattern-error[role="alert"]')).toBeVisible()
    ...
    return { status: sameEnvelope && replayExact && afterRetry.length === 1 ? 'PASS' : 'COUNTEREXAMPLE',
      ...
      wire: wire.records, visible_error: await page.locator('.pattern-error[role="alert"]').innerText().catch(() => ''),
      pending_banner: await page.locator('.pattern-error[role="status"]').innerText().catch(() => ''),
...
async function roleRetry(ui) {
  ...
  try {
    ...
    await save.click()
    await ui.expect.poll(() => wire.records.length).toBe(1)
    await ui.expect(page.locator('.access-error[role="alert"]')).toBeVisible()
    await ui.expect(save).toBeEnabled()
    ...
    return { status: sameEnvelope && replayExact && afterRetry.length === 1 ? 'PASS' : 'COUNTEREXAMPLE',
      ...
      visible_error: await page.locator('.access-error[role="alert"]').innerText().catch(() => ''),
      pending_banner: await page.locator('.access-error[role="status"]').innerText().catch(() => ''),
      privilege_escalation_claimed: false }
  } finally { await u.context.close() }
}
function fixture(scope, today) {
  const raw = execFileSync('python', [resolve(here, 'gpt_unknown_fixture.py'), scope, String(today)],
...
async function unknownRead(ui, today, scope) {
  ...
  try {
    ...
    await ui.expect(target).toHaveText(String(expectedPositive))
...
export async function cases(ui, today) {
  return [
    ['F9UI:RECOVERY:PATTERN_COMMIT_REPLY_LOST:rev2', () => patternRetry(ui)],
    ['F9UI:RECOVERY:ROLE_DUPLICATE_COMMIT_REPLY_LOST:rev2', () => roleRetry(ui)],
  ]
}
