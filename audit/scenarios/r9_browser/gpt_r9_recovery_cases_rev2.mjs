// Rev2 adapts only UI selectors/replay trigger to pending status markup on d1bc8ad.
...
async function patternRetry(ui) {
  ...
  try {
    ...
    await ui.expect(page.locator('.pattern-error[role="status"]')).toBeVisible()
    ...
    await d.getByRole('button', { name: 'Batal', exact: true }).click()
    await page.locator('.pattern-error[role="status"]').getByRole('button', { name: 'Kirim ulang perubahan tertunda' }).click()
    ...
    return { status: sameEnvelope && replayExact && afterRetry.length === 1 ? 'PASS' : 'COUNTEREXAMPLE',
      ...
      wire: wire.records, visible_error: await page.locator('.pattern-error[role="status"]').innerText().catch(() => ''),
...
async function roleRetry(ui) {
  ...
  try {
    ...
    await save.click()
    await ui.expect.poll(() => wire.records.length).toBe(1)
    await ui.expect(page.locator('.access-error[role="status"]')).toBeVisible()
    await ui.expect(save).toBeEnabled()
    ...
    if (!emptySuccess(wire.records[0]) || afterFirst.length !== 1) return { status: 'INCOMPLETE', stage: 'FIRST_SAVE_MUST_COMMIT', wire: wire.records, afterFirst }
    await d.getByRole('button', { name: 'Batal', exact: true }).click()
    await page.locator('.access-error[role="status"]').getByRole('button', { name: 'Kirim ulang perubahan tertunda' }).click()
    await ui.expect.poll(() => wire.records.length).toBe(2)
...
export async function cases(ui, today) {
  return [
    ['G8UI:RECOVERY:PATTERN_COMMIT_REPLY_LOST', () => patternRetry(ui)],
    ['G8UI:RECOVERY:ROLE_DUPLICATE_COMMIT_REPLY_LOST', () => roleRetry(ui)],
    ['G8UI:UNKNOWN:LAUNDRY_INITIAL_READ', () => unknownRead(ui, today, 'LAUNDRY')],
    ['G8UI:UNKNOWN:QC_INITIAL_READ', () => unknownRead(ui, today, 'QC')],
  ]
}
