"""Disposable rev4 setup. Product master identities come from public initial-import RPC.
...
def fixture(scope, today):
    ...
    with psycopg.connect(url) as conn, conn.cursor() as raw:
        ...
        """, (model,size)).fetchall()
        api.ordinary(cur)
        policy = cur.execute('select erp.set_contractor_hpp_policy_v1(%s::jsonb,%s,null)', (
            json.dumps(dict(contractor_id=str(contractor),effective_from=str(today-timedelta(days=30)),
                            is_special=False,attendance_required=False,reason='GPT disposable valid-UUID browser fixture')),
        ...
        return dict(scope=scope,query=code,opening_batch=opening['batch'],
                    ...
                    contractor_policy_api='erp.set_contractor_hpp_policy_v1',
