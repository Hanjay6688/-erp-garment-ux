// Rev4: actual Auth/browser/HTTP, unchanged candidate parser; no response replacement.
...
function fixture(scope,today){
 const raw=execFileSync('python',[resolve(here,'gpt_unknown_fixture_rev4.py'),scope,String(today)],
...
async function healthy(ui,today,scope){
 ...
 try{
   ...
   u=await ui.login('OWNER',{label:'unknown4-control-'+scope})
...
async function unknown(ui,today,scope){
 ...
 let u
 try{
   u=await ui.login('OWNER',{label:'unknown4-fault-'+scope})
   const {page}=u
   let dropped=0
...
export async function cases(ui,today){
 return[
  ['G8UI:UNKNOWN:LAUNDRY_HEALTHY_CONTROL:rev4',()=>healthy(ui,today,'LAUNDRY')],
  ['G8UI:UNKNOWN:LAUNDRY_INITIAL_READ:rev4',()=>unknown(ui,today,'LAUNDRY')],
  ['G8UI:UNKNOWN:QC_HEALTHY_CONTROL:rev4',()=>healthy(ui,today,'QC')],
  ['G8UI:UNKNOWN:QC_INITIAL_READ:rev4',()=>unknown(ui,today,'QC')],
 ]
}
