...
const parserSha256 = createHash('sha256').update(parserSource).digest('hex')
...
function inspectParser(body) {
  try {
    const w = parserModule.exports.parseLaundryQcWorkspace(body)
    return { ok: true, source_sha256: parserSha256, scope: w.scope }
  } catch (e) { return { ok: false, source_sha256: parserSha256, error: String(e.message) } }
}
...
async function unknownRead(ui, today, scope) {
  ...
  page.on('requestfailed', req => { if (req.url().includes('/rpc/erp_get_laundry_qc_workspace_v1')) failedRequests.push(req.failure()?.errorText) })
  try {
    ...
    const controlParser = inspectParser(ws)
    ...
    const refetchParser = inspectParser(refetchBody)
    let freshReload = null
    if (renderError && refetchParser.ok) {
      await page.reload()
      await ui.expect(page.locator('.top-title strong')).toBeVisible()
      await navigate(page, label)
      const freshTarget = page.locator('.clq-kpis article').nth(scope === 'LAUNDRY' ? 1 : 0).locator('strong')
      try {
        await ui.expect(freshTarget).toHaveText(String(expectedPositive))
        freshReload = { rendered: true, qty: await freshTarget.innerText() }
      } catch (e) {
        freshReload = { rendered: false, qty: await freshTarget.innerText().catch(() => null),
          error_ui: await page.locator('.clq-alert.error').innerText().catch(() => ''), error: String(e.message).slice(0, 800) }
      }
    }
    if (renderError) return { status: refetchParser.ok && freshReload?.rendered ? 'COUNTEREXAMPLE' : 'INCOMPLETE',
      stage: 'POSITIVE_REFETCH_CONTROL', scope, fixture: f, actual_server_qty: expectedPositive,
      ...
      controlParser, refetchParser, freshReload, failedRequests,
      refetch_workspace: refetchBody, renderError,
      oracle: 'M3825 unknown is not zero; M3819/3826 valid reload must recover. A genuine fresh-browser positive control is required before attributing failed refetch to product state.' }
    ...
    return { status: falseZero ? 'COUNTEREXAMPLE' : 'PASS', oracle: 'M3825: failed initial read is unknown, not an authoritative zero; successful refetch restores actual nonzero value',
      ...
      controlParser, refetchParser, failedRequests, dropped_reads: dropped, initial_kpis: cards, primary, initialError, initialWriterLocked,
...
export async function cases(ui, today) {
  return [
    ['G8UI:UNKNOWN:LAUNDRY_INITIAL_READ:rev3', () => unknownRead(ui, today, 'LAUNDRY')],
    ['G8UI:UNKNOWN:QC_INITIAL_READ:rev3', () => unknownRead(ui, today, 'QC')],
  ]
}
