...
| ACC-DEC05 | Kredit retur nota mandor | Cara aksesori yang dikembalikan mandor mengurangi nota | Mode: `CREDIT_UNPAID_ONLY`, `CREDIT_THEN_CARRY`, atau `CREDIT_THEN_REFUND`. Kondisi yang dikreditkan: `USABLE` dan/atau `DAMAGED` | **Perlu keputusan owner.** `CREDIT_UNPAID_ONLY`: barang layak yang dikembalikan sesudah nota lunas tidak mendapat kredit. `CREDIT_THEN_CARRY`: bagian nota yang belum dibayar dikreditkan, sisanya dibawa sebagai utang ke mandor dan dibayar sekali lewat payroll berikutnya (diuji probe BC `ACC-C09`). `CREDIT_THEN_REFUND`: sisanya dibayar tunai. Kalau retur sesudah lunas memang boleh, yang sesuai adalah `CREDIT_THEN_CARRY`. Kondisi yang dikreditkan: `USABLE` | Kredit retur nota (`CREDIT_NOTE_RETURN`) ditolak |
...
- **Belum lengkap:** no. 6 (ACC-DEC07), menunggu nominal owner.
- **Perlu keputusan owner:** no. 4 (ACC-DEC05), no. 11 (LAU-DEC04), no. 13 (LAU-DEC06, bagian sesudah bayar).
- **Tidak perlu diisi:** no. 12 (LAU-DEC05).
