...
**Usulan Claude untuk 2b (celup ulang BS → SKU baru), menunggu konfirmasi owner:**
- Perlakukan sebagai **konversi**, polanya sama dengan Ganti Merek: sumber keluar sebagai stok lama (EXISTING_STOCK), hasil masuk sebagai stok baru SKU tujuan (NEW_STOCK). Ini konsisten dengan keputusan 2A: rework yang kembali ke SKU yang sama adalah stok lama, sedangkan celup yang mengubah identitas menghasilkan stok baru bagi SKU tujuan.
- Asal dicatat **saat BS dikirim ke celup**, bukan diingat belakangan. Pada saat itu barang masih berwarna asli, jadi model, warna asal, ukuran, dan qty terlihat. Yang biasanya tidak diketahui hanya PO asal.
- Bila PO asal tidak diketahui, sistem mengalokasikan BS dari SKU dan ukuran itu secara FIFO (BS tertua lebih dulu) dan memberi label "alokasi sistem", bukan fakta. Qty dan nilai total tetap tepat di tingkat SKU dan ukuran; yang bisa sedikit meleset hanya atribusi per PO.
- Terima hasil celup: GOOD masuk SKU baru, reject tetap tercatat sebagai BS, dan biaya celup vendor mengikuti pola kirim–terima–invoice seperti laundry (harga belum diketahui tidak menjadi Rp0).
- Jangan memakai jalan pintas "rework ke SKU asal lalu Ganti Merek". Cara itu mengarang stok GOOD warna asal yang tidak pernah ada secara fisik, dan FIFO konversi dapat mengambil lot lain.
- Penempatan: change request terpisah dari blocker CP6. Sesuai L1697, dikerjakan sebagai successor tersendiri **sebelum** Stok per-SKU dan laporan CP7 bergantung padanya.
