...
-- :marker = 'yes' writes the SYNC marker (else: installed after the facts, the contractor pool keeps the constant).
begin;
\ir common_lot.sql
...
update erp.hpp_versions set total_cost=case when is_current then 90.5 else 100 end;
