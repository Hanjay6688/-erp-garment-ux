\set QUIET 1
\pset footer off
-- Realistic order: the recost core updates every pocket-fabric movement first, so the rebuild triggered by P1's fact
-- already holds P1+P2 (-10.00); the rebuild triggered by P2's fact changes nothing (AY sync #2: dh=0, dp=-1.00).
...
insert into erp.pocket_period_destinations(pool_id,po_id) values('00000000-0000-0000-0000-00000000f001','00000000-0000-0000-0000-0000000000a0'),('00000000-0000-0000-0000-00000000f002','00000000-0000-0000-0000-0000000000a0');
...
create function pg_temp.pool_recost(p_day date, p_hpp numeric) returns void language plpgsql as $f$
declare v uuid;
begin
 ...
 insert into erp.hpp_versions(id,lot_id,version_no,qty_basis_pcs,total_cost,is_current,calculated_at)
  select gen_random_uuid(),lot_id,version_no+1,10,total_cost+p_hpp,true,statement_timestamp() from erp.hpp_versions where id=v;
 insert into erp.hpp_version_components(hpp_version_id,component_type,total_cost,source_type)
  select h.id,'OTHER',(select total_cost from erp.hpp_version_components where hpp_version_id=v)+p_hpp,'POCKET_PERIOD_ALLOCATION' from erp.hpp_versions h where h.is_current;
 perform erp.sync_po_hpp_to_gl('00000000-0000-0000-0000-0000000000a0',p_day);
end $f$;
select pg_temp.pool_recost(current_date-5,-10), pg_temp.pool_recost(current_date-2,0);
