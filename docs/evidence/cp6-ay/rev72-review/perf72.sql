...
insert into erp.hpp_versions(lot_id,version_no,qty_basis_pcs,total_cost,is_current,calculated_at)
 select id,1,100,1000,false,now()-interval '2 hour' from erp.fg_lots union all select id,2,100,850,true,now()+interval '1 hour' from erp.fg_lots;
...
-- rev7.2 load: :nev revaluation events on other movements, 2 events per movement of this PO after the state; pocket components
insert into erp.material_cost_revaluation_events(movement_id,delta_amount,created_at,effective_date) select gen_random_uuid(),1,now()-interval '10 day',current_date-10 from generate_series(1,:nev);
insert into erp.material_cost_revaluation_events(movement_id,delta_amount,created_at,effective_date) select id,0.5,now()-interval '30 minute',current_date-(k*3) from erp.material_stock_movements, generate_series(1,2) k;
insert into erp.hpp_version_components(hpp_version_id,component_type,total_cost,source_type) select id,c,10,case when c='OTHER' then 'POCKET_PERIOD_ALLOCATION' else 'X' end from erp.hpp_versions, unnest(array['OTHER','MATERIAL','LABOR','ACCESSORY']) c;
insert into erp.pocket_periods values('00000000-0000-0000-0000-00000000f001',current_date-100,current_date-50);
...
\if :idx
create index on erp.material_cost_revaluation_events(movement_id); create index on erp.hpp_version_components(hpp_version_id);
\endif
...
\timing on
select erp.:fn('00000000-0000-0000-0000-0000000000a0',current_date-800);
\timing off
