\set QUIET 1
\pset footer off
-- Variant: ONE pool P1 (d-10..d-5) whose pocket fabric came from two adjustments; the invoice recosts both, so
-- guard_pocket_period_v1 fires twice for P1 in one statement (two RECOST journals on d-5, two rebuild+sync).
...
select pg_temp.pool_recost(current_date-5), pg_temp.pool_recost(current_date-5);
