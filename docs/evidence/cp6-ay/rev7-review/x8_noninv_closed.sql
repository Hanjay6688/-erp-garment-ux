...
begin;
insert into erp.settings select 'E',(current_date-6)::text where :'mode'<>'noninv'; insert into erp.settings values('SRCQ','0'); update erp.accounting_period_control set closed_through=current_date-6 where :'mode'='closed';
...
select jsonb_agg(x order by x::text) as_tot from (select j.transaction_date,j.economic_date,l.mapping_key,l.debit,l.credit from erp.journal_entries j join erp.journal_lines l on l.journal_entry_id=j.id) x \gset
rollback to savepoint s;
select erp.sync_po_hpp_to_gl('00000000-0000-0000-0000-0000000000a0',current_date-6);
select * from public.show();
select (select jsonb_agg(x order by x::text) from (select j.transaction_date,j.economic_date,l.mapping_key,l.debit,l.credit from erp.journal_entries j join erp.journal_lines l on l.journal_entry_id=j.id) x)=:'as_tot'::jsonb identical_to_as, (select count(*) from erp.journal_entries) journals, (select jsonb_agg(x) from (select effective_date,fg_delta,cogs_delta,other_delta from erp.po_hpp_gl_events) x) events;
rollback;
