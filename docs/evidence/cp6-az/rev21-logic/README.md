...
- Penghapusan bahan (item keluar penyesuaian bahan, bukan pemakaian kain kantong) yang dibalik diperlakukan sama, melawan akun penghapusan (OTHER_EXPENSE). Event dan state-nya dicatat pada gerakan pembalik supaya berada di luar buku kumulatif dokumen penyesuaian.

`run.sh` memasang fungsi apa adanya dari `supabase/dev/cp6_az_t1_family.sql` ke stub `schema.sql`, lalu menjalankan `scenario.sql` dan `writeoff.sql` pada tiga mode, serta `sequence.sql`. Hasilnya ada di `output_rev21.txt`:
