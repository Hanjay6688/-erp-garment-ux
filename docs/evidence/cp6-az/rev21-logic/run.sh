...
f(){ grep -v '^$\|^(.* rows\?)\|sync_material_cost_revaluation\|^-*$' || true; }
for s in scenario writeoff pocket; do for m in open closed noninv; do echo "=== $s mode=$m"; psql -d $DB -v ON_ERROR_STOP=1 -v mode=$m -f "$H/$s.sql" 2>&1 | f; done; done
echo "=== sequence (open, no invoice, open again at 8.00)"; psql -d $DB -v ON_ERROR_STOP=1 -f "$H/sequence.sql" 2>&1 | f
