...
- **ACC-C12 kunci baru: dua opsi untuk owner (handoff auditor R12 tugas 4).** GPT mereproduksi batas di atas (3 → 6 → 9: baris tertunda yang sama diimpor ulang dengan kunci custody baru dan terhitung lagi). Status ACC-C12 tetap **PARTIAL** sampai owner memilih salah satu:
  ...
  - **Keputusan owner D09 (26 Sep 2026): opsi (a).** Dikerjakan di keluarga BD (penggantian fungsi impor BC yang dicek, tabel `erp.bd_custody_sources_v1`); kolom `count_sheet` + `sheet_line` atau `source_lot` di berkas `OPENING_ACCESSORY_CUSTODY`. Kasus `D09:ACC_C12_SOURCE_IDENTITY` di probe BD (tabel kasus BD). Paket dan fungsi BC sendiri tidak berubah; pada rantai BC tanpa BD batas lama tetap ada (fase before probe BD: COUNTEREXAMPLE). ACC-C12 ditutup sesudah auditor memverifikasi.
