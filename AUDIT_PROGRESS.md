...
| cp6-auditor-scenario.yml | 36042210333 / 36042223829 | (ambil via API) | date_family_2 after / before — BERJALAN: fixture diperbaiki (potong d+1), kasus tertutup, dua potongan, probe S04 potong sebelum jam penerimaan di hari yang sama |
...
| F1-09 | P2 (sementara) | PRODUK | Potongan 4 unit pada d 08:00 dari roll yang baru diterima d 23:30 DITERIMA oleh RPC cutting, lalu invoice susulan ditolak `AM_BACKDATE_WOULD_CREATE_NEGATIVE_LOCATION_ROLL_HISTORY` (recost). Dua guard tidak konsisten: cutting tidak memvalidasi saldo historis per waktu (AUD-S04, M:6002-6026 "Validasi current dan histori adalah dua hal berbeda"), recost memvalidasinya, sehingga invoice tidak pernah bisa dibukukan. Diuji ulang eksplisit di date_family_2 kasus DATE2:S04. | AUD-S04 M:6002-6026 | run 36041422305 4 kasus INCOMPLETE (error identik di phase before) |
...
1. Ambil hasil run 36042210333 (date_family_2 after) dan 36042223829 (before): job log via GitHub MCP `get_job_logs` (API langsung diblokir proxy pada redirect blob) → parse baris JSON `{"group":...,"case":...}` → `audit/runs/auditor_<run>.json`, tabel d), temuan e) (khususnya DATE2:S04 → F1-09).
