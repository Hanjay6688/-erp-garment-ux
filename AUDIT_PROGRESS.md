...
**LANGKAH BERIKUTNYA (sesi baru langsung lanjut):** checkout `audit/cp6-final-20260924-gpt-a0bcadf`, cari run workflow T2 pemicu commit ini, catat run+3job IDs; setelah selesai baca per-case T2 native dan `primary_unchanged`, simpan ledger/commit+push. Baru berikutnya T3 Release Package dan rollback masing-masing satu workflow fase terpisah. Baca blok putaran9 teratas serta `out/gpt_r9_business_tool_run.md`, `out/gpt_r9_browser_rev2_run.md`, empat oracle hash. Pantau writer `claude/new-session-deapao`: head terakhir saat run ini d1bc8ad, BB–BE belum di-push. Setiap family BB/BC/BD/BE: review diff product vs acuan BA dan tooling, bekukan hash skenario audit dari 39ACC+36LAU+22ALL, satu workflow per fase (maksimal 8–10 job/agen), run disposable T1/T2/T3/rollback/browser yang relevan, catat run+job+hasil lalu commit+push. Khusus LAU-04 uji final sale harga UNKNOWN versus M:4475; D06 formal belum boleh disimpulkan dari kutipan ambigu, perbaiki sitasi M:1757 policy. Ratifikasi C6 setelah pencocokan owner; semua 75+22 tetap UNVERIFIED/HOLD sampai bukti native+HTTP/UI. Tidak sentuh main/competition/writer/hosted/legacy/production. Sesudahnya verifikasi peta ALL22 terhadap sumber register dan revisi C6 rev3 D06/LAU-04, buat gate 75+22 tanpa mengubah oracle sesuai produk writer BB–BE; tiap family baru dievaluasi terpisah di exact head. Seluruh CP6 HOLD, production_go=false. Jangan sentuh main/competition/writer/hosted/legacy/production.
